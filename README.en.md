...
| 🔁 **Wanting it fully automated** | Let an outer loop (`/loop`, etc.) run the intent → implement → reflect cycle automatically | [③ Run the whole thing automatically (full IDD)](#sec-full-idd) |
...
When any of purpose, target users, outcomes, scope, acceptance criteria, promises to preserve, or external contracts is unresolved, it is an important decision in its own right. A hard-to-reverse change and an effect on multiple packets are separate additional conditions for an important decision. The AI always gives an answer proposal, its rationale, and the condition that would change the recommendation, then obtains one of three outcomes: a decision, out-of-scope for this work, or scope-limited explicit continuation. A bare “OK” or “next” does not count as a decision or explicit continuation. When a work unit declares its expected size, a suspected declaration-implementation gap (overbuilt or thin) is warned exactly once during implementation (warning-only by default, strengthenable to a stop via a setting; nothing happens without a declaration).
...
So, to add something new to build while the loop is running automatically, **you just add one more packet**. Concretely, a person adds it in a **separate session** from the loop:

```
Separate session (human-operated): /intent-plan
...
Running `intent-plan` to add intent is done **by a human in a separate session** for a reason: it rewrites documents and assumes human approval (see the warning below). Keep the automated loop running as-is, and do just the approval work of adding new intent calmly elsewhere — that's the separation.

> ### ⚠️ The cost of automated operation (make sure you understand it)
>
...
- **`intent-graphiti-sync` is the entry for the explicit availability preflight of an optionally installed Graphiti and for the explicit sync of an allowed range of domain materials.** It does not sync during preflight, so preflight sends no documents and changes no external state. Sync runs only after an explicit request with range rules and the approval of a batch confirmation, and hard exclusions and secrets are denied before sending. If Graphiti is absent or stopped, existing Intent Planning, SDD, and implementation continue from canonical Markdown and source artifacts. Team use defaults to a local Graphiti per developer; on a shared Graphiti only a single writer syncs while other users stay search-only. Complete deletion runs only after an explicit confirmation of targets and impact. Each Intent Planning stage may run a read-only search narrowed to its own question only when needed (candidates carry provenance and effective periods), and confirmation happens on the canonical source. When the target work unit references the search contract, the stage-specific search conditions are also handed to cc-sdd requirements, design, tasks, and implementation through the export drafts.
...
- **Supplementing a missing specialist perspective** — only when an important decision lacks an internal basis, confirm the exact outgoing wording and information before research, then return source-backed candidates → [usage and failure handling](docs/guide.en.md#supplementing-a-specialist-perspective-missing-from-internal-material)
