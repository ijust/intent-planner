...
4. **`/intent-export-cc-sdd`** (or `/intent-export-openspec`) — converts the selected work unit into a draft for the implementation tool. The draft carries the acceptance material (expected behavior and how acceptance is measured), so the downstream requirements generation doesn't come out thin.
