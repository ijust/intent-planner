...
Only if you also want term-drift, accept its interactive prompt or pass `--with-term-drift`; intent-planner then installs or updates the compatibility-verified term-drift 0.2.5 for the selected agent. `--yes` consents only to the root-document append and is not term-drift consent. Status is reported as `not-installed`, `ready`, or `inconsistent`; `inconsistent` is split into `additive-compatible`, where the official installer can safely add missing components, `update-attemptable`, where the official update may migrate a known prior state, and `blocked`, where automatic processing is refused. `install-failed` describes the current attempt separately from filesystem health. New installation and safe additions are delegated to the official installer, while migration from a known prior version is delegated to the official update. Updates still require explicit consent: intent-planner does not automatically follow unknown self-consistent or future versions, and it never independently repairs or overwrites term-drift-owned files. Once `ready`, start the full terminology inspection from the dedicated term-drift skill in the selected agent.
...
- **`/intent-validate`** — reports contradictions, coverage gaps, and boundary mismatches across the intent documents with severity, read-only (it only proposes fixes; it never rewrites on its own). Running it once before export is a good safety check. If it finds project-local term-drift artifacts, `/intent-validate` neither judges terminology or health itself nor launches external commands; it tells you to manually run the existing installer's `npx intent-planner . --agent <selected-agent> --with-term-drift --dry-run`, and directs you to the dedicated term-drift skill only after it reports `ready`.
