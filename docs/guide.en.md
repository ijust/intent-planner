...
- **`/intent-status`** — summarizes where you are and recommends exactly one "next move". It lists all packets by five signals (reflected / you-are-here / not-started / unreflected / merged), annotating each row with "which stage now, and which stage next". It also warns about missed write-backs and packets suspected of being implemented without going through a packet.
...
├── milestones.md         # records of milestone events (one of the split records below)
...
- **The canonical glossary** — `.intent/glossary.md` (correct terms + spelling variants + a one-line explanation) collects "this is the correct term". It's a glossary you grow; the commands only read it and never rewrite it automatically.
...
- **Prevention** — the distributed convention document (CLAUDE.md / AGENTS.md) includes the discipline "don't invent words not in the glossary; if you do, attach a one-line explanation on first use".
