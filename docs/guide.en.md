...
| `intent-graphiti-sync` | Checking an optionally installed Graphiti and syncing an allowed range | Runs only on explicit invocation; it does not sync during preflight, sync runs only after range rules and an approved batch confirmation, and complete deletion only after an explicit confirmation of targets and impact. Team use defaults to a local Graphiti with a single writer on shared setups |
