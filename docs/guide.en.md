...
| `intent-graphiti-sync` | Checking an optionally installed Graphiti and syncing an allowed range | Runs only on explicit invocation; it does not sync during preflight, and sync runs only after range rules and an approved batch confirmation |
