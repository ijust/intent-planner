...
- **If your terminology is already full of coinages (optional, a separate tool)** — [term-drift](https://github.com/ijust/term-drift) finds not only words missing from the glossary, but also ordinary words borrowed into an in-group meaning, and fixes them using **only the rewordings you approved, one term at a time**. Only after explicit consent, intent-planner delegates installation or update of term-drift 0.2.5 for the selected agent to the official installer or official update. When a placement is found, `/intent-validate` does not judge terminology or compatibility itself; it guides you to run `npx intent-planner . --agent <selected-agent> --with-term-drift --dry-run` and inspect health and the planned operation through the existing installer. Start the full terminology inspection from the selected agent's dedicated term-drift skill only after health is `ready`. The dedicated path does not replace `/intent-validate`'s structural checks, but terminology inspection after placement has one entry: the dedicated skill.
...
npx intent-planner --lang en --agent codex --with-term-drift  # explicitly install term-drift for Codex too
```
...
| `--with-term-drift` | Pre-consent to installing or updating term-drift 0.2.5 for the selected agent (optional) |
...
term-drift is optional. intent-planner starts an external owner process only after you give explicit consent through the dedicated interactive prompt or `--with-term-drift`. `--yes` skips only the root-document prompt; it is not consent to install or update term-drift. A dry run never starts the external process and, only when requested, shows term-drift 0.2.5, the selected agent, the planned `install` / `update` / no-op, and why it would run or be suppressed.

Installation health is `not-installed`, `ready` (the version, rules, and selected agent's dedicated skill form a compatible set), or `inconsistent` (partial or mismatched). An `inconsistent` installation is `additive-compatible` when the official installer can safely add only missing components, `update-attemptable` for a trusted known state such as a complete match to the previously verified 0.2.3 baseline, or `blocked` when automatic processing is refused. `install-failed` describes the current owner-operation attempt separately from persistent filesystem health and is shown with post-health from the same inspector even after failure. `ready` is a no-op and is not reinstalled; the full terminology inspection starts from the selected agent's dedicated term-drift skill. For `blocked`, intent-planner reports the problem paths and does not recommend an immediate retry.

New installations and safe additions are delegated to the official term-drift 0.2.5 installer; updates from a trusted known baseline are delegated to the official update. If the owner refuses for safety, intent-planner reports post-health and does not independently roll back. Updates require explicit consent, and intent-planner does not automatically follow unknown self-consistent or future versions newer than 0.2.5. It never independently repairs, overwrites, or deletes term-drift-owned rules, skills, or glossary data.
