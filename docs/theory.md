...
intent-planner との接合は**疎**に保ちます。導入・更新は専用の `--with-term-drift` または対話中の明示同意だけを入口にし、選択中の agent 向けの term-drift 0.2.5 を公式 installer / 公式 update へ委譲します（`--yes` はルート文書の確認だけに効きます）。installer inspector は互換な一式を read-only で見て `not-installed` / `ready` / `inconsistent` を区別し、`inconsistent` を、安全に不足だけを足せる `additive-compatible`、直前に検証した 0.2.3 の完全一致など owner update を試せる `update-attemptable`、自動処理しない `blocked` に分けます。`not-installed` は install、`update-attemptable` は update、`ready` は no-op へ一意に対応します。今回の試行結果 `install-failed` と永続状態は混ぜず、owner が拒否または異常終了しても同じ inspector で post-health を取り直します。intent-planner は rollback せず、term-drift 所有ファイルを独自に修復・上書き・削除しません。未知の自己整合版や 0.2.5 より新しい将来版にも自動追随しません。

責務は三段に分けます。日常の予防はルート規約・質問前点検・export 注入で続けます。project-local な term-drift 配置物がない環境だけ、`/intent-validate` の軽い `coinage-suspect` を後方互換として残します。配置物が見つかったら、validate は用語の意味や version/hash/skill の互換性を判定せず、既存 installer の `--with-term-drift --dry-run` health 確認へ案内します。そこで `ready` と確認された後だけ、その agent の term-drift 専用 skill が本格点検の入口になります。これにより、validate の構造検査は保ったまま、health の正本を installer inspector 一つ、詳細な用語点検の入口を term-drift 専用 skill 一つに保てます。
