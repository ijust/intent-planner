...
intent-planner との接合は**疎**に保ちます。導入は専用の `--with-term-drift` または対話中の明示同意だけを入口にし、選択中の agent 向けの term-drift 0.2.1 を公式 installer へ委譲します（`--yes` はルート文書の確認だけに効きます）。導入後はその agent の term-drift 専用 skill が本格点検の入口です。intent-planner は互換な一式を read-only で見て `not-installed` / `ready` / `inconsistent`（安全に不足だけを足せる `additive-compatible` / 自動修復しない `blocked`）を区別し、今回の試行結果 `install-failed` と永続状態を混ぜません。安全な追加は owner の installer に任せ、既存導入の自動更新や term-drift 所有ファイルの独自修復・上書きはしません。この境界により、入れなければ従来挙動を変えず、検出アルゴリズムの正本も term-drift 側に一本化できます。
