...
intent-planner の価値は指示量を最大化することではなく、設計逸脱や統合手戻りの損失が大きい案件へ**最小十分な steering**を供給することです。Invariant、Scope / Acceptance、Decision、Preference / Heuristic の4段で拘束力を分けると、実装方法を細かく命令しなくても、AI は越えてはいけない境界を読めます。これが**境界付き自律**です。境界内は自律して進み、境界を越える変更だけを人間の判断へ戻します。

JIT の入力も同じ最小十分の原則に従います。今回の判断に必要な確定材料と、実装時に現実化した場合だけ再検討する候補を分け、無関係な Intent Tree・Compass・履歴の全量を常時注入しません。これは「情報を減らす」こと自体が目的ではなく、必要な判断材料を生存させたまま不要な指示を減らす設計です。したがって、小さな試作や vibe coding で十分な案件にまで同じ工程を課すことは目的外であり、`direct` または省略可能な経路を残します。

進捗表示も一つの健康判定に畳みません。**工程の状態**、**未決の設計判断**、**利用者成果**は異なる観測です。工程が green でも利用者成果が正しいとは限らないため、成果の証拠がなければ「未観測」と表示し、総合PASSやスコアを作りません。代わりに、次に人が判断すべき一点を前面に出します。
...
|---|---|---|
| spec → Intent（内向き） | 仕様書が書かない why / invariant / anti-direction（問題空間のもの） | 既存の外部仕様書や断片メモを入力に取り、沈黙を物差しに照らしてギャップとして表出する（`/intent-from-spec`） |
...
> **正直な注記（現状の実装範囲と未確立の主張）**: 上表の3辺はいずれも実装済みです — Intent → code（drift 検出）は drift-watch / improve、spec → Intent（沈黙検出）は `/intent-from-spec`、Intent → spec（捏造抑制つき自然言語 Spec 生成）は `/intent-to-spec`。内向き `/intent-from-spec` は `/intent-discover` の「対話で集める」を「文書から読む」に差し替えた入口で、出力は確定ではなく仮説（Assumptions）として omission recap を経て人が確認します。入力はまとまった仕様書に限らず、断片メモ・走り書き・音声書き起こしも受けます — その場合は話題ごとの束ね（クラスタリング）と「決まっている／迷っている」の仕分けを抽出の前段に挟み、頭が散らかったままの人でも discover の質問に答えられる状態まで整えます（仕分けの印もすべて推測標識付きで、確定は本人のレビューに委ねます）。なお「仕様書が黙って落とした不変則のうち何%が後の実欠陥に対応していたか」は **やれば測れる経験的仮説**であって測った結果ではありません — その測定は git 履歴を使えば設計できる、というところまでが現状です。

外向きの射影は、行き先が spec / code とは限りません。**上流の Intent layer は format 非依存**で、プログラムへの結合は下流に局在しています — export（spec 駆動コーディングツールへの受け渡し）と packets の検証語彙が、プログラム前提を担う層です。したがって非プログラム案件（文書・業務・研究など）への対応は、上流を再設計する問題ではなく、**下流に縫い目を1本足す**問題に縮みます。具体的な経路は2本の併用です — (a) `/intent-to-spec` に非プログラム向けの target format を足し、export を迂回して `.intent/nl-spec/` 配下に読める成果物を出す。(c) 非プログラム専用モード `non-code` を置き、プログラム前提の degrade を切る。これは「損失付き射影」の **外向き拡張** にあたります — spec / code 以外の成果物（文書・業務・研究）へも Intent を射影できるようにする一手であって、別の仕掛けを足しているわけではありません。format 非依存という同じ性質から、**読み手別の target format** も同じ縫い目に足せます — `/intent-to-spec` の「ステークホルダー一枚もの」「定例報告」は、同じ三層の素材を**結論先行（BLUF）**で組み替え、内部識別子（Invariant / Decision Rule / packet id）を読み手向けの普通の言葉へ開いたプリセットです。射影元は同じで、並べ替えと語彙の開き方だけが読み手に応じて変わります（新しいスキルを足さず、format ルールを1枚足すだけで済む）。「意思決定メモ」も同じ縫い目のプリセットです — compass に残る判断（採用・不採用と理由・見直し条件）を採否先行の1枚に組み替え、素材に無い評価・採否を補完しません（迷いがまだ compass に書かれていなければ生成せず、先に詰めるよう案内します。不採用の理由が採用と同じ紙面に残ることで、決着済みの案を蒸し返すループを止めます）。この「語彙の開き方」は、**読み手が誰かで分岐する共通の規律**として全ての読み手別 format に効きます — 読み手がコードを書かない当事者外の相手（顧客・経営・営業など）のときは、内部記号（Invariant / Decision Rule）を既存の記号→ラベル台帳（`scripts/symbol-labels.json`。配布物向けの publish 時ラベル置換と同じ台帳を流用）で普通の言葉へ言い換え、台帳に短名の無い識別子（packet id・スキル名など）は本文から**省きます**（読み手が知らない語を推測で言い換えず、素材に書かれた分だけを写す捏造抑制の姿勢）。読み手が repo の当事者のときは、記号は辿るための参照として従来どおり残します。開き方の本体は1枚の rule に一本化し、各 format はそれを参照するだけなので、同じ規律が format ごとにずれません。読み手に加えて、**どこまで詳しく書くか（詳細度）**も同じ射影の軸として持てます — 範囲（どの素材を使うか）・体裁（どう並べるか）とは直交する第3の軸で、`概要 / 標準 / 詳細` の3段を取ります。詳細度を上げても新しい format を増やさず（度合いの調整であって別の器ではありません）、深くするのは**読み取る素材の範囲を広げること**です — 概要は現状どおり packet の証拠欄まで、詳細では packet 本文（期待挙動・守るべき制約・受入基準）まで素材にします。ここで肝心なのは、深さを**素材で稼ぐ**ことです — 流暢さのために素材に無い記述を足して厚く見せる方向（捏造）は、詳細度を上げても禁じられ、トレース照合はどの詳細度でも同じに効きます（外向き射影の load-bearing な課題が「捏造の抑制」である以上、厚くする軸こそこの規律を外せない）。詳細度が指定されていないときは、**生成前に一度だけ**どの深さで書くかを確認します — これは体裁が無指定なら黙って既定を使うのと**意図的に非対称**です。体裁の取り違えは読めば分かりますが、厚みの不足は「何が欠けているか」が読み手から見えないため、明示だけでは救えないからです（読み手向けに情報を絞る一枚もの・定例報告・意思決定メモは、絞ること自体が体裁の核なので厚み固定で、詳細度を適用しません）。

外向き射影のもう一つの行き先は **git の履歴**です。`/intent-release-note` は、git のコミット履歴を read-only で読み、各コミットを intent（packet 名 / parent intent / deltas / milestones）と照合して「**なぜ変わったか**」を補い、changelog 風 / GitHub Releases 風、あるいは**顧客向け changelog**（利用者に何が変わるかを先に書き、内部実装の語を開く**利用者影響先行**のプリセット）、さらに **PR 説明の下書き**（pr-description format＝コミット範囲を意図と照合し「意図の要約・関係する判断基準・レビューの見どころ・コミット↔意図の対応一覧」の4部で組み立てる。GitHub へは書き込まず貼り付けは人が行い、外へ出すのは要約+識別子まで＝packet/compass 本文を転記しない）の読み物として `.intent/release-note/` 配下へ射影します。これは「Intent → code」の辺を**逆向きに読む**操作です — code（git 履歴）には「何が変わったか」は残りますが「なぜ・どの意図のために」は落ちています。その落差を、intent と照合して埋め戻すのが release note の射影です。照合には二つの経路があります。**記録に基づく対応**は、コミット作成者がメッセージ末尾に任意で残した意図参照（Intent trailer＝`Intent: <packet 名> (<packet_id>)` の形。Git 標準の trailer）で、作成者自身が明示した対応です。trailer が無いコミットは従来どおり**テキスト照合**で intent に当てにいきますが、これはツールが後から推定した**推測**です。release note はこの記録に基づく対応と推測による対応を混同せず区別して見せます — 書いた本人が示した対応と、機械が当てた対応は確度が違うからです。ここでも照合は status と同型の温度に保ちます — 機械スコアリングは持たず、テキスト照合（推測）で確信が低ければ断定せず候補として提示し、**意図に紐づかないコミットは黙って捨てず薄い行で残して落差そのものを見せます**（紐づかない変更が多いほど「意図に載っていない実装」が可視化される）。trailer は**任意**で、その有無を咎めません（付け忘れを警告せず、trailer 無しリポでは従来のテキスト照合だけで動く＝挙動が変わらない）。trailer には識別子（packet 名・packet_id）だけを書き、機密は書きません（コミット履歴は公開されうる）。git は読むだけ（commit / tag / push をしない）で、出力は再生成可能な派生物に閉じます。この trailer は「意図 → コミット」の対応を、事後の推測照合から**コミット作成時の記録**へ変える一手で、意図と実装の対応を記録から追跡する試みの入口です。なお「コミットのうち何割が intent に正しく紐づくか」「紐づかない変更の割合がプロジェクトの健全性とどう相関するか」は **やれば測れる経験的仮説**であって、測った結果ではありません — 現状は落差を可視化する射影を置いたところまでです。
...
ただし「添えて渡す」だけでは、下流の生成過程で意図は静かに減衰します。本リポジトリの dogfood（36 export × 下流 spec の突合・2026-07）の実測では、下書きに載せた意図記号のうち下流の requirements に残るのは約3分の2で、生成された各タスクへ不変則参照が転記される率は1割強でした。減衰の主因は構造です — 受入基準の材料（packet の期待挙動・適合基準）が「下流に自動の読者がいないヒントファイル」へ写像されており、下流の requirements 生成は Project Description だけから受入基準を書くため、薄くなるか創作するかになる。そこで注入を2点で構造化しています。(1) requirements 下書きに**受入基準の材料**の節（`## Acceptance Material`）を必須で置き、期待挙動と適合基準を要求度が一意に読める語（RFC 2119 の使い分け）で**転記**する — 下流が受入基準を創作でなく「転記＋整形」で書けるようにする（転記に留め、EARS 完成形には踏み込まない＝下書きの範囲）。(2) design / tasks フェーズへ進む際に該当ヒントファイルの本文を渡すことを受け渡し案内で明示する — 下流ツールはヒントを自分では読まないため、この手渡しが唯一のフェーズ間経路になる（渡されなくても止めない）。伝播は引き続き保証ではなく確率ですが、確率を上げる手段はすべて intent 側（下書きと案内）に置き、下流ツールは改造しません（アドオン原則）。
...
技術的な定石を read-only で照合するときの堰は、サービスデザインの確立したフレームにもそのまま広げます。ロールレンズで読み取った案件の性質とカタログの適合条件を意味で照合し、道具の名前が似ているだけでは候補にしません。

適合が弱いときは候補を提示せず沈黙し、人が採用・見送り・保留の採否を決める形に留めます。採用前には何も生成しないままにし、採用後だけ `.intent/nl-spec/design-frame-<frame-id>.md` へ再生成可能な派生下書きを作ります。Intent Tree・Intent Compass・packet の正本は自動で変更しないため、候補の提示が設計判断の確定へすり替わりません。

画像や図の生成、アナリティクスによる実測、体験段階の追加、数値の優先度、日付コミット、進捗率は、この拡張の対象外です。確立したフレームは問いを補助する道具であり、プロジェクト管理や行動計測の仕組みへ責務を広げません。
...
この「全面的な能動質問化はしない」姿勢は**既定の詰めの深さ（question-depth=standard）**の規律であって、上限そのものではありません。「もっと掘ってほしい」利用者のために、詰めの深さには **deep（深掘り）** という一段があり、利用者が明示的に選んだときだけ、問う**範囲**を広げます（機能の中身・前提・エッジケース・反例・非機能まで能動的に問う）。deep は明示 opt-in の**例外**であり、既定 standard の挙動・体験は一切変えません。そして deep でも歯止め — 1回最大4問・毎回「後で／不明／該当なし／その場で終了」を選べる・尋問調にしない・値を先に置かない（anchoring 回避を継承） — は外しません。deep が広げるのは問いの**範囲**であって、圧ではないからです。あわせて、質問をするかどうかの入口の問い（designer-questions の要否確認）も、仕組みの内部説明を先に並べるのではなく「質問をしてよいか」をストレートに問い、選択のトレードオフ — 質問を受ければでき上がりの正確性が増し（意図と違うものを作るリスクが下がる）、飛ばせば開発速度が上がる代わりに AI が利用者の思い描く仕様と異なるものを作る可能性が残る — を明示して、利用者が結果を理解して選べるようにします。
...
**draft-content-dropped** は、export 下書き（`.intent/cc-sdd/<スラッグ>/`）と、それを渡して下流の spec ツールが生成した成果物（`.kiro/specs/<feature>/`）を読み比べ、下書きに載せた中身が下流で落ちていないかを名指しする検査です。上の `requirement-oracle-check` が「下書きに載せる**前**の基準が誤実装を弾けるか」を見るのに対し、こちらは「下書きに載せた**後**、下流の生成物にそれが残ったか」を見ます — 同じ lossy projection を、変換の前後という別の面で拾う姉妹の軸です。見るのは2つ、下書きが引いた意図の参照（守る約束・判断基準・親の意図）が下流の requirements / design / tasks に残っているか、そして受入基準の材料（`## Acceptance Material`）由来の判定条件が下流の受入基準に現れているか。突合の相手は、export 記録の分割ファイルに残る feature 名の追記行を**記録された対応の手がかり**として同定し、その記録が無ければ packet 名の包含で補助照合し、どちらでも同定できないときは「突合不能」と告げて**推測では対応づけません**（誤検知を出すくらいなら黙る側に倒す）。生存率の数値化・閾値判定は持たず（markdown のみ・DB を持たない核）、どの参照がどのファイルで落ちたかを名指しするところまでです。下流に spec が無い案件（直接実装する案件）では軸ごと沈黙し、「落ちなし」と誤標識しません。同じく read-only・候補提示・誤検知前提で、export も実装も止めません。この軸が要るのは、写像（下書きが受入基準の材料を運ぶ）と案内（フェーズごとにヒントを手渡す）で注入の確率を上げても、伝播は保証ではなく確率のままだからです — 構造で確率を上げ、残る減衰は検査で見張る、という役割分担になっています。
...
- **トリガ＝Revisit when 発火の拾い上げ** — Decision Rules の `Revisit when`（[有効期限つき決定](#decision-rules--軽量-adr-と設計根拠)）は「成立したら知らせる」ところまでを仕組みにする欄です。`/intent-improve` の coherence 軸が、実装の現実と deltas（実装から得た学びの記録）から条件の成立を読み取り、見直しを再提案します。
  かつてはこれに加えて、利用者が節目イベント（「本番構成を確定」等）を宣言的に記入する台帳 `.intent/milestones.md` を置き、その各 event を全 Decision Rule の `Revisit when` と文字列照合する経路を併走させていました。**これは畳みました。** 実測すると設計の前提が二重に成立していなかったためです — (1) 本リポジトリの `Revisit when` 143 個を全数調査したところ、「本番構成の確定」のようなリポジトリの外で起きる出来事を条件にした Rule は実質ゼロで、ほぼ全てが「〜と観測されたとき」「誤検知が多いと分かったとき」というリポジトリの内側と対話から拾える条件でした（＝照合の発火先が構造的に存在しない）。(2) 台帳への人間の自発記入もゼロのまま推移しました。**発火先の無い照合と、読み手の無い台帳は畳む** — 「気づかせる仕組みを横に足す」より、既に材料と行き先が一致している経路（deltas 起点の検出）を残すほうが強い、という判断です。節目そのものは、学び＝deltas・完了＝packet の archive・コミットとの対応記録＝Intent trailer・外向きの語り＝release note が既に受けています。
...
### 実測が示した3つの穴（2026-07・予防の多層化・語の身分・救済）

実プロジェクトで「生成された設計文書の語彙をそのまま引用した質問が、エンジニアの利用者にも通じない」事故が起き、上の設計に3つの穴が見つかりました。いずれも**実測から**の修正です。

- **指示は文脈が深くなるほど減衰する** — 「普通の言葉で話す」規律は会話の入口に常駐していたのに、下流の実装フェーズの質問では発火しませんでした。**常駐指示の増強だけを対策にしない**（それはこの案件が否定した当の解法です）。予防は「生成の瞬間の軽い指示」と「事後の検査」を**必ず対で**置き、指示の届く場所を多層化します — 会話入口の規律に加え、(i) 下流の spec ツールへ渡す下書きへ言葉の規律を同梱して運ぶ、(ii) 利用者へ問いを出す直前の点検を各コマンドの手順に埋める。
- **語の身分は語の内在的性質では判別できない** — 「これは造語か」を語そのものの性質で決めようとすると、英語圏の慣用語（footgun / hydrate）と真の内輪語の線引き、一般語と衝突する製品名（Go / Zen）で破綻します。判別は**運用的に**決めます — チーム共通語＝台帳に承認済みで載る語／勝手語の疑い＝台帳に無い・暫定の語／一般語＝読み手の基礎語彙側。つまり「誰の言葉か」は**台帳のステータスと読み手が誰か**で決まり、検査は読み手を必ず入力に取ります（用語ガバナンスの業界標準形＝推奨/許容/非推奨の3状態と同型。専門用語かどうかの判定が読み手依存であることは研究でも実証されています）。台帳に**否認済み**の状態を残すのは、消してしまうと同じ語が後から作り直されるからです（否認の記録が再発明を防ぎます）。ここで最も警戒するのは**承認洗浄** — AI が「承認を取ること」を目的化し、人の認識が甘いまま一括・流れ作業の承認で造語を公認語にしていく経路です。だから登録・昇格は**1語ずつ・一行説明の確認を伴う個別承認のみ**で成立させ、まとめ承認の入口を構造的に作りません。
- **台帳照合だけでは最も危険な造語を素通しする** — 実在リポジトリ2件を手作業で調べたところ、真の勝手語の主形態は「台帳に無い発明語」ではなく、**普通の言葉の内輪転用**（「配線する」「drain する」「settle する」）でした。これは辞書照合では原理的に拾えません。検出は**多層**にします — 台帳差集合＋比喩転用＋定義への到達可能性（語は定義されているが、使う場所からその定義に辿り着けない「置き場所ズレ」）。

### 救済 — すでに造語で埋まったプロジェクトを戻す（term-drift）

予防と検出は「これから」に効きますが、**すでに造語で埋まったプロジェクト**（AI 支援開発の産物として世に多く存在します）は救えません。そこで救済は独立ツール [term-drift](https://github.com/ijust/term-drift) として外に置きました（intent-planner を使っていないプロジェクトにも届かせるため）。

構成は**ハイブリッド**です — 走査→**検出・3分類・言い換え提案（LLM の読解）**→**人が1語ずつ承認**→**置換の適用・再検査（決定的照合）**の反復ループ。検出層に LLM が要るのは比喩転用が辞書照合で拾えないからで、適用層を決定的にするのは再現性のためです（判断は LLM・適用は機械、という役割分担は本体の「記号→ラベルの機械置換」と同型）。安全の一線は、承認していない置換は1バイトも書かない・git 管理下でのみ適用する（可逆性）・実行時に外部へ通信しない・秘密ファイルを走査しない・判断に迷う語は黙って推測せず**すみやかに人へ確認する**、の5つです。

intent-planner との依存方向は**一方向**に保ちます。term-drift 0.3.0をexact npm dependencyとして標準導入し、対象repositoryとagentが確定する通常CLIからinstalled owner CLIの公式 installer / 公式 updateへ委譲します。ownerは`./.term-drift/`とagent専用skillをproject-localに配置し、intent-plannerはasset本文を複製しません。旧`--with-term-drift`は既存scriptの互換性のため受理しますが、配置のgateではありません。`--yes`は従来どおりルート文書追記だけを制御します。installer inspector は互換な一式を read-only で見て `not-installed` / `ready` / `inconsistent` を区別し、`inconsistent` を、安全に不足だけを足せる `additive-compatible`、検証済みの 0.2.3 / 0.2.5 の完全一致など owner update を試せる `update-attemptable`、自動処理しない `blocked` に分けます。`not-installed` は install、`update-attemptable` は update、`ready` は no-op へ一意に対応します。今回の試行結果 `install-failed` と永続状態は混ぜず、owner が拒否または異常終了しても同じ inspector で post-health を取り直します。intent-planner は rollback せず、term-drift 所有ファイルを独自に修復・上書き・削除しません。未知の自己整合版や 0.3.0 より新しい将来版にも自動追随しません。

0.3.0 では、1語ずつ確認した承認済みの一般語分類を台帳の任意列へ保存し、次のセッションで復元できます。これは既定の読み手に対する語の分類だけを保存するもので、同じ字面を内輪の意味へ転用した箇所や曖昧な文章まで承認済みにはしません。分類の読み取り・保存・再開判断は term-drift の rules / skill が所有し、intent-planner はその本文を複製せず、公開asset一式の互換性だけを検証します。

責務は三段に分けます。日常の予防はルート規約・質問前点検・export 注入で続けます。project-local な term-drift 配置物がない環境だけ、`/intent-validate` の軽い `coinage-suspect` を後方互換として残します。配置物が見つかったら、validate は用語の意味や version/hash/skill の互換性を判定せず、通常 installer の `--dry-run` health 確認へ案内します。そこで `ready` と確認された後だけ、その agent の term-drift 専用 skill が本格点検の入口になります。これにより、validate の構造検査は保ったまま、health の正本を installer inspector 一つ、詳細な用語点検の入口を term-drift 専用 skill 一つに保てます。
...
進捗には単一の万能指標がありません。要求の充足、実装の完了、証拠の有無、依存の解消は、それぞれ別のものを測っています。intent-planner の俯瞰（`/intent-overview`）は進捗を1つの総合％に圧縮せず、**意図の安定度 / 実現の完了度 / 証拠の確定度**という性質の異なる軸に分けて提示します。これにより「実装は進んでいるが証拠が未確定」「証拠は揃ったが上位意図とのギャップがある」といった、潰すと見えなくなる状態を表現できます。俯瞰にはもう一つ、対象範囲を指定したときだけ生成する**コードから意図を逆引きできない領域一覧**があります — ここでいう逆引きとは、指定範囲の各コード領域を起点に、packet の対象範囲・Invariant の影響パス・コミットと意図の紐づきという3種類の記録から、対応する意図を探すことです。どの記録からも対応する意図を確認できなかった領域を、根拠付きで列挙します（判定・スコア化はせず観測の列挙まで。空白が正当な領域もあるため「空白＝悪」とは断定しない）。既につながっている箇所を記録から辿れるようにするだけでは見えない「そもそも意図が無い所」を地図にする一手で、逆読み（コード→意図）をどこへ向けるかの優先判断の材料になります。さらに、求めたときだけ生成する横断の派生ビューが2つあります — **判断待ちインボックス**は、散在する未回答の Open Questions・承認待ちの delta・追跡された警告を「いま人の判断を待っている問い」として出所リンク付きに一覧化し、溜まった判断を一望で捌けるようにします。**ロードマップ射影**は、active packet の順序（`depends_on` の前後関係）・状態・ブロッカー連鎖・保留の区分を**日付なし**で束ね、前向きの作業順序を1枚に映します。この「前後関係」は依存（`depends_on`）＝技術的な前提だけでは表せない場面があります — 「まずログイン、次にリマインダ」という**工程のまとまりと優先度**は人の計画意図で、依存関係とは別物です。そこで `.intent/packets/plan.md` に任意の**工程計画**の節を置けます。グループの見出しは利用者の言葉で自由に書け（`Phase`・`step`・`期` など何でも・入れ子可）、番号付きの並び順そのものが優先順位（同番号は同順位＝並行着手可）。ツールはこれを read-only で読んで推奨着手順を導く（上から順・`done`／依存未解決／他セッション着手中を飛ばす・矛盾したら `depends_on` が常に勝つ）だけで、順序を強制したり数値スコアを付けたりはしません。これも「PM ツール化しない」床の内側（並び順＝優先で、日付・点数を持たない）で、書かなければ従来どおり動きます。同じ「求めたときだけ」の系譜に、意図の全体像と作業順序を GitHub / VSCode がそのまま描画する **Mermaid 図射影**（掴むためのマインドマップ系の図と、`depends_on` の閉塞を映すロードマップ図。図にも日付・ガント・進捗％を持ち込まない）と、`.intent/assignments/` の割当宣言から「どの packet を誰が実装中か・同じ packet への二重着手はどこか」を read-only で映す **割当ビュー**（名指しの warn まで・止めない・機械閾値で自動解放しない）が加わります。同じ系譜のもう一つが **引き継ぎブリーフ** です — セッションが長くなった切れ目で新しいセッションへ乗り換えるとき、`.intent/` に直列化済みの状態から「現在地・残タスク・正本への pull ポインタ・罠・repo 状態・実走オラクル」の6要素を短い引き継ぎ文書として派生します。ここで効くのが intent-planner の非対称です — 会話履歴の要約や自動 compaction はハーネスの領分（本ツールは会話ログを読まずトークンも測りません）で、intent-planner が担うのは「状態が最初から `.intent/` に直列化されているからこそ安く正確に出せる引き継ぎの中身」です。ブリーフは使い捨ての派生で、生成時点を明記し「正本が常に勝つ」と告げ、その瞬間のローカル事情（未コミット・並行セッション）を載せられるよう git 非追跡の置き場（`.intent/handoff/`）に置きます。いずれも「PM ツール化しない」床（日付・ガント・ベロシティ・進捗％を持たない）を守り、正本を書き換えず生成時点のスナップショットとして機能する read-only な派生です（この床が、状態の外部化が保証しない鮮度・唯一性を、materialized view と single source of truth の分離で扱う後述の姿勢の実装面です）。
...
**出口は案件種別と導入状況で選ぶ（決め打たない）。** 複数の出口があるとき、どこへ進むかは案件の性質と「そのリポジトリで実際に使えるツールはどれか」で変わります。`/intent-packets` の「次の一手」は cc-sdd を一律に推さず、**target format**（`.intent/mode.local.md` の任意 `format` 行＝ `cc-sdd` / `openspec` / `speckit` / `to-spec` / `direct`）と mode・**下流 spec ツール3つの導入目印**（cc-sdd=`.kiro/` / OpenSpec=repository-root `openspec/` / Spec Kit=repository-root `.specify/` の有無を read-only で観測）から出口を選びます。format が明示されていればその出口（OpenSpec 案件なら OpenSpec を、Spec Kit 案件なら Spec Kit を、文書など読める成果物が目的なら `/intent-to-spec` を、spec ツールを使わず直接実装する案件なら `direct` を）を推し、未指定なら案件種別と導入状況から推論し、一意に決まらなければ候補を列挙します（出口は利用者の意図次第なので断定より提示）。判定の正本は出口判定レーン（`intent-packets/rules/export-route.md`）に1つだけ置き、各 export スキルはそれを参照します。

推論のとき、候補は**導入済みのツールを先に・未導入のものは「導入が要る」注記付きで後ろに**並べ、各出口に案件適合の一言（3段承認で実装まで運ぶ／提案を立てて合意してから実装する／Spec Kit の仕様→計画→タスクで実装する、など固定の短文）を添えます。ここで守っている線は3つあります。第一に、**導入状況は観測と注記であって gate ではない** — 未導入を理由に候補から消さず、判定も export も止めず、インストールを代行しません（後からツールを入れる経路を潰さない。前提不在の preflight が warn だけして止めないのと同じ哲学）。第二に、**導入済み同士に優先順位を発明しない** — 「導入済みを先に」は導入*状態*による区分であって、ツール*間*の序列ではありません。複数が導入済みなら候補列挙のまま人が選びます。第三に、**導入検知をレジストリ・検知スクリプト・案件適合の自動診断エンジンへ抽象化しない** — 目印はディレクトリの有無を読むだけの観測で、適合の説明は固定の短文で足ります（判定の意味的な性質を機械検査へ寄せない設計と同線）。これは「cc-sdd が導入されていないのに cc-sdd を筆頭に推され、導入済みの OpenSpec や Spec Kit が候補に現れない」という落差を、観測の解像度だけを上げて吸収する最小の拡張です。format の書き手は `/intent-discover` のみで、mode/designer-questions/purpose と同じ追認規律（任意・保留可・推測で埋めない）に従います。さらに各 export スキルは前提が見当たらないとき（cc-sdd=`.kiro/` 不在 / OpenSpec=repository-root `openspec/` 不在 / Spec Kit=repository-root `.specify/` 不在）に **warn だけして止めません**（drift-watch と同じ誤検知前提の warn-only。前提を後から入れる経路を潰さない）。これは「出口が1つに決め打たれて案件と噛み合わない」という落差を、判定の単一正本と warn-only の案内で吸収する設計です。なお `direct`（ツールを使わず直接実装する案件）を選んでおくと、実装後の `/intent-writeback` がその記録を**対象特定の一次情報**として使い、cc-sdd / OpenSpec を経ない案件でも書き戻し対象の packet をテキスト照合フォールバックに頼らず一意化できます（記録が無い案件は `spec_refs 空 + export-log 行なし + state=done` の推論にフォールバックします）。
...
- "Scalability of iStar: a Systematic Mapping Study," WER 2016（119論文の系統的調査で i* のスケーラビリティを「悪い」62本 対「良い」8本と集計＝単一の巨大なゴールモデルを育てる方向のスケール限界）
...
compass は 1 記号 1 ファイルの split store を新規既定とするが、既存利用者の旧単一形式を恒久的に読み取る。移行は opt-in・非破壊で、installer は user-data を移送しない。

# 領域ガバナンス（federated governance）— 実体は割らず、ガバナンスだけを領域単位に委譲する

意図の正本（compass / tree）は、案件が増えるほど2つの点でスケールに圧されます。**並行する複数のセッションが同じ正本へ書き込むときの衝突**と、**維持のループ（validate / improve）の実行コストが記号総数に比例して増えること**です。本リポジトリ自身の実測でも、一人の開発者と並行 AI セッションの運用だけで compass の記号は800件を超え、全量走査を前提にした維持ループは実行の頻度が現実に追いつかなくなりつつありました。

この問題に対する先行事例の帰結は一貫しています。まず、**ゴールモデルを単一の巨大な木として育てる方向はスケールしません** — i* の系統的マッピング（WER 2016）は、119論文中「スケーラビリティが悪い」62本に対し「良い」は8本と集計しており、KAOS でも大規模適用でモデルが不完全・複雑化することが知られています。一方、大規模で実証されてきた解は、Linux カーネル（単一ツリー＋サブシステムごとのメンテナ階層）、Python の PEP（委任された決定者への権限委譲）、Google のモノレポ（単一リポジトリ＋API 既定 private の可視性ルール）のいずれも同じ形をしています — **実体は単一のまま保ち、ガバナンス（誰が決め・どこまでを対象に実行するか）だけを分割統治する**。実体そのものを分割する方向（いわば意図のマイクロサービス化）の成功例は、調査では見つかりませんでした。

intent-planner の領域ガバナンスは、この形を Data Mesh の **federated computational governance**（Dehghani）から借ります — 中央には全領域共通の最小限の規律だけを置き、個々の判断権限は領域へ委譲する。具体的には、compass の横断規律（`always` タグ＝全領域に効く少数の判断基準）だけを中央に保ち、**領域**（各記号に付いた領域タグの値）ごとに所有権と実行スコープを委譲します。

実体を割らない理由は3つあります。(1) **1回の grep で「案件の領域 + always」を横断的に引ける**性質が JIT 供給の基盤であり、実体を分割するとこの検索性が壊れます。(2) 複数領域にまたがる変更が「複数リポジトリの同期問題」に化けます（Google がモノレポに固執する理由と同型）。(3) 外部連携を片方向に留め双方向同期を背負わない、という本ツールの姿勢と衝突します。分割のイメージとしては、マイクロサービスではなく**モジュラーモノリス**に近い形です。

機構は4つで、いずれも既存機構の意味を強めるものであって新しいレイヤーではありません。

- **領域スコープ実行**: validate / improve が、案件の領域が定まるときは「当該領域 + always」の記号だけを読んで実行します（維持ループのコストが記号総数に比例しなくなる）。ただし、陳腐化・死蔵の検出のような「動かないもの・差分に現れないものを見る軸」は全体走査が本質なので絞りません — 節約と検出力のトレードオフを、一律でなく検査軸の性質ごとに分けます。
- **領域ガバナンスの置き場**: 領域の定義（名前と一行説明）は git 追跡のチーム共有の語彙として、owner の宣言（誰がいまその領域を触っているか）は git 非追跡のローカル宣言として分けて持ちます — 組織の情報を共有物に載せない既存の規律との両立です。記号がどの領域に属するかの正は各記号のタグだけで、宣言側は目録を持ちません（同じ情報を二重に持たなければ、食い違いは構造的に起きません）。
- **書き込み側のつなぎ込み**: 新しい記号を起こすとき、その領域を案件の文脈から導出して一問で確認し、書き込む領域に他のセッションの owner 宣言があれば「別のセッションが宣言中です」と一言添えます。これは書き込みを止める仕組みではありません — 割当宣言と同じ「気づかせるだけで、止めない・奪わない」宣言型で、並行運用の判断は人に残します。
- **always 登録の関門**: 新しい記号を全領域横断（always）で登録する前に、「本当に全領域に効きますか」を一問だけ確認します。中央に置く横断規律が惰性で肥大すると領域スコープの節約が漸減するためで、利用者が always を選べばそれに従います（一問より重い摩擦を課しません）。

利用者が一貫して求めてきた「DB のようにトランザクションで管理したい」への対応も、この延長にあります。実 DB は導入せず（依存ゼロ・grep と git diff の可読性を保つ）、**レコード＝1単位1ファイルの正規化収納（同時書き込みの分離）／トランザクション＝git のコミット（複数ファイルにまたがる変更の原子性と履歴）／ビュー＝派生 index の再生成／絞り込み＝領域タグの grep** という対応で同じ性質を満たします。compass はこの正規化を適用済みで、intent-tree の案件記録（機能追記のまとまり）への適用が本対応の最終ピースとして計画されています。

> **正直な注記（実装の範囲と未検証の主張）**: この設計の一次対象は実在する要求 — 一人の開発者が並行 AI セッションを回す運用と、実測800件超の記号の走査コスト — であり、人間のチームでの所有権運用は「宣言のスキーマと read-only の読み手契約」という置き場までに留めています（まだ観測されていない要求のための作り込みをしない）。また「領域スコープで維持ループの実行頻度が実際に上がるか」「並行セッションの書き込み衝突が実運用でゼロになるか」は**やれば測れる経験的仮説**であって、測った結果ではありません — 特に後者は、intent-tree の案件記録の正規化（計画済み・未実施）が完了して初めて閉じます。
