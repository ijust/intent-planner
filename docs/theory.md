...
**draft-content-dropped** は、export 下書き（`.intent/cc-sdd/<スラッグ>/`）と、それを渡して下流の spec ツールが生成した成果物（`.kiro/specs/<feature>/`）を読み比べ、下書きに載せた中身が下流で落ちていないかを名指しする検査です。上の `requirement-oracle-check` が「下書きに載せる**前**の基準が誤実装を弾けるか」を見るのに対し、こちらは「下書きに載せた**後**、下流の生成物にそれが残ったか」を見ます — 同じ lossy projection を、変換の前後という別の面で拾う姉妹の軸です。見るのは2つ、下書きが引いた意図の参照（守る約束・判断基準・親の意図）が下流の requirements / design / tasks に残っているか、そして受入基準の材料（`## Acceptance Material`）由来の判定条件が下流の受入基準に現れているか。突合の相手は、export 記録の分割ファイルに残る feature 名の追記行を**実線の手がかり**として同定し、実線が無ければ packet 名の包含で補助照合し、どちらでも同定できないときは「突合不能」と告げて**推測では対応づけません**（誤検知を出すくらいなら黙る側に倒す）。生存率の数値化・閾値判定は持たず（markdown のみ・DB を持たない核）、どの参照がどのファイルで落ちたかを名指しするところまでです。下流に spec が無い案件（直接実装する案件）では軸ごと沈黙し、「落ちなし」と誤標識しません。同じく read-only・候補提示・誤検知前提で、export も実装も止めません。この軸が要るのは、写像（下書きが受入基準の材料を運ぶ）と案内（フェーズごとにヒントを手渡す）で注入の確率を上げても、伝播は保証ではなく確率のままだからです — 構造で確率を上げ、残る減衰は検査で見張る、という役割分担になっています。
...
- **トリガ＝Revisit when 発火の拾い上げ** — Decision Rules の `Revisit when`（[有効期限つき決定](#decision-rules--軽量-adr-と設計根拠)）は「成立したら知らせる」ところまでを仕組みにする欄です。`/intent-improve` の coherence 軸が、実装の現実と deltas（実装から得た学びの記録）から条件の成立を読み取り、見直しを再提案します。
  かつてはこれに加えて、利用者が節目イベント（「本番構成を確定」等）を宣言的に記入する台帳 `.intent/milestones.md` を置き、その各 event を全 Decision Rule の `Revisit when` と文字列照合する経路を併走させていました。**これは畳みました。** 実測すると設計の前提が二重に成立していなかったためです — (1) 本リポジトリの `Revisit when` 143 個を全数調査したところ、「本番構成の確定」のようなリポジトリの外で起きる出来事を条件にした Rule は実質ゼロで、ほぼ全てが「〜と観測されたとき」「誤検知が多いと分かったとき」というリポジトリの内側と対話から拾える条件でした（＝照合の発火先が構造的に存在しない）。(2) 台帳への人間の自発記入もゼロのまま推移しました。**発火先の無い照合と、読み手の無い台帳は畳む** — 「気づかせる仕組みを横に足す」より、既に材料と行き先が一致している経路（deltas 起点の検出）を残すほうが強い、という判断です。節目そのものは、学び＝deltas・完了＝packet の archive・コミットの実線＝Intent trailer・外向きの語り＝release note が既に受けています。
