...
進捗には単一の万能指標がありません。要求の充足、実装の完了、証拠の有無、依存の解消は、それぞれ別のものを測っています。intent-planner の俯瞰（`/intent-overview`）は進捗を1つの総合％に圧縮せず、**意図の安定度 / 実現の完了度 / 証拠の確定度**という性質の異なる軸に分けて提示します。これにより「実装は進んでいるが証拠が未確定」「証拠は揃ったが上位意図とのギャップがある」といった、潰すと見えなくなる状態を表現できます。俯瞰にはもう一つ、対象範囲を指定したときだけ生成する**意図の空白地帯マップ**があります — 指定範囲のコード領域を「それを統べる packet が在るか・Invariant の影響パスに載っているか・コミットが意図に紐づいているか」の3面で突合し、どれにも統べられていない空白を根拠付きで列挙します（判定・スコア化はせず観測の列挙まで。空白が正当な領域もあるため「空白＝悪」とは断定しない）。繋がっている所の実線化だけでは見えない「そもそも意図が無い所」を地図にする一手で、逆読み（コード→意図）をどこへ向けるかの優先判断の材料になります。さらに、求めたときだけ生成する横断の派生ビューが2つあります — **判断待ちインボックス**は、散在する未回答の Open Questions・承認待ちの delta・追跡された警告を「いま人の判断を待っている問い」として出所リンク付きに一覧化し、溜まった判断を一望で捌けるようにします。**ロードマップ射影**は、active packet の順序（`depends_on` の前後関係）・状態・ブロッカー連鎖・保留の区分を**日付なし**で束ね、前向きの作業順序を1枚に映します。同じ「求めたときだけ」の系譜に、意図の全体像と作業順序を GitHub / VSCode がそのまま描画する **Mermaid 図射影**（掴むためのマインドマップ系の図と、`depends_on` の閉塞を映すロードマップ図。図にも日付・ガント・進捗％を持ち込まない）と、`.intent/assignments/` の割当宣言から「どの packet を誰が実装中か・同じ packet への二重着手はどこか」を read-only で映す **割当ビュー**（名指しの warn まで・止めない・機械閾値で自動解放しない）が加わります。いずれも「PM ツール化しない」床（日付・ガント・ベロシティ・進捗％を持たない）を守り、正本を書き換えず生成時点のスナップショットとして機能する read-only な派生です（この床が、状態の外部化が保証しない鮮度・唯一性を、materialized view と single source of truth の分離で扱う後述の姿勢の実装面です）。
