...
|---|---|---|
| spec → Intent | 仕様書が書かない why / invariant / anti-direction（問題空間のもの） | 「この仕様書は何が壊れてはいけないかを言っていない」を表出する（※下記注記） |
...
> **正直な注記（現状の実装範囲）**: 上表のうち Intent → code 側（drift 検出）は drift-watch / improve として実装済みです。spec → Intent 側 — 既存の外部仕様書（PRD・設計仕様・issue など）を入力に取り、落ちた意図をギャップとして表出する辺 — は**現時点では将来の拡張候補**であり、現在の `/intent-discover` は外部仕様書を読むのではなく対話で問題空間を集める形です。落差を測る物差し（完全性スキーマ・固定カテゴリ枠・Requirements Smells・PBR）は既にフローに組み込まれているため、この辺は「対話で集める」を「文書から読む」に差し替える拡張として位置づけられます。
...
横の網羅性は「漏れを拾う仕組み」を持つだけでは足りず、**いま何が完了し・何が反映待ちで・何が未着手かを人間が一目で掴める射影**を伴って初めて運用に乗ります。`/intent-overview` と `/intent-status` の**工程レール**は、全 packet を `state` × export 履歴 × delta の突合から5信号（反映済 / 今ここ / 未着手 / 反映漏れ / 統合済）に畳んで縦に並べた、横の網羅性の人間可読な射影です。とりわけ「実装は完了したのに意図へ書き戻されていない」取り残し（writeback 漏れ）を、別の警告ではなく**レール上のズレ**として浮かせます。これは Gotel & Finkelstein の pre-RS 切断（実装→意図の逆方向の切断点）を運用面で可視化する装置であり、レール自体は状態を算出・推論しない read-only な射影に徹します（正本は packet・export-log・deltas のまま）。
