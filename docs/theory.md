...
intent-planner との接合は**疎**に保ちます — 台帳の共有（同じ `.intent/glossary.md` を正本として読む・スキーマは「追加のみ・旧形式は常に読める」をバージョン間の契約とする）、存在検知による検査の接続（目印があれば `/intent-validate` が term-drift の検出ルールを**読んで**実行する。外部プロセスは起動せず、読み取り専用という検査層の性質を壊しません）、そして導入は opt-in（入れなければ挙動は1ビットも変わりません）。検出アルゴリズムの正本を**片方に一本化**するのは、同じ検出が2つの実装に分かれると育ち方が割れるからです。
