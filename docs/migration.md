...
### 正規化された compass への opt-in 移行

旧単一ファイル `.intent/intent-compass.md` から、`.intent/compass/`（1記号1ファイル）へ移す場合は、利用者の判断で次を一巡します。installer はこの移送を自動実行しません。

1. git 管理下なら先に作業ツリーをコミットします。git 管理外・部分追跡なら `.intent/` の複製を別場所へ取り、復元用に保持します（DR132）。
2. 旧本文から対象記号を取り出して配置します（例: `sed -n '/^# INV1/,/^# /p' .intent/intent-compass.md > .intent/compass/INV1.md`）。frontmatter の `id`・`area`・`status` と `## Law` を補い、`index.md` に `- INV1` の行を追加します。既存の単一ファイルは削除しません。
3. 利用する skill を一度実行し、`index.md` → 対象記号の `## Law` の順で読まれることを確認します。記号が分割収納に無い場合は旧単一ファイルが読まれることを確認します。
4. 途中で止める場合はそのまま戻して構いません。git 管理下は revert、非管理は複製から復元します。

この手順は dogfood fixture で実走確認済みです。旧単一ファイルだけの環境は恒久的にサポートされ、移行しなくても動作は変わりません。

### 正規化された intent-tree（案件記録）への opt-in 移行

`.intent/intent-tree.md` の本体末尾に積まれた**案件記録**（`## 機能追記:` / `## 機能撤去:` / `## 履歴:` / `## 再起案:`）を、`.intent/tree/`（1案件1ファイル）へ移す場合は、利用者の判断で次を一巡します。骨格（`## L0`〜`## L4`・製品の検証仮説・画面ラフ参照・`## Open Questions`）は本体に残します。installer はこの移送を自動実行しません（compass と同じ opt-in・INV3）。

1. git 管理下なら先に作業ツリーをコミットします。git 管理外・部分追跡なら `.intent/` の複製を別場所へ取り、復元用に保持します（DR132）。
2. 本体の各案件記録（`## 機能追記: <feature>（…）` 見出しから次の案件見出しの直前まで）を1案件ずつ取り出し、`.intent/tree/<feature>.md` として配置します（`<feature>` は見出しの `機能追記:` に続く名前）。frontmatter（`feature` / `status: active` / `kind`）を先頭に補い、本文（見出し行を含む）は**要約せず byte のまま**移します（move であって edit でない・INV80）。`index.md` に `- <feature> [<kind>] active — <要旨1行>` の行を追加します。本体側の当該案件記録は移送後に本体から取り除きます（骨格は残す）。
3. 見出しと中身がズレた節（別案件の記述が同じ節に同居している残骸）があれば、機械的な見出し分割で汚染ファイルを作らず、中身の帰属を目視で確かめてから分けます。
4. 移送前後で「本体末尾の案件記録の全文」と「`.intent/tree/` 配下の全ファイルの本文の連結」が byte 一致することを確認します（移動のみ・削除や言い換えを混ぜていない証跡）。`.intent/tree/` の全ファイルが git 追跡対象であること（`git check-ignore .intent/tree/<任意>` が何も返さないこと）も確認します（compass 型＝全追跡・gitignore に入れない）。
5. 利用する skill を一度実行し、案件記録が `.intent/tree/index.md` → 該当 `<feature>.md` の順で読まれること、`.intent/tree/` が無い環境では本体末尾が読まれることを確認します（恒久フォールバック・DR133）。
6. 途中で止める場合はそのまま戻して構いません。git 管理下は revert、非管理は複製から復元します。

この手順は dogfood の実移行（30案件→32ファイル・見出しズレの1節は3分割）で実走確認済みです。旧形式（本体末尾追記）だけの環境は恒久的にサポートされ、移行しなくても全 skill の動作は変わりません（DR133）。
...
- 分割収納の規約: `.intent/tree/README.md`（1案件1ファイル・全 git 追跡・読み手契約）
