...
const CONSTRAINT_SELECTION_GUIDES = {
  ja: read("docs", "guide.md"),
  en: read("docs", "guide.en.md"),
};

function constraintSelectionGuideSection(body, lang) {
  const heading = lang === "ja"
    ? "### export後に制約選別を確認する"
    : "### Check constraint selection after export";
  const start = body.indexOf(heading);
  assert.notEqual(start, -1, `${lang}: constraint-selection operation heading exists`);
  const next = body.indexOf("\n### ", start + heading.length);
  const nextMajor = body.indexOf("\n## ", start + heading.length);
  const ends = [next, nextMajor].filter((index) => index !== -1);
  return body.slice(start, ends.length === 0 ? body.length : Math.min(...ends));
}

function validateConstraintSelectionGuide(body, lang) {
  const section = constraintSelectionGuideSection(body, lang);
  ...
  for (const marker of [
    ...
    assert.ok(section.includes(marker), `${lang}: guide explains ${marker}`);
  }

  assert.match(
    section,
    lang === "ja"
      ? /確認候補[^]*(?:不足情報|missing)[^]*(?:人が確認|確認する)[^]*(?:正本|canonical)[^]*(?:再export|exportを再実行)/
      : /confirmation candidate[^]*(?:missing information|missing)[^]*(?:human|confirm)[^]*canonical[^]*(?:re-export|run the export again)/i,
    `${lang}: guide explains how to resolve confirmation candidates`,
  );
  assert.match(
    section,
    lang === "ja"
      ? /正本[^]*(?:更新|修正)[^]*(?:再export|exportを再実行)/
      : /(?:update|correct)[^]*canonical[^]*(?:re-export|run the same export again)/i,
    `${lang}: guide reruns export from updated canonical material`,
  );
  assert.match(
    section,
    lang === "ja"
      ? /再export[^]*(?:下書き)[^]*選別記録[^]*(?:全置換|置き換)/
      : /re-export[^]*replac[^]*draft[^]*selection record/i,
    `${lang}: guide replaces the draft and selection record together`,
  );
  assert.match(
    section,
    lang === "ja"
      ? /legacy-not-applied[^]*(?:新しい選別|共通選別)[^]*(?:適用していない|未適用)/
      : /legacy-not-applied[^]*(?:new|common) selection[^]*not applied/i,
    `${lang}: guide distinguishes legacy-not-applied from an applied selection`,
  );
  assert.match(
    section,
    lang === "ja"
      ? /Legacy Output[^]*(?:従来|既存)/
      : /(?:Legacy Output[^]*(?:legacy|existing)|(?:legacy|existing)[^]*Legacy Output)/i,
    `${lang}: guide points legacy-not-applied runs to the legacy output`,
  );
  assert.match(
    section,
    lang === "ja"
      ? /(?:index-missing|split-store-missing|symbol-missing)[^]*selection_status: applied/
      : /(?:index-missing|split-store-missing|symbol-missing)[^]*selection_status: applied/i,
    `${lang}: guide distinguishes degraded applied runs from legacy-not-applied`,
  );
  assert.doesNotMatch(
    section,
    lang === "ja"
      ? /theory(?:\.md)?[^。\n]*(?:必須|必要|読んでから)/i
      : /(?:must|required to|need to) read[^\n]*theory/i,
    `${lang}: theory reading is not an operation prerequisite`,
  );
...
test("意味オラクルが採用・確認の排他性、下流ID一致、読み取り元、内部記録と履歴分離を検査する", () => {
  ...
  assert.equal(subject.record.excluded_count, 5);
...
test("意味オラクルは状態・関連性・写像可否の取り違えmutationを個別に検出する", () => {
  const mutations = [
    ...
    ["state.archived-selected", { ignoreArchive: true }],
    ["state.inactive-confirm", { inactiveToConfirm: true }],
    ["prerequisite.unsatisfied-selected", { ignoreUnsatisfiedPrerequisite: true }],
  ...
  for (const [expectedViolation, mutation] of [
    ...
    assert.deepEqual(
      collectConstraintSelectionViolations(createConstraintSelectionSubject({ mutation })),
      [expectedViolation],
      `${expectedViolation}だけを報告する`,
    );
  }
});
...
test("同じ共通fixtureを使う3出口は採用集合・順序・下流6項目が一致し、配置だけが異なる", () => {
  ...
  for (const target of subject.targets) {
    for (const excludedId of [
      ...
      assert.ok(!target.selected_ids.includes(excludedId), `${target.target}: ${excludedId}`);
      assert.ok(
        !target.downstream.some(({ Identifier }) => Identifier === excludedId),
        `${target.target} downstream: ${excludedId}`,
      );
    }
  }
