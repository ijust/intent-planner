...
//
// 範囲: 対象スキル intent-from-code (4系統の SKILL.md + rules) の「規律本文」プロパティと、
//   日英のREADME・guide・theoryの公開案内をREAD-ONLYで検査する。本ファイルは配布物を変更しない。
...
function readPublicDoc(relativePath) {
  return fs.readFileSync(path.join(REPO_ROOT, relativePath), "utf8");
}

function publicDocContractErrors(kind, content) {
  const errors = [];
  if (!/(任意の補助|optional aid)/i.test(content)) errors.push(`${kind}.optional`);
  if (
    !/(依存(?:先)?にはせず|依存では(?:なく|ありません)|責務でも依存でもありません|does not become an intent-planner dependency|without becoming an intent-planner dependency|neither an intent-planner responsibility nor an intent-planner dependency)/i.test(
      content,
    )
  ) {
    errors.push(`${kind}.no-dependency`);
  }
  if (!/(直接読解へ戻|falls? back to direct code reading)/i.test(content)) {
    errors.push(`${kind}.direct-reading-fallback`);
  }
  if (
    !/(解析結果だけで意図(?:を)?(?:判断|確定)し(?:ない|ません)|analysis output alone does not determine or confirm intent|never determines or confirms intent from analysis output alone)/i.test(
      content,
    )
  ) {
    errors.push(`${kind}.separate-analysis-from-intent`);
  }
  return errors;
}

function theoryBoundaryErrors(content) {
  const errors = publicDocContractErrors("theory", content);
  if (!/(導入・初期化・更新・索引同期・状態管理|installation, initialization, updates, index synchronization, and state management)/i.test(content)) {
    errors.push("theory.lifecycle-reason");
  }
  if (!/(読む箇所を絞|narrows? the locations to read)/i.test(content)) {
    errors.push("theory.narrowing-benefit");
  }
  if (
    !/(動的な呼び出し、生成コード、条件付き import|dynamic calls, generated code, conditional imports)/i.test(
      content,
    )
  ) {
    errors.push("theory.analysis-limit");
  }
  return errors;
}
...
function optionalAnalysisContractErrors({ scope, extraction, recap, safety }) {
  ...
  const disclosesAnalysisWhenUnused =
    /(?:^|\n)\s*(?:always state|state even when)[^\n]*analysis[^\n]*(?:was used|assisted structural observation)[^\n]*(?:even when|although)[^\n]*(?:was not used|unused)/im.test(
      recap,
    ) ||
    /(?:^|\n)\s*任意解析を使わなかった場合も[^\n]*(?:使った|構造把握の補助)[^\n]*記(?:す|載せる)/m.test(
      recap,
    );
  const requiresMandatoryAnalysisDetails =
    /(?:^|\n)\s*(?:the recap|recap)[^\n]*(?:must|required to)[^\n]*tool name[^\n]*quer(?:y|ies)[^\n]*raw analysis output[^\n]*resource values?/im.test(
      recap,
    ) ||
    /(?:^|\n)\s*recap[^\n]*(?:ツール名|解析ツール名)[^\n]*問い合わせ[^\n]*生の解析出力[^\n]*資源値[^\n]*必須/m.test(
      recap,
    );
  const autoPromotesFromRecap =
    /(?:^|\n)\s*promotion (?:happens|runs|is performed) automatically/im.test(recap) ||
    /(?:^|\n)\s*(?:recapは)?[^\n]*候補を自動(?:で|的に)?(?:昇格|正本へ反映)/m.test(recap);
  const omitsExistingRecapReadout =
    /(?:^|\n)\s*(?:omit|do not report)[^\n]*(?:scope read|silence|unreadable areas)/im.test(
      recap,
    ) ||
    /(?:^|\n)\s*(?:読んだ範囲|沈黙|読めなかった箇所)[^\n]*(?:省略する|報告しない)/m.test(
      recap,
  ...
  requireContract(
    /(使った場合に限り|only when .* was used)/i.test(recap) &&
      /(構造把握の補助|assisted structural observation)/i.test(recap) &&
      !disclosesAnalysisWhenUnused,
    "recap.analysis-use-disclosure",
  );
  requireContract(
    /(ツール名|tool names?)/i.test(recap) &&
      /(問い合わせ|quer(?:y|ies))/i.test(recap) &&
      /(生の解析出力|raw analysis output)/i.test(recap) &&
      /(資源値|resource values?)/i.test(recap) &&
      /(必須形式にしない|do not make .* mandatory recap schema)/i.test(recap) &&
      !requiresMandatoryAnalysisDetails,
    "recap.no-mandatory-analysis-details",
  );
  requireContract(
    /(読んだ範囲|scope read)/i.test(recap) &&
      /(沈黙|fell silent)/i.test(recap) &&
      /(読めなかった箇所|could not be read)/i.test(recap) &&
      !omitsExistingRecapReadout,
    "recap.readout-continuity",
  );
  requireContract(
...
`,
  recap: `
Only when optional local analysis was used, state that it assisted structural observation.
Do not make tool names, queries, raw analysis output, or resource values a mandatory recap schema.
Retain the scope read, the categories that fell silent, and the areas that could not be read.
...
const APPENDED_CONTRADICTIONS_EN = [
  ...
  },
  {
    ...
    field: "recap",
    appended:
      "Always state that analysis assisted structural observation even when it was not used.",
  },
  {
    id: "recap.no-mandatory-analysis-details",
    field: "recap",
    appended:
      "The recap must include the tool name, queries, raw analysis output, and resource values.",
  },
  {
    id: "recap.readout-continuity",
    field: "recap",
    appended: "Omit the scope read, silence, and unreadable areas from the recap.",
  },
  {
...
const APPENDED_CONTRADICTIONS_JA = [
  ...
  },
  {
    id: "recap.analysis-use-disclosure",
    field: "recap",
    appended: "任意解析を使わなかった場合も、構造把握の補助に使ったと記す。",
  },
  {
    id: "recap.no-mandatory-analysis-details",
    field: "recap",
    appended: "recap はツール名、問い合わせ、生の解析出力、資源値を必須にする。",
  },
  {
    id: "recap.readout-continuity",
    field: "recap",
    appended: "読んだ範囲、沈黙、読めなかった箇所は recap から省略する。",
  },
  {
    id: "recap.manual-promotion",
    field: "recap",
...
for (const lang of LANGS) {
  for (const agent of AGENTS) {
    test(`群5c実配布面: ${lang}/${agent} recap-and-promotion は利用報告契約を満たす (1.4, 2.3, 3.2)`, () => {
      const recap = readRule(lang, agent, "recap-and-promotion");
      const recapErrors = optionalAnalysisContractErrors({
        ...COMPLETE_OPTIONAL_ANALYSIS_CONTRACT,
        recap,
      ...
      assert.deepEqual(
        recapErrors,
        [],
        `${lang}/${agent}: 利用報告契約の不足: ${recapErrors.join(", ")}`,
      );

      const disclosure =
        lang === "ja"
          ? "任意のローカル解析・索引を使った場合に限り、構造把握の補助に使ったことも記す。"
          : "Only when an optional local analysis or index was used, also state that it assisted structural observation.";
      assert.ok(recap.includes(disclosure), `${lang}/${agent}: 利用表示の義務が本文に存在する`);
      const omitted = recap.replace(disclosure, "");
      assert.notEqual(omitted, recap, `${lang}/${agent}: 利用表示の欠落変異が入力を変更した`);
      const omissionErrors = optionalAnalysisContractErrors({
        ...COMPLETE_OPTIONAL_ANALYSIS_CONTRACT,
        recap: omitted,
      }).filter((id) => id.startsWith("recap."));
      assert.deepEqual(
        omissionErrors,
        ["recap.analysis-use-disclosure"],
        `${lang}/${agent}: 利用表示の欠落を対応する診断IDだけで拒否する`,
      );
      ...
      for (const { id, appended } of contradictions) {
        const mutated = `${recap}\n${appended}`;
        assert.notEqual(mutated, recap, `${lang}/${agent}/${id}: 禁止契約が本文を変更した`);
        const mutatedErrors = optionalAnalysisContractErrors({
          ...COMPLETE_OPTIONAL_ANALYSIS_CONTRACT,
          recap: mutated,
        }).filter((errorId) => errorId.startsWith("recap."));
        assert.deepEqual(
          mutatedErrors,
          [id],
          `${lang}/${agent}/${id}: 正しい本文と併存する禁止契約を単独診断する`,
  ...
  const mutations = [
    ...
    },
    {
      ...
      from:
        "Do not make tool names, queries, raw analysis output, or resource values a mandatory recap schema.",
      to:
        "The recap must include tool names, queries, raw analysis output, and resource values.",
    },
    {
      id: "recap.readout-continuity",
      field: "recap",
      from:
        "Retain the scope read, the categories that fell silent, and the areas that could not be read.",
      to: "Omit the scope read, silence, and unreadable areas from the recap.",
    },
    {
      id: "recap.manual-promotion",
      field: "recap",
...
// ---- 群5d: feature-growth の影響分析も同じ任意解析境界を持つ (1.1–1.3, 3.1, 3.3–3.4, 4.5–4.6, 5.4) ----

const IMPACT_RULE = "algo-impact-analysis";

function readImpactRule(lang, agent) {
  const p = path.join(
    TEMPLATES,
    lang,
    agent,
  ...
  assert.ok(fs.existsSync(p), `影響分析sourceが実在する: ${lang}/${agent} (${p})`);
  return fs.readFileSync(p, "utf8");
}
...
function impactAnalysisContractErrors(content) {
  ...
  const requiresAnalysisWhenAvailable =
    /(?:^|\n)\s*analysis is mandatory whenever it is available/im.test(content) ||
    /(?:^|\n)\s*解析が利用可能なら必須(?:とする|化する)/m.test(content);
  const determinesImpactFromAnalysisAlone =
    /(?:^|\n)\s*determine impact from analysis output alone/im.test(content) ||
    /(?:^|\n)\s*解析結果だけで影響を確定する/m.test(content);
  const forcesAnalysisForDirectReadingCase =
    /(?:^|\n)\s*when the scope is small[^\n]*require analysis/im.test(content) ||
    /(?:^|\n)\s*対象範囲が小さくても解析を必須化する/m.test(content);
  const stopsWithoutAnalysis =
    /(?:^|\n)\s*(?:if|when)[^\n]*(?:analysis|index)[^\n]*(?:unavailable|not callable|uninitialized|stale|insufficient)[^\n]*(?:stop|abort|fail)/im.test(
      content,
    ) ||
    /(?:^|\n)\s*(?:解析|索引)[^\n]*(?:利用不能|呼び出せない|未初期化|古い|不足)[^\n]*(?:停止する|中断する|失敗させる)/m.test(
      content,
    );
  const expandsScopeWithoutConfirmation =
    /(?:^|\n)\s*expand the (?:confirmed )?scope without (?:asking|confirmation)/im.test(
      content,
    ) ||
    /(?:^|\n)\s*(?:利用者へ)?確認せず(?:確定した)?対象範囲を広げる/m.test(content);
  const ownsAnalysisLifecycle =
    /(?:^|\n)\s*(?:intent planner|this (?:skill|rule))\s+(?:installs?|initializes?|requires?|updates?|synchronizes?|manages?)[^\n]*(?:analysis|index|state)/im.test(
      content,
    ) ||
    /(?:^|\n)\s*(?:Intent Planner|本(?:スキル|規則))[^\n]*(?:解析|索引)[^\n]*(?:導入する|初期化する|必須化する|更新する|同期する|状態管理する)/m.test(
      content,
    );
  const sendsExternally =
    /(?:^|\n)\s*send (?:the )?(?:code|analysis (?:output|results?))[^\n]*external (?:api|service)/im.test(
      content,
    ) ||
    /(?:^|\n)\s*(?:コード|解析結果)[^\n]*外部(?: API・)?サービスへ送信する/m.test(content);
  const bindsSpecificTool =
    /(?:^|\n)\s*(?:the )?common contract (?:requires|is named)[^\n]*CodeGraph/im.test(
      content,
    ) ||
    /(?:^|\n)\s*共通契約(?:では|は)[^\n]*CodeGraph[^\n]*(?:必須|契約名)/im.test(content);

  requireContract(
    /(already available|(?:すでに|既に)利用可能)/i.test(content) &&
      /(local read-only|ローカル read-only)/i.test(content) &&
      /(provided by the host|ホストが提供)/i.test(content) &&
      /(callable under the current skill execution permissions|現在のスキル実行権限から呼び出せる)/i.test(
        content,
      ) &&
      /(may optionally assist|任意利用してよい)/i.test(content) &&
      /(symbols?|シンボル)/i.test(content) &&
      /(references?|参照)/i.test(content) &&
      /(calls?|call paths?|呼び出し)/i.test(content) &&
      /(dependency direction|依存方向)/i.test(content) &&
      /(impact candidates?|影響候補)/i.test(content) &&
      /(narrow the locations to read directly|直接読む箇所を絞る)/i.test(content) &&
      /(current code|現在のコード)/i.test(content) &&
      /(never determine impact from its output alone|解析結果だけで影響を確定しない)/i.test(
        content,
      ) &&
      !requiresAnalysisWhenAvailable &&
      !determinesImpactFromAnalysisAlone,
    "impact.optional-structural-hints",
  );
  requireContract(
    /(scope is small|対象範囲が小さい)/i.test(content) &&
      /(locations? to read (?:are|is) known|読む箇所が既知)/i.test(content) &&
      /(more expensive than direct reading|直接読解より重い)/i.test(content) &&
      /(choose direct (?:code )?reading|直接読解を選ぶ)/i.test(content) &&
      !forcesAnalysisForDirectReadingCase,
    "impact.direct-reading-choice",
  );
  requireContract(
    /(unavailable|利用不能)/i.test(content) &&
      /(not callable under those permissions|権限から呼び出せない)/i.test(content) &&
      /(uninitialized|未初期化)/i.test(content) &&
      /(stale|古い)/i.test(content) &&
      /(insufficient|不足)/i.test(content) &&
      /(fall back to reading the current code directly|現在のコードの直接読解へ戻る)/i.test(
        content,
      ) &&
      !stopsWithoutAnalysis,
    "impact.direct-reading-fallback",
  );
  requireContract(
    /(do not mix .*out-of-scope.*into the impact analysis|範囲外の情報を影響分析へ混ぜない)/i.test(
      content,
    ) &&
      /(ask the user before expanding the scope|範囲を広げる前に利用者へ確認)/i.test(content) &&
      !expandsScopeWithoutConfirmation,
    "impact.confirmed-boundary",
  );
  requireContract(
    /(does not install, initialize, require, update, synchronize indexes, or manage state|導入・初期化・必須化・更新・索引同期・状態管理しない)/i.test(
      content,
    ) && !ownsAnalysisLifecycle,
    "impact.no-analysis-lifecycle",
  );
  requireContract(
    /(do not send code or analysis (?:output|results?) to an external API or service|コードや解析結果を外部 API・外部サービスへ送信しない)/i.test(
      content,
    ) && !sendsExternally,
    "impact.no-external-send",
  );
  requireContract(
    /(do not make a specific tool name, API, command, output format, or schema part of the common contract|特定の解析ツール名・API・コマンド・出力形式・スキーマを共通契約にしない)/i.test(
      content,
    ) &&
      !bindsSpecificTool,
    "impact.tool-neutral",
...
const IMPACT_OMISSIONS = Object.freeze({
  ja: [
    ["impact.optional-structural-hints", "ホストが提供し、現在のスキル実行権限から呼び出せる"],
    ["impact.optional-structural-hints", "解析結果だけで影響を確定しない。"],
    ["impact.direct-reading-choice", "対象範囲が小さい、読む箇所が既知、または解析結果の取得と読解が直接読解より重い場合は、解析を使わず直接読解を選ぶ。"],
    ["impact.direct-reading-fallback", "解析が利用不能、現在のスキル実行権限から呼び出せない、未初期化、古い、または結果が不足している場合は、停止せず現在のコードの直接読解へ戻る。"],
    ["impact.confirmed-boundary", "解析結果が確定した対象範囲の外を示しても、範囲外の情報を影響分析へ混ぜない。範囲を広げる前に利用者へ確認する。"],
    ["impact.no-analysis-lifecycle", "Intent Planner は解析基盤を導入・初期化・必須化・更新・索引同期・状態管理しない。"],
    ["impact.no-external-send", "コードや解析結果を外部 API・外部サービスへ送信しない。"],
    ["impact.tool-neutral", "特定の解析ツール名・API・コマンド・出力形式・スキーマを共通契約にしない。"],
  ],
  en: [
    ["impact.optional-structural-hints", "provided by the host and callable under the current skill execution permissions"],
    ["impact.optional-structural-hints", "Never determine impact from its output alone."],
    ["impact.direct-reading-choice", "When the scope is small, the locations to read are known, or retrieving and reading analysis output is more expensive than direct reading, do not use analysis; choose direct code reading."],
    ["impact.direct-reading-fallback", "When analysis is unavailable, not callable under those permissions, uninitialized, stale, or insufficient, do not stop; fall back to reading the current code directly."],
    ["impact.confirmed-boundary", "When analysis points outside the confirmed scope, do not mix that out-of-scope information into the impact analysis. Ask the user before expanding the scope."],
    ["impact.no-analysis-lifecycle", "Intent Planner does not install, initialize, require, update, synchronize indexes, or manage state for analysis."],
    ["impact.no-external-send", "Do not send code or analysis output to an external API or service."],
    ["impact.tool-neutral", "Do not make a specific tool name, API, command, output format, or schema part of the common contract."],
  ],
});

const IMPACT_APPENDED_CONTRADICTIONS = Object.freeze({
  ja: [
    ["impact.optional-structural-hints", "解析が利用可能なら必須とする。"],
    ["impact.optional-structural-hints", "解析結果だけで影響を確定する。"],
    ["impact.direct-reading-choice", "対象範囲が小さくても解析を必須化する。"],
    ["impact.direct-reading-fallback", "解析が利用不能または古い場合は処理を停止する。"],
    ["impact.confirmed-boundary", "利用者へ確認せず確定した対象範囲を広げる。"],
    ["impact.no-analysis-lifecycle", "Intent Planner は解析索引を初期化する。"],
    ["impact.no-external-send", "コードや解析結果を外部サービスへ送信する。"],
    ["impact.tool-neutral", "共通契約では CodeGraph を契約名として必須にする。"],
  ],
  en: [
    ["impact.optional-structural-hints", "Analysis is mandatory whenever it is available."],
    ["impact.optional-structural-hints", "Determine impact from analysis output alone."],
    ["impact.direct-reading-choice", "When the scope is small, require analysis."],
    ["impact.direct-reading-fallback", "When analysis is unavailable or stale, stop impact analysis."],
    ["impact.confirmed-boundary", "Expand the confirmed scope without asking."],
    ["impact.no-analysis-lifecycle", "Intent Planner initializes the analysis index."],
    ["impact.no-external-send", "Send code or analysis output to an external service."],
    ["impact.tool-neutral", "The common contract is named CodeGraph."],
  ],
});

for (const lang of LANGS) {
  for (const agent of AGENTS) {
    test(`群5d実配布面: ${lang}/${agent} 影響分析は任意解析境界を満たす (1.1–1.3, 3.1, 3.3–3.4, 4.5–4.6, 5.4)`, () => {
      const content = readImpactRule(lang, agent);
      assert.deepEqual(
        impactAnalysisContractErrors(content),
        [],
        `${lang}/${agent}: 影響分析契約の不足`,
      );

      for (const [id, phrase] of IMPACT_OMISSIONS[lang]) {
        assert.ok(content.includes(phrase), `${lang}/${agent}/${id}: 欠落対象の義務が本文に存在する`);
        const omitted = content.replace(phrase, "");
        assert.notEqual(omitted, content, `${lang}/${agent}/${id}: 欠落変異が入力を変更した`);
        assert.deepEqual(
          impactAnalysisContractErrors(omitted),
          [id],
          `${lang}/${agent}/${id}: 義務欠落を対応する診断IDだけで拒否する`,
        );
      }

      for (const [id, appended] of IMPACT_APPENDED_CONTRADICTIONS[lang]) {
        const mutated = `${content}\n${appended}`;
        assert.notEqual(mutated, content, `${lang}/${agent}/${id}: 禁止契約の追記が入力を変更した`);
        assert.ok(mutated.startsWith(content), `${lang}/${agent}/${id}: 正しい契約本文を残している`);
        assert.deepEqual(
          impactAnalysisContractErrors(mutated),
          [id],
          `${lang}/${agent}/${id}: 正しい文と併存する禁止契約を単独診断する`,
        );
      }
...
for (const lang of LANGS) {
  test(`群5dパリティ: ${lang} の影響分析sourceは claude/codex でbyte等価 (5.1)`, () => {
    assert.equal(
      readImpactRule(lang, "claude"),
      readImpactRule(lang, "codex"),
      `${lang}: 影響分析sourceの同一言語本文が一致する`,
    );
  });
}
...
// ---- 群11: 公開入口・guide・theory の日英同期 (5.1, 5.3–5.5, 7.4) ----
...
for (const lang of LANGS) {
  ...
  test(`群11: ${lang} のguideは任意解析・非依存・復帰・意図判断分離を案内する (5.1, 5.3–5.4)`, () => {
    const relativePath = PUBLIC_DOCS[lang].guide;
    const content = readPublicDoc(relativePath);
    assert.deepEqual(publicDocContractErrors("guide", content), [], `${relativePath}: guide契約`);

    const omitted = content.replace(
      lang === "ja" ? "利用できなければ直接読解へ戻ります。" : "If it cannot be used, the skill falls back to direct code reading.",
      "",
    );
    assert.notEqual(omitted, content, `${relativePath}: 直接読解復帰の欠落変異が入力を変更した`);
    assert.deepEqual(
      publicDocContractErrors("guide", omitted),
      ["guide.direct-reading-fallback"],
      `${relativePath}: 直接読解復帰の欠落だけを診断する`,
    );
  });

  test(`群11: ${lang} のtheoryは解析基盤を内蔵しない理由・利点・限界を示す (5.1, 5.3–5.4)`, () => {
    const relativePath = PUBLIC_DOCS[lang].theory;
    const content = readPublicDoc(relativePath);
    assert.deepEqual(theoryBoundaryErrors(content), [], `${relativePath}: theory境界`);

    const omitted = content.replace(
      lang === "ja"
        ? "動的な呼び出し、生成コード、条件付き import"
        : "dynamic calls, generated code, conditional imports",
      "",
    );
    assert.notEqual(omitted, content, `${relativePath}: 限界説明の欠落変異が入力を変更した`);
    assert.deepEqual(
      theoryBoundaryErrors(omitted),
      ["theory.analysis-limit"],
      `${relativePath}: 限界説明の欠落だけを診断する`,
    );
  });
}
