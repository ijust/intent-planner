...
const PERSPECTIVE_INTEGRATION_LINE = "deep の追加事項を同じ確認範囲へ加えた直後に、`rules/role-perspective-review.md` を正確に1回だけ読み、適用する。観点ラベル付きの必要論点を上で使った同じ確認範囲へ追加し、観点ごとの別の質問ループ、別の状態、別の永続形式は作らない。";
...
function perspectiveIntegrationErrors(text) {
  ...
  const firstListLine = "手順 6.55 の同じ確認範囲を使い、deep で追加される事項を未確認として加える。状態と保留の扱いは共通契約から再定義せず、新しい永続形式や台帳も作らない。";
  ...
  const checks = [
    ...
    [deep.includes(expectedAdjacency), "deep の追加事項を共通の確認範囲へ加えた直後に接続する"],
    [/観点ラベル付きの必要論点を上で使った同じ確認範囲へ追加/.test(deep), "観点ラベル付き論点を同じ確認範囲へ追加する"],
    [/観点ごとの別の質問ループ、別の状態、別の永続形式は作らない/.test(deep), "観点ごとの別ループ・状態・永続形式を作らない"],
    [/共通契約の表示上限/.test(deep) && /共通契約の終了条件/.test(deep), "質問束と終了条件を共通契約から参照する"],
...
test("Task 3.1: 接続位置、同じ一覧、一度だけの読み込み、別ループ禁止を壊す変異を拒否する", () => {
  ...
  const mutations = [
    ...
    ["観点ラベル付きの必要論点を上で使った同じ確認範囲へ追加し", "観点ラベル付きの必要論点を観点別の一覧へ追加し"],
...
const EN_PERSPECTIVE_INTEGRATION_LINE = "Immediately after adding deep concerns to the same question coverage, read and apply `rules/role-perspective-review.md` exactly once. Add the necessary perspective-labeled concerns to the same question coverage used above; do not create a separate question loop, state, or persistent format for each perspective.";
...
function englishPerspectiveIntegrationErrors(text) {
  ...
  const firstListLine = "Reuse the same coverage from step 6.55 and add deep-only concerns as unconfirmed. Do not redefine the shared states or deferral handling, and create no new persistent format or ledger.";
  ...
  const checks = [
    ...
    [deep.includes(expectedAdjacency), "connect immediately after deep additions enter shared coverage"],
    [/perspective-labeled concerns to the same question coverage used above/.test(deep), "add perspective-labeled concerns to the same question coverage"],
    ...
    [/shared contract's display limit/.test(deep) && /shared contract's completion condition/.test(deep), "reference the shared batch and completion contract"],
    ...
    [/Once a concern is answered, out of scope, or deferred with a non-impact reason, do not re-ask it in different words/.test(deep) && /source or basis cannot be shown, do not add it/.test(deep), "preserve no-reask and source-or-basis discipline"],
...
function integrationSemanticCoverage(text, language) {
  ...
  const patterns = language === "ja" ? {
    ...
    sameList: /観点ラベル付きの必要論点を上で使った同じ確認範囲へ追加/,
    noParallelMechanism: /観点ごとの別の質問ループ、別の状態、別の永続形式は作らない/,
    existingDeepContract: /共通契約の表示上限[^]*共通契約の終了条件[^]*利用者が終了を選んだら[^]*終端した論点は言い換えて再質問しない/,
  } : {
    ...
    sameList: /perspective-labeled concerns to the same question coverage used above/,
    noParallelMechanism: /do not create a separate question loop, state, or persistent format for each perspective/,
    existingDeepContract: /shared contract's display limit[^]*shared contract's completion condition[^]*If the user chooses to stop[^]*Once a concern is answered, out of scope, or deferred with a non-impact reason, do not re-ask it in different words/,
  };
...
test("Task 3.2: English connection mutations are applied and rejected", () => {
  ...
  const mutations = [
    ...
    ["perspective-labeled concerns to the same question coverage used above", "perspective-labeled concerns to a separate list"],
...
function runIntegratedFixture(input) {
  ...
  const contract = readDesignerQuestions(path.join(ROOT, "templates", "en", "claude", "skills", "CONTRACT.md"));
  const result = {};
  ...
  if (input.neededConcerns === 9) {
    const limit = Number(contract.match(/at most (\d+) questions per batch/i)?.[1] ?? 0);
    result.batches = limit > 0
      ? Array.from({ length: Math.ceil(input.neededConcerns / limit) }, (_, index) =>
        Math.min(limit, input.neededConcerns - (index * limit)))
      : [];
    result.usesSingleList = /perspective-labeled concerns to the same question coverage used above/.test(englishDeepSection(deep));
  }
