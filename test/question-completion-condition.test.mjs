...
test("公開説明と配布用ルート文書が可変の質問量と範囲付き完了を説明する", () => {
  ...
  for (const file of rootDocs) {
    const content = read(file);
    assert.match(content, /最大4問|at most 4 questions per batch/i, `${file}: 質問束の表示上限`);
    assert.match(content, /総質問数|total number of questions/i, `${file}: 総数は終了条件でない`);
    assert.match(content, /残る.*確認|remaining.*(?:decision|concern)/is, `${file}: 残件を表示`);
  }
});
