...
test("リポジトリ自身でも intent-plan を同じ内容で利用できる", () => {
  for (const [local, template] of [
    ...
    assert.deepEqual(
      fs.readFileSync(path.resolve(local, "SKILL.md")),
      fs.readFileSync(path.resolve(template, "SKILL.md")),
    );
    assert.deepEqual(
      fs.readFileSync(path.resolve(local, "generated/sources/intent-discover/instruction.md")),
      fs.readFileSync(path.resolve(template, "generated/sources/intent-discover/instruction.md")),
    );
  }
});
