...
for (const [lang, agent] of SURFACES) {
  ...
  test(`${relative}: 薄い一続きの進行役である`, () => {
    ...
    assert.match(content, /intent-to-spec\/instruction\.md/);
...
test("5出口すべての参照先が4公開面に存在する", () => {
  for (const [lang, agent] of SURFACES) {
    const root = path.resolve(`templates/${lang}/${agent}/skills/intent-plan/generated`);
    for (const relative of [
      "views/intent-export-cc-sdd/draft.md",
      "views/intent-export-openspec/draft.md",
      "views/intent-export-speckit/draft.md",
      "sources/intent-to-spec/instruction.md",
      "sources/intent-overview/rules/mermaid-tree.md",
      "sources/intent-packets/instruction.md",
    ]) assert.equal(fs.existsSync(path.join(root, relative)), true, `${lang}/${agent}: ${relative}`);
  }
});

test("リポジトリ自身でも intent-plan を同じ内容で利用できる", () => {
  for (const [local, template] of [
    [".claude/skills/intent-plan", "templates/ja/claude/skills/intent-plan"],
    [".agents/skills/intent-plan", "templates/ja/codex/skills/intent-plan"],
  ]) {
    const localRoot = path.resolve(local);
    const templateRoot = path.resolve(template);
    const files = relativeFiles(templateRoot);
    assert.deepEqual(relativeFiles(localRoot), files);
    for (const relative of files) {
      assert.deepEqual(
        fs.readFileSync(path.join(localRoot, relative)),
        fs.readFileSync(path.join(templateRoot, relative)),
        relative,
      );
    }
  }
