...
//     能動温度（行動促し）・pull 規律・SDD 3出口・案件固有制約のexport供給。ja ↔ en・agent 間のパリティ。
...
for (const doc of BODY_DOCS) {
  ...
  test(`3つのSDD出口と案件固有制約のexport供給を含む: ${doc.rel}`, () => {
    const body = readDoc(doc.rel);
    assert.ok(body !== null, `${doc.rel} が存在する`);
    for (const tool of ["cc-sdd", "OpenSpec", "Spec Kit"]) {
      assert.ok(body.includes(tool), `${doc.rel} は対応SDDツール ${tool} を含む`);
    }
    for (const skill of ["intent-export-cc-sdd", "intent-export-openspec", "intent-export-speckit"]) {
      assert.ok(body.includes(skill), `${doc.rel} は対応export ${skill} を含む`);
    }
    const carriesCaseConstraints = doc.lang === "ja"
      ? /案件固有の制約.+プロジェクト共通文書.+重複させない/s.test(body)
        && /選んだexport.+対象packet.+必要な制約.+下書き/s.test(body)
      : /case-specific constraints.+project-wide guidance/s.test(body)
        && /target packet.+necessary constraints.+chosen export/s.test(body);
    assert.ok(carriesCaseConstraints, `${doc.rel} は案件固有の制約を共通文書へ重複させず選んだexportから渡す`);
    const keepsDraftBoundary = doc.lang === "ja"
      ? /下書きまで.+本体の仕様作成.+レビュー.+SDDツール側/s.test(body)
      : /stops at the draft.+SDD tool owns the full specification.+human review/s.test(body);
    assert.ok(keepsDraftBoundary, `${doc.rel} はIntent Plannerの責任を下書きまでに保つ`);
    assert.doesNotMatch(body, /steering/i, `${doc.rel} の共通説明をKiro固有のsteering名へ戻さない`);
  });
}
...
test("dogfoodもSDD共通説明へ同期し、AGENTS.md上部のKiro固有ガイドは保持する", () => {
  const claudeDogfood = readDoc("CLAUDE_intent.md");
  const agentsDogfood = readDoc("AGENTS.md");
  assert.ok(claudeDogfood !== null && agentsDogfood !== null, "dogfood 2面が存在する");
  for (const body of [claudeDogfood, agentsDogfood]) {
    for (const marker of ["cc-sdd", "OpenSpec", "Spec Kit", "intent-export-speckit", "案件固有の制約はexportで渡す"]) {
      assert.ok(body.includes(marker), `dogfood は ${marker} を含む`);
    }
  }
  const quickstart = agentsDogfood.slice(agentsDogfood.indexOf("# intent-planner（Codex 向け quickstart）"));
  assert.doesNotMatch(quickstart, /steering/i, "Intent Planner quickstartはsteering固有表現へ戻さない");
  assert.match(agentsDogfood, /\.kiro\/steering\//, "上部のKiro固有ガイドはsteeringの実パスを保持する");
  assert.match(agentsDogfood, /\$kiro-steering-custom/, "上部のKiro固有ガイドはcustom steeringのコマンドを保持する");
});
