...
// ---- 2. ロック対象の総数が移行前と同じ（何を守るかは不変・持ち方だけ変えた） ----
// 移行前の内訳: byte=22 / frontmatter=10 / installer=4 / skillBody=18 = 54。
test("2: ロック対象の件数が移行前と一致する（対象の増減なし＝検査を緩めていない）", () => {
  const m = readManifest();
  const counts = {
  ...
  assert.equal(total, 54, "ロック対象は合計54件");
});
