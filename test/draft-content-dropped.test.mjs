...
//   glossary は、初期4軸（coinage-suspect 等）こそ配布 scaffold に載るが、A38 以降に足された軸
//   （invariant-oracle-missing / requirement-oracle-check / compass-rule-decay / provisional-carryover /
//   oracle-test-link-missing）はいずれも dogfood（.intent/glossary.md）のみに登録される慣行になっている。
//   本軸もそれに揃え、glossary 登録は dogfood でのみ検査する。
...
// 注: 配布 scaffold の glossary には初期4軸（coinage-suspect / groundless-conclusion /
//   unverified-hypothesis / dangling-reference）だけが載っており、A38 以降に足された軸
//   （invariant-oracle-missing / requirement-oracle-check / compass-rule-decay /
//   provisional-carryover / oracle-test-link-missing）はいずれも dogfood のみに登録されている。
//   「配布には一切積まない」規則ではなく「途中から積まなくなった」慣行であり、本軸もそれに揃える
//   （利用者の repo が使う語彙の母集合を、そのプロジェクトで実際に使う語に保つため）。
test(`8(b): 配布 scaffold の glossary へ新しい検査軸（${CHECK_ID}）を積まない（A38 以降の軸と同じ扱い）`, () => {
  for (const lang of LANGS) {
    const g = fs.readFileSync(path.join(TEMPLATES, lang, "intent", "glossary.md"), "utf8");
    assert.ok(
      !g.includes(CHECK_ID),
      `templates/${lang}/intent/glossary.md に本軸を積まない（A38 以降の既存6軸と同じ扱い）`,
    );
    // 同じ扱いの先行例が実在することを固定する（慣行の根拠が消えたら気づけるように）。
    assert.ok(
      !g.includes("compass-rule-decay") && !g.includes("provisional-carryover"),
      `templates/${lang}/intent/glossary.md: A38 以降の既存軸も配布 scaffold に載っていない`,
    );
  }
});
