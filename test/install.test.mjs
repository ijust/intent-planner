...
const JA_COUNT = listLangFiles(JA_ROOT).length;
const EN_COUNT = listLangFiles(EN_ROOT).length;
...
test("install(en): templates/en/** と配置結果が 1:1 一致 (内容・件数・3階層ネスト)", () => {
  ...
  try {
    ...
    assert.equal(langFiles.length, EN_COUNT, "templates/en は en/ja パリティのファイル数");
    assert.equal(EN_COUNT, JA_COUNT, "en と ja のファイル数は一致 (翻訳パリティ)");
    assert.equal(result.copied.length, EN_COUNT, "en 配置で全ファイルコピー");
    ...
    assert.equal(placedFiles.length, EN_COUNT, "配置先にも全ファイルのみ (余剰なし)");
...
test("install(ja 既定): templates/ja/** と配置結果が byte 完全一致 (移行回帰の核心)", () => {
  ...
  try {
    ...
    assert.equal(langFiles.length, JA_COUNT, "templates/ja は en/ja パリティのファイル数");
    assert.equal(JA_COUNT, EN_COUNT, "ja と en のファイル数は一致 (翻訳パリティ)");
    assert.equal(result.copied.length, JA_COUNT, "ja 既定で全ファイルコピー");
