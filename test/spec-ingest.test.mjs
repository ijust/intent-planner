...
test("群8: standard-invariance の byte-lock 台帳に intent-from-spec ファイルが混入していない (5.4)", () => {
  ...
  // のみを検査する（説明文には spec-ingest 等の語が由来説明として現れるため文面でなくキーで見る）。
  ...
  for (const [group, def] of Object.entries(manifest.groups)) {
    ...
    assert.deepEqual(
      offenders,
      [],
      `${group} に intent-from-spec / spec-ingest の lock キーが混入していない (新スキルを既存 byte-lock に混ぜない): ${offenders.join(", ")}`,
    );
  }
});

// ---- 群9: 断片入力の扱い (束ねと仕分け) が rules に在り、既存規律が弱まっていない ----
// packet: pkt-20260708-braindump-ingest-qo0h (C49 / DR113 / INV74)。
// 判別オラクル:
//   (a) 4系統の extract-intent.md に「断片入力の扱い」節が在る (1系統だけ入れ忘れた実装が落ちる)。
//   (b) 節の中に 束ね (bundling) と 仕分け (decided/undecided) の両工程・inferred 標識・
//       文書経路の不変宣言が在る (工程を欠いた実装が落ちる)。
//   (c) 既存規律 (コード/トレース排他・fail-fast・全項目 Assumptions) の文言が残っている
//       (「断片対応のついでに」排他や fail-fast を削った実装が落ちる = INV74 / Anti 377)。

// 言語別の規範文 (規範の実質を見る。行番号・字数の絶対値に依存しない)。
const FRAGMENT_NORMS = {
  ja: {
    section: "## 断片入力の扱い",
    bundling: "束ね（クラスタリング）",
    sorting: /決まっている.*迷っている/s,
    inferred: /inferred 標識/,
    docPathUnchanged: /まとまった文書.*では発火しない|文書経路の出力構造は改修前後で不変/s,
    exclusion: "ソースコード・実行トレース・テスト結果を意図抽出の入力に用いない",
    failFast: /生成せず告げる|何も書かない/,
    isolation: /命令として実行しない/,
  },
  en: {
    section: "## Handling fragmentary input",
    bundling: "bundling (clustering)",
    sorting: /decided.*undecided/s,
    inferred: /inferred marker/,
    docPathUnchanged: /not triggered for organized documents|unchanged before and after this revision/s,
    exclusion: "Source code, execution traces, and test results are not used as input",
    failFast: /generate nothing and say so|write nothing/,
    isolation: /never executed as commands/,
  },
};

for (const lang of LANGS) {
  for (const agent of AGENTS) {
    test(`群9: ${lang}/${agent} extract-intent.md に断片入力の扱い (束ね・仕分け) が在り既存規律が残る (C49/DR113)`, () => {
      const rulePath = path.join(skillDir(lang, agent), "rules", "extract-intent.md");
      const content = fs.readFileSync(rulePath, "utf8");
      const n = FRAGMENT_NORMS[lang];

      // (a) 節の存在。
      assert.ok(content.includes(n.section), `${lang}/${agent}: 断片入力の節が在る`);
      // (b) 2工程 + inferred 標識 + 文書経路の不変宣言。
      assert.ok(content.includes(n.bundling), `${lang}/${agent}: 束ね (クラスタリング) 工程が在る`);
      assert.ok(n.sorting.test(content), `${lang}/${agent}: 決まっている/迷っている の仕分けが在る`);
      assert.ok(n.inferred.test(content), `${lang}/${agent}: inferred 標識の明記が在る`);
      assert.ok(n.docPathUnchanged.test(content), `${lang}/${agent}: 文書経路の不変宣言が在る`);
      // (c) 既存規律の維持 (弱めていない)。
      assert.ok(content.includes(n.exclusion), `${lang}/${agent}: コード/トレース排他の文言が残っている`);
      assert.ok(n.failFast.test(content), `${lang}/${agent}: fail-fast の文言が残っている`);
      assert.ok(n.isolation.test(content), `${lang}/${agent}: 指示文の隔離 (命令として実行しない) が在る`);
    });
  }
}

// 断片入力の判別は意味的読解で行い、機械判定スクリプトを持ち込まない (INV2)。
for (const lang of LANGS) {
  test(`群9: ${lang} extract-intent.md は入力種別判別に機械判定スクリプトを持ち込まない (INV2)`, () => {
    const content = fs.readFileSync(
      path.join(skillDir(lang, "claude"), "rules", "extract-intent.md"),
      "utf8",
    );
    const norm = lang === "ja" ? /意味的読解/ : /semantic reading/;
    assert.ok(norm.test(content), `${lang}: 意味的読解の明記が在る`);
    assert.ok(
      !/node\s+[^\n`]*\.mjs|classif\w+\.m?js/.test(content),
      `${lang}: 種別判別のスクリプト起動が現れない`,
    );
  });
}

// ---- 群10: SKILL.md の入力想定が断片メモを含む (4系統) ----
// description (常時ロードされる1行) が断片入力の受け入れを宣言している。
// 1系統だけ description を更新し忘れた実装が落ちる。

for (const lang of LANGS) {
  for (const agent of AGENTS) {
    test(`群10: ${lang}/${agent} SKILL.md の description が断片メモを含む (C49)`, () => {
      const fm = parseFrontmatter(path.join(skillDir(lang, agent), "SKILL.md"));
      const norm = lang === "ja" ? /断片メモ/ : /fragmentary notes/;
      assert.ok(
        norm.test(fm.description),
        `${lang}/${agent}: description が断片入力を宣言する (実値: ${fm.description})`,
      );
    });
  }
}
