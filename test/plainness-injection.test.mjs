...
// ---- 5. dogfood（.claude）に、このテストが所有する平易さ契約が同期している（存在すれば検査） ----
test("5: dogfood の問い rules に平易さ点検が同期し、専用ruleはja/claudeとbyte一致する（存在すれば検査）", () => {
  // .claude（非追跡ミラー）だけでなく .agents（追跡ミラー）も見る（report-plainness.test の両ツリー検査と同型・
  // dialogue-precision の独立レビュー指摘 2026-07-17 で追補）。
  for (const rel of QUESTION_RULES) {
    for (const tree of [".claude", ".agents"]) {
      const dogfood = path.join(REPO_ROOT, tree, "skills", rel);
      if (fs.existsSync(dogfood)) {
        assert.equal(
          block(fs.readFileSync(dogfood, "utf8"), "ja"),
          block(fs.readFileSync(p("ja", "claude", rel), "utf8"), "ja"),
          `${tree}/skills/${rel} の平易さ点検は ja/claude と byte 同一`,
        );
      }
    }
  }

  for (const rel of ["intent-export-cc-sdd/rules/map-cc-sdd.md", "intent-validate/rules/validate-checks.md"]) {
