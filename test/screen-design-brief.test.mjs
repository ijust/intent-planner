...
function screenDesignRuleErrors(text, lang) {
  const patterns = lang === "ja"
    ? [
        ...
        [/問いの量・歯止め——まとめて少数・保留可・尋問調にしない——は変えない/, "質問戦略が歯止めを変えない"],
        [/答えによって下書き・モックの結果が大きく変わる未確定点を優先する/, "生成を左右する未確定点から問う"],
        [/全体の方向（テーマ・トーン・誰向けか）→画面の構造（レイアウト・情報の優先順位）→個別の部品/, "意味の階層順に詰める"],
        [/逆順に聞くと手戻りになる/, "階層順の理由が書かれている"],
        [/Sketch2Code ベンチマーク, 2024。Web デザイン文脈での知見/, "質問戦略に出典と適用範囲がある"],
        [/選択肢と自由回答を併置し/, "答えやすい形で問う"],
        ...
        [/見た目の方向が依頼者の中でまだ定まっていないと読めるとき/, "並列提示の発火は発散と読めるときだけ"],
        [/実質的に異なるテーマ案2〜3案を対等・根拠付きで並べ/, "複数案を対等に並べる"],
        [/同じ案の言い換えで数を膨らませない/, "案の水増しをしない"],
        [/初回限定・毎回・回数上限のような機械的な条件は置かず/, "機械的な発火条件を置かない"],
        [/依頼者が1案を選んだら、以後はその案だけを単一案として反復改善する/, "選択後は単一案の反復へ切り替える"],
        [/DreamSketch, UIST 2017 \/ DesignTrace, CHI 2026/, "並列提示に出典がある"],
        [/\*\*利用者の合意または明示の終了だけ\*\*で終える/, "往復の終了は利用者に属する"],
        [/「満足したはず」と AI が自己宣言して打ち切らない/, "満足を自己宣言しない"],
        [/版ごとに1行（版番号・何を変えたか・依頼者の反応の要約）/, "往復ログは版ごと1行"],
        [/ファイルは現行版のみを保持し、全置換の規律は変えない/, "現行版のみ保持で全置換不変"],
        [/版ファイルの連番保存・履歴ディレクトリを作らない/, "版ファイルを増やさない"],
        [/ログの記述を根拠に該当状態を再生成する/, "過去案はログから再生成する"],
        [/ログから再現できないときはその旨を正直に伝え/, "再現できないときは正直に伝える"],
        [/HistoryPalette, CHI 2026/, "往復ログに出典がある"],
    ...
    : [
        ...
        [/few at a time, deferrable, never interrogative — stay unchanged/, "the strategy keeps the guardrails"],
        [/prioritize the unsettled points whose answer would change the draft or the mock the most/, "asks the outcome-swaying unsettled points"],
        [/overall direction \(theme, tone, audience\) → screen structure \(layout, information priority\) → individual parts/, "settles meaning top-down"],
        [/asking in reverse order causes rework/, "states why the order matters"],
        [/the Sketch2Code benchmark, 2024; a finding in a web-design context/, "the strategy cites its source and scope"],
        [/pair options with free-form answers/, "asks in an answerable form"],
        ...
        [/when the visual direction reads as not yet settled in the requester's mind/, "parallel proposals fire only while unsettled"],
        [/lay out 2–3 substantively different theme proposals side by side, evenly and with grounds/, "lays proposals out evenly with grounds"],
        [/never pad the count with rewordings of the same proposal/, "no padding with rewordings"],
        [/Place no mechanical condition such as first-time-only, every-time, or a run cap/, "no mechanical firing condition"],
        [/Once the requester picks one, iterate on that single proposal from then on/, "switches to single-proposal iteration after the pick"],
        [/DreamSketch, UIST 2017 \/ DesignTrace, CHI 2026/, "parallel proposals cite their sources"],
        ...
        [/one line per version \(version number, what changed, a summary of the requester's reaction\)/, "the loop log is one line per version"],
        [/Keep only the current version of the file and leave the full-replacement rule unchanged/, "keeps only the current version"],
        [/create no numbered version files and no history directory/, "creates no version files"],
        [/regenerate that state from the log's description/, "regenerates past states from the log"],
        [/When the log cannot reproduce it, say so honestly/, "admits when the log cannot reproduce"],
        [/HistoryPalette, CHI 2026/, "the loop log cites its sources"],
...
test("ScreenDesignRule: 沈黙条件の逆転・標識の削除・出力先の変更などの誤実装を判別する", () => {
  ...
  const mutations = [
    ...
    ["歯止めの緩和へ逆転", "問いの量・歯止め——まとめて少数・保留可・尋問調にしない——は変えない", "重要な案件では一問一答で全論点を確認してよい"],
    ["階層順を逆転", "全体の方向（テーマ・トーン・誰向けか）→画面の構造（レイアウト・情報の優先順位）→個別の部品（色・部品の形）", "個別の部品（色・部品の形）→画面の構造→全体の方向"],
    ["質問戦略の出典を削除", "（Sketch2Code ベンチマーク, 2024。Web デザイン文脈での知見）", "（一般的な知見）"],
    ["常時複数案へ逆転", "見た目の方向が依頼者の中でまだ定まっていないと読めるときは", "どの依頼でも最初は必ず"],
    ["並列提示の機械条件化へ逆転", "初回限定・毎回・回数上限のような機械的な条件は置かず", "並列提示は初回の1回に限り"],
    ["選択後も複数案継続へ逆転", "依頼者が1案を選んだら、以後はその案だけを単一案として反復改善する", "依頼者が1案を選んだ後も毎回複数案を並べて提示する"],
    ["版ファイル保存へ逆転", "版ファイルの連番保存・履歴ディレクトリを作らない", "版ごとに連番ファイルとして保存し履歴ディレクトリへ蓄積する"],
    ["往復ログの削除", "版ごとに1行（版番号・何を変えたか・依頼者の反応の要約）", "特に記録は残さず、依頼者の記憶"],
    ["再現の偽装へ逆転", "ログから再現できないときはその旨を正直に伝え", "ログから再現できないときも最も近い状態を再現できたことにして"],
  ];
...
test("DocSync: README と guide と theory の日英が画面デザインの詰めを案内する", () => {
  ...
  assert.match(guideJa, /全体の方向→画面の構造→細部の順/, "guide ja が質問戦略の順序を示す");
  assert.match(guideEn, /overall direction → screen structure → details/, "guide en が質問戦略の順序を示す");
  assert.match(guideJa, /複数のテーマ案から始まり、1案を選んだ後は単一案の反復改善/, "guide ja が複数案→単一案の流れを示す");
  assert.match(guideEn, /multiple switchable theme proposals[\s\S]{0,120}single proposal/, "guide en が複数案→単一案の流れを示す");
  assert.match(guideJa, /版ごとの軽い記録[\s\S]{0,120}現行版のみ/, "guide ja が往復ログと現行版のみ保持を示す");
  assert.match(guideEn, /light record[\s\S]{0,200}only the current version/, "guide en が往復ログと現行版のみ保持を示す");
});
