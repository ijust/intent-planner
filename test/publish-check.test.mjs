...
test("(c) AGENT_REGISTRY は公開表示中のエントリ名だけを抽出する", () => {
  assert.deepEqual(mod.collectAgents(), ["claude", "codex"]);
});
...
test("(c) keywords から対応エージェントを1つ抜くと missing（誤実装=見逃しを落とす）", () => {
  const pkg = JSON.parse(fs.readFileSync(path.join(ROOT, "package.json"), "utf8"));
  pkg.keywords = pkg.keywords.filter((k) => k !== "codex");
  const res = mod.checkPromotion(pkg);
  assert.equal(res.status, "missing");
  assert.ok(res.reasons.join("\n").includes("codex"), "未収載の codex を列挙する");
});
