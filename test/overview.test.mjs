...
//     SKILL.md + 同名 10 rules が 1:1 で存在 (R7.1, R7.2)。
...
// ---- 4系統パリティ: SKILL.md + 同名 10 rules が 1:1 で存在 (R7.1, R7.2) ----
for (const lang of LANGS) {
  for (const agent of AGENTS) {
    test(`4系統パリティ: ${lang}/${agent} に SKILL.md + 10 rules が存在する`, () => {
      ...
      // 余剰の rules が無い (10 枚ちょうど)。
      const present = fs
        .readdirSync(path.join(root, "rules"))
        .filter((f) => f.endsWith(".md"))
        .sort();
      assert.deepEqual(present, RULES.map((r) => `${r}.md`).sort(), `${lang}/${agent}: rules は 10 枚ちょうど`);
    });
  }
}
