...
const FORBIDDEN_BEHAVIORS = Object.freeze([
  ["answer-proposal-omitted", /`proposals: off`.+重要判断.+回答案・理由・推奨を変える条件.+省略する/s],
  ["short-approval-broadens-product-priority", /「OK」「次」.+製品の方向.+実装の優先順位.+決定済みとして扱う|短い承認を明示続行として扱う|(?:shall|may|must|can|should) treat (?:a )?short approval as explicit continuation/is],
  ["explicit-continuation-erases-unresolved", /明示続行なら.+回答済み・解決済みとして.+Open Question.+消去する|明示続行を解決済みとして(?:扱う|消去する)|(?:shall|may|must|can|should) mark explicit continuation as resolved/is],
  ["unknown-evidence-stops-whole-project", /紐付け先や影響根拠が不明なら案件全体を停止する|紐付け不能(?:だけ)?で案件全体を停止する|(?:shall|may|must|can|should) stop the entire project (?:only )?because (?:the )?link(?:age)? is unknown/is],
  ["retroactive-legacy-bulk-migration", /過去分を含むすべての既存 Open Questions.+遡及して一括変更・一括移行する|未接触(?:の旧項目)?を一括(?:変更|移行)する|(?:shall|may|must|can|should) bulk migrate untouched/is],
]);
...
function contractViolations(text) {
  ...
  violations.push(...forbiddenBehaviorViolations(text));
  return violations;
}

function forbiddenBehaviorViolations(text) {
  const violations = [];
...
test("必須意味と出口開始確認の削除を、注入確認後に拒否する", () => {
...
test("実際の共通契約へ禁止動作を個別注入すると、対応する違反だけを検出する", () => {
  const relativePath = "templates/ja/codex/skills/CONTRACT.md";
  const baseline = fs.readFileSync(path.join(ROOT, relativePath), "utf8");
  assert.deepEqual(forbiddenBehaviorViolations(baseline), []);

  const mutations = [
    {
      id: "answer-proposal-omitted",
      search: "重要判断の回答案・理由・推奨を変える条件は省略しない",
      replacement: "重要判断の回答案・理由・推奨を変える条件は省略する",
    },
    {
      id: "short-approval-broadens-product-priority",
      search: "未決の重要判断への決定にも明示続行にも読み替えない",
      replacement: "製品の方向や実装の優先順位を含む未決の重要判断を決定済みとして扱う",
    },
    {
      id: "explicit-continuation-erases-unresolved",
      search: "明示続行なら、回答済み・解決済みにせず未決のまま保持する",
      replacement: "明示続行なら、回答済み・解決済みとして Open Question を消去する",
    },
    {
      id: "unknown-evidence-stops-whole-project",
      search: "紐付け不能だけを理由に案件全体を停止しない",
      replacement: "紐付け先や影響根拠が不明なら案件全体を停止する",
    },
    {
      id: "retroactive-legacy-bulk-migration",
      search: "今回触れていない既存 Open Questions は一括変更・一括移行しない",
      replacement: "過去分を含むすべての既存 Open Questions を遡及して一括変更・一括移行する",
    },
  ];
  ...
  for (const mutation of mutations) {
    ...
    assert.deepEqual(
      forbiddenBehaviorViolations(mutated),
      [`forbidden:${mutation.id}`],
      mutation.id,
    );
  }
});
...
const PUBLIC_DECISION_DOCUMENTS = Object.freeze([
  ...
  ["theory/en", "docs/theory.en.md", "en"],
]);

const PUBLIC_DECISION_DOCUMENT_MEANINGS = Object.freeze({
  ja: [
    ["重要判断の分類", /目的・対象者・成果・範囲・受入条件・守る約束・外部契約.+(?:いずれか|どれか).+未決.+重要判断/s],
    ["重要判断の追加条件", /後戻り.+複数 packet.+(?:別の|追加).+条件/s],
    ["回答案の三要素", /回答案.+理由.+推奨を変える条件/s],
    ["三つの結果", /決定.+今回の範囲外.+範囲限定の明示続行/s],
    ["短い承認の限定", /「OK」「次」.+(?:決定|明示続行).+(?:扱わない|読み替えない)/s],
    ["明示続行の限定", /範囲限定の明示続行.+未決.+許可された項目と範囲/s],
    ["影響範囲だけ停止", /根拠.+影響範囲だけ.+停止/s],
    ["全工程と出口", /discover.+compass.+packets.+cc-sdd.+OpenSpec.+Spec Kit.+自然言語 Spec.+direct.+intent-plan.+実装/s],
    ["外部ツール非所有", /外部.+(?:spec|実装).+ツール.+(?:状態|セッション).+管理しない/s],
  ],
  en: [
    ["important-decision classification", /purpose, target users?, outcomes?, scope, acceptance criteria, promises to preserve, or external contracts.+unresolved.+important decision/is],
    ["additional important-decision conditions", /hard-to-reverse.+multiple packets.+(?:separate|additional) conditions/is],
    ["answer proposal evidence", /answer proposal.+rationale.+condition that would change the recommendation/is],
    ["three outcomes", /decision.+out-of-scope for this work.+scope-limited explicit continuation/is],
    ["short approval is bounded", /“OK”.+“next”.+(?:does not|do not).+(?:decision|explicit continuation)/is],
    ["continuation remains bounded", /scope-limited explicit continuation.+unresolved.+authorized item and scope/is],
    ["affected scope only", /evidence.+stop only the affected scope/is],
    ["all stages and routes", /discover.+compass.+packets.+cc-sdd.+OpenSpec.+Spec Kit.+natural-language Spec.+direct.+intent-plan.+implementation/is],
    ["external tool state is not owned", /do not manage.+(?:state|session).+external spec or implementation tools/is],
  ],
});

const GUIDE_RECORD_FIELDS = Object.freeze([
  "Decision deadline",
  "Owning stage",
  "Proposed answer",
  "Rationale",
...
test("公開ガイドは重要な Open Question 1件の記録例と工程ごとの確認時点を示す", () => {
  const guides = [
    ["docs/guide.md", /discover.+終了.+compass.+開始.+終了.+packets.+開始.+終了.+export.+direct.+実装/s],
    ["docs/guide.en.md", /discover.+end.+compass.+start.+end.+packets.+start.+end.+export.+direct.+implementation/is],
  ];
  for (const [relativePath, stageTimingPattern] of guides) {
    const text = fs.readFileSync(path.join(ROOT, relativePath), "utf8");
    for (const field of GUIDE_RECORD_FIELDS) {
      assert.match(text, new RegExp(`\\b${field}\\b`, "i"), `${relativePath}: ${field}`);
    }
    assert.match(text, /continue-authorized.+unresolved/is, `${relativePath}: continuation remains unresolved`);
    assert.match(text, stageTimingPattern, `${relativePath}: stage timing table`);
  }
});

test("INV10: 公開文書は重要判断の解決規律を日本語・英語で案内する", () => {
  for (const [label, relativePath, language] of PUBLIC_DECISION_DOCUMENTS) {
    const text = fs.readFileSync(path.join(ROOT, relativePath), "utf8");
    for (const [meaning, pattern] of PUBLIC_DECISION_DOCUMENT_MEANINGS[language]) {
      assert.match(text, pattern, `${label}: ${meaning}`);
    }
  }
});
