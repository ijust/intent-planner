...
test("dogfood 参照保全: active packet の parent_intents 記号が新旧どちらかの収納から到達可能", () => {
  ...
  // C/A 記号の正本は tree・INV/DR/Anti の正本は compass。到達性は両正本+分割収納+退避先で判定する
  // （退避は move であって削除ではない＝現用の参照性は history ファイルで保つ・INV40/DR64）。
  // tree-normalize（P-fed6）で案件記録（機能追記/機能撤去/履歴/再起案）が本体末尾から
  // 分割収納 .intent/tree/ へ move された。C/A 記号の多くはその案件記録の中で定義・参照される
  // ため、到達性判定は分割収納 .intent/tree/ も含める（move であって削除でない＝参照性は保つ・
  // INV80「失ってはいけない4点」②既存参照の到達性）。収納が無ければ従来どおり本体末尾で拾う（DR133）。
  const treeStoreDir = path.join(root, ".intent/tree");
  const treeStoreBody = fs.existsSync(treeStoreDir)
    ? fs
        .readdirSync(treeStoreDir)
        .filter((f) => f.endsWith(".md"))
        .map((f) => read(path.join(".intent/tree", f)))
        .join("\n")
    : "";
  const oldBody =
    read(".intent/intent-compass.md") +
    "\n" +
    read(".intent/intent-tree.md") +
    "\n" +
    treeStoreBody +
    "\n" +
    (fs.existsSync(path.join(root, ".intent/intent-tree.history.md")) ? read(".intent/intent-tree.history.md") : "") +
    "\n" +
    (fs.existsSync(path.join(root, ".intent/compass-history.md")) ? read(".intent/compass-history.md") : "");
