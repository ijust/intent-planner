...
test("6.2: enforce 省略と enforce:false の plan は deepEqual で従来形（フックエントリ・mode キー無し）", () => {
  ...
  try {
    ...
    for (const e of plain) {
      // claude-md-onboarding-doc (2026-06) で templates/{ja,en}/agents/claude/CLAUDE.md が
      // 実在化し、claude 既定計画にルート直下 CLAUDE.md が入る (codex/AGENTS.md と同型)。
      // Anti-533 (2026-07-14) で本体 CLAUDE_intent.md も常に計画へ入る（新規リポでも本体を配る）。
      assert.ok(
        e.relative.startsWith(".claude") ||
          e.relative.startsWith(".intent") ||
          e.relative === "CLAUDE.md" ||
          e.relative === "CLAUDE_intent.md",
        `従来形: relative は .claude/.intent 始まりかルート CLAUDE.md / CLAUDE_intent.md: ${e.relative}`,
      );
...
test("6.1: enforce + .git ありで plan 末尾に pre-push フックエントリ（COPY・mode 0o755）", () => {
  ...
  assert.deepEqual(
    last,
    {
      ...
      action: "COPY",
      // フックは shared 種別（ユーザー領域共有）。新規 (.git fixture は空) なので backup なし。
      kind: "shared",
      backup: false,
      mode: 0o755,
    },
