...
for (const lang of LANGS) {
  for (const agent of AGENTS) {
    ...
    test(`3c: ${lang}/${agent} の validate 規律が installer の詳細health契約を複製しない`, () => {
      ...
      assert.doesNotMatch(joined, /term-drift 0\.2\.1|\.term-drift\/version\.json|sha256/i, "互換版・version file・hash値を持たない");
      assert.doesNotMatch(joined, /\.claude\/skills\/term-drift|\.agents\/skills\/term-drift|\.gemini\/skills\/term-drift/, "agent対応表を持たない");
    });
  }
}
...
for (const lang of LANGS) {
  ...
  test(`3e: ${lang} guide が配置なし fallback と dry-run → ready → 専用 skill の手順を示す`, () => {
    ...
    if (lang === "ja") {
      ...
      assert.ok(/`npx intent-planner \. --agent <選択中のagent> --with-term-drift --dry-run`/.test(c), "installer dry-run の手動確認を示す");
      assert.ok(/`ready` になった後[\s\S]{0,100}term-drift 専用 skill から始めます/.test(c), "ready 後だけ専用 skill へ進む");
      assert.ok(/専用経路は `\/intent-validate` の構造検査を置き換えません/.test(c), "構造検査と詳細レビューを分ける");
    } else {
      ...
      assert.ok(/`npx intent-planner \. --agent <selected-agent> --with-term-drift --dry-run`/.test(c), "installer dry-run の手動確認を示す");
  ...
  test(`3f: ${lang} theory が予防・構造検査・専用レビューと health 正本を分離する`, () => {
    ...
    if (lang === "ja") {
      ...
      assert.ok(/配置物が見つかったら[\s\S]{0,160}validate は用語の意味や version\/hash\/skill の互換性を判定せず[\s\S]{0,120}`--with-term-drift --dry-run` health 確認へ案内/.test(c), "validate は判定せず installer health へ案内する");
      ...
      assert.ok(/repositories with a placement use validate only to guide the user to the existing installer's `--with-term-drift --dry-run` health display/i.test(c), "validate は installer health へ案内するだけである");
