...
import { install, detectTermDrift } from "../src/install.mjs";
...
// ---- 1. 存在検知は「対象リポに配置された目印」だけを見る（node_modules・npx キャッシュを見ない） ----
test("1: detectTermDrift は対象リポの .term-drift/ だけを見る（未導入では false）", () => {
  const dir = tmpDir();
  assert.equal(detectTermDrift(dir), false, "目印が無ければ未導入");
  fs.mkdirSync(path.join(dir, ".term-drift"));
  assert.equal(detectTermDrift(dir), true, "目印があれば導入済み");
});
...
test("1b: node_modules に term-drift のコピーがあっても導入済みと誤判定しない（過去版の影を踏まない）", () => {
  ...
  assert.equal(
    detectTermDrift(dir),
    false,
    "node_modules 配下のコピーを検知経路にしない（検知は対象リポの目印のみ）",
  );
});
...
test("2: install は term-drift を配置しない（既定 no・押し付けない）", () => {
  ...
  assert.equal(result.termDriftDetected, false, "未導入として報告される");
...
test("2b: 導入済みの repo では termDriftDetected=true になり、install は目印を壊さない（非破壊）", () => {
  const dir = tmpDir();
  fs.mkdirSync(path.join(dir, ".term-drift"));
  fs.writeFileSync(path.join(dir, ".term-drift", "glossary.md"), "# 利用者の台帳（消してはいけない）\n");
  const result = install(dir, { lang: "ja", agent: "claude" });
  assert.equal(result.termDriftDetected, true, "導入済みとして報告される");
  assert.equal(
