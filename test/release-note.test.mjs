...
// format-changelog-customer は pdm-presets packet で追加された顧客向け changelog の出力構造
// (利用者影響先行・DR93 の changelog-for-humans / doc-type-separation を設計制約に持つ)。
// format-pr-description は pr-description packet (pkt-20260706-pr-description-format-vazb・A55/C41/DR101)
// で追加された PR 説明の下書きの出力構造 (結論先行4部構成・INV67 要約+識別子・外部書き込みゼロ)。
// 内容の判別オラクルは test/pr-description-format.test.mjs が担う。
const RULE_NAMES = [
  "source-scope",
  "format-select",
  "format-changelog",
...
// ---- 群1: 4系統の SKILL.md + 6 rules + scaffold README が全て存在する (Req 1.1/1.2/1.3) ----
// 期待: 4 SKILL.md + 24 rule files + 2 scaffold README (ja/en)。1つでも欠ければ落ちる。

test("群1: 4系統 (ja/en × claude/codex) に SKILL.md + 6 rules が全て存在する (1.1/1.2)", () => {
  let skillCount = 0;
  let ruleCount = 0;
  ...
  assert.equal(ruleCount, 24, "rule は 4系統 × 6種 = 24 ちょうど");
});
