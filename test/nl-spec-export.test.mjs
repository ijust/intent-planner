...
//     1.  4系統 (ja/en × claude/codex) の SKILL.md + 8 rules が全て存在する (5.4)。
...
// ---- 群1: 4系統の SKILL.md + 8 rules が全て存在する (Req 5.4) ----
// 期待: 4 SKILL.md + 32 rule files = 36 ファイル。1つでも欠ければ落ちる。

test("群1: 4系統 (ja/en × claude/codex) に SKILL.md + 8 rules が全て存在する (5.4)", () => {
  let skillCount = 0;
  let ruleCount = 0;
  ...
  assert.equal(ruleCount, 32, "rule は 4系統 × 8種 = 32 ちょうど");
});
...
for (const agent of AGENTS) {
  test(`群2: ${agent} の intent-to-spec サブツリーが ja/en で 1:1 一致 (5.4)`, () => {
    ...
    // SKILL.md と 8 rules が確かに含まれる (空集合の偽陽性防止)。
