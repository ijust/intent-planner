...
test("公開 docs は owner 境界・安全な追加・更新方針を説明する", () => {
  const meaningChecks = {
    ...
    "docs/theory.md": [/--with-term-drift/, /term-drift 0\.2\.5/, /公式 installer/, /(?:自動更新|自動追随)/, /term-drift 所有/],
    "docs/theory.en.md": [
      /--with-term-drift/,
      /--yes/,
      /term-drift 0\.2\.5/,
...
test("ja/en CLI help は専用 opt-in と --yes の別責務を同じ固定版で案内する", () => {
  for (const [lang, separateConsent] of [
    ...
    assert.match(result.stdout, /--with-term-drift[\s\S]{0,160}term-drift 0\.2\.5/);
