...
function featureArtifactBoundaryErrors(changedPaths = serviceDesignCommitPaths()) {
  return SERVICE_DESIGN_DOGFOOD_ARTIFACTS
    .filter((file) => changedPaths.includes(file))
    .map((file) => `out-of-bound feature artifact: ${file}`);
}
...
test("BoundaryTests: 公開前のversion・依存と対象外の実装境界を保つ", () => {
  assert.deepEqual(
    packageBoundaryErrors(SERVICE_DESIGN_PACKAGE_BASELINE),
    [],
    "対象機能の基準入力でversion・依存を変更しない",
  );
...
test("BoundaryTests: version・依存・対象外・dogfood境界の違反を判別する", () => {
  assert.notDeepEqual(
    packageBoundaryErrors({ ...SERVICE_DESIGN_PACKAGE_BASELINE, version: "0.21.3" }),
    [],
    "version変更を検出する",
  );
  assert.notDeepEqual(
    packageBoundaryErrors({
      ...SERVICE_DESIGN_PACKAGE_BASELINE,
      dependencies: { ...SERVICE_DESIGN_PACKAGE_BASELINE.dependencies, analytics: "1.0.0" },
    }),
    [],
  ...
  assert.notDeepEqual(
    featureArtifactBoundaryErrors([".agents/skills/intent-discover/rules/design-frame-surfacing.md"]),
    [],
    "dogfood配置物の追加を検出する",
  );
});
