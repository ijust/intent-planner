...
- **読み手向けの一枚もの・報告にする** — 詰めた意図は、`/intent-to-spec` で関係者向けの読める文書へ、`/intent-overview` で散らばった意図の俯瞰ビューへまとめられます（どちらも `.intent/` の中に Markdown を作るだけで、アプリのコードには触れません）。`/intent-to-spec` には読み手別のかたち（**ステークホルダー向けの一枚もの**・**定例報告**、どちらも結論を先に書く形）が、`/intent-release-note` には**顧客向けの変更履歴**（利用者に何が変わるかを先に書く形）があり、専門用語を読み手に合わせて開いてくれます。
