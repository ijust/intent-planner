...
- **散らばった情報を見渡す** — `/intent-overview` では、未決事項や承認待ちを集めた「判断待ちの一覧」、作業順と依存関係を示す「ロードマップ」、意図や作業順を表す Mermaid 図、並行作業の担当を示す「割当ビュー」を作れます。また、次のセッションへ現在地・残作業・参照すべき正本・注意点・リポジトリの状態を渡す「引き継ぎブリーフ」も作れます。これは `.intent/` の状態から生成する一時的な文書で、会話ログは読みません。
