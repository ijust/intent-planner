...
term-drift も使う場合だけ、対話中の確認に同意するか `--with-term-drift` を付けると、選択中の agent 向けに互換確認済みの term-drift 0.2.5 を導入または更新します。`--yes` はルート文書追記だけの同意で、term-drift の同意にはなりません。状態は `not-installed`（未導入）、`ready`（利用可能）、`inconsistent`（部分導入または不整合）で表示し、`inconsistent` は、安全に不足だけを足せる `additive-compatible`、公式 update を試せる既知状態の `update-attemptable`、自動処理しない `blocked` に分かれます。`install-failed` は今回の試行失敗として配置状態と分けて表示します。新規導入と安全な追加は公式 installer、既知の旧版からの更新は公式 update に委譲します。更新は明示同意時だけで、未知の自己整合版や将来版には自動追随せず、term-drift 所有ファイルを独自に修復・上書きしません。`ready` になったら、選択中の agent の term-drift 専用 skill から本格的な用語点検を始めます。
...
- **`/intent-validate`** — 意図の文書どうしの矛盾・カバレッジ漏れ・境界のずれを、深刻度を付けて読み取り専用で報告します（直すのは提案までで、勝手に書き換えません）。export の前に一度かけておくと安心です。project-local な term-drift 配置物を見つけた場合も、`/intent-validate` 自身は用語や health を独自判定せず、外部コマンドも起動しません。利用者へ既存 installer の `npx intent-planner . --agent <選択中のagent> --with-term-drift --dry-run` を手動実行するよう案内し、`ready` の確認後だけ term-drift 専用 skill へ進みます。
