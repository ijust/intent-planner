...
4. **`/intent-export-cc-sdd`**（または `/intent-export-openspec`）— 選んだ作業単位を実装ツールの下書きに変換します。下書きには受入基準の材料（期待挙動・受入をどう測るか）が含まれ、下流の要件生成が薄くならないようになっています。
