...
- **読み手向けの一枚もの・報告にする** — 詰めた意図は、`/intent-to-spec` で関係者向けの読める文書へ、`/intent-overview` で散らばった意図の俯瞰ビューへまとめられます（どちらも `.intent/` の中に Markdown を作るだけで、アプリのコードには触れません）。`/intent-to-spec` には読み手別のかたち（**ステークホルダー向けの一枚もの**・**定例報告**、どちらも結論を先に書く形）が、`/intent-release-note` には**顧客向けの変更履歴**（利用者に何が変わるかを先に書く形）と **PR 説明の下書き**（コミット範囲から「なぜこの変更か」「レビューで先に見る点」「コミットと意図の対応」を組み立てる形。GitHub には書き込まず、貼り付けはあなたが行います）があり、専門用語を読み手に合わせて開いてくれます。`/intent-overview` には、まだ決まっていない問い・承認待ちの学び・警告を1画面に集めた**判断待ちの一覧**（インボックス）と、作業の順序と「何が何を塞いでいるか」を日付なしで映した**ロードマップ**もあり、「判断待ち」「ロードマップ」と指定すると出せます。
