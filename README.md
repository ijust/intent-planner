...
- **画面デザインの詰めと下書き** — UI を含む案件で深掘りまたは画面設計の観点を選んだときだけ、画面の目的・情報の優先順位・主要状態・見た目の方向を対話で詰め、推測標識付きの下書きを `.intent/nl-spec/screen-design-brief.md` に作ります。画像・完成モック・ブランドの自動確定は対象外です → [画面デザインの詰めと下書き](docs/guide.md#画面デザインの詰めと下書き)
