...
- **並行 AI セッションと領域ガバナンス** — 判断基準（compass）と意図ツリーが大きくなっても、実体は割らず「所有権と処理範囲」だけを領域ごとに委ねる考え方（宣言型・止めない・宣言が無ければ従来どおり）。使い方は [docs/guide.md の「領域ガバナンス」節](docs/guide.md#領域ガバナンス並行セッションの所有権と実行範囲任意)、背景は [docs/theory.md の「領域ごとに管理責任を分ける」節](docs/theory.md)
