...
特に向いているのは、設計の逸脱や統合ミスが大きな損失になり、やり直しの手戻りが高い**高リスクな案件**です。反対に、短い試作や vibe coding で十分な小さな変更には、この工程が**過剰**になりえます。その場合は packet から直接実装する `direct` 経路を選ぶか、intent-planner 自体を省略できます。品質上の境界を弱めるのではなく、案件に必要な最小限だけを使います。
...
term-drift 0.3.0 は intent-planner の exact npm 依存として標準導入されます。通常の intent-planner セットアップは、選択中の agent を term-drift の公式 installer へ渡し、owner 方針どおり対象repositoryの `./.term-drift/` と専用 skillをproject-localに配置します。旧 `--with-term-drift` は既存scriptの互換性のため受理しますが、配置の条件ではありません。状態は `not-installed`（未導入）、`ready`（利用可能）、`inconsistent`（部分導入または不整合）で表示し、`inconsistent` は、安全に不足だけを足せる `additive-compatible`、公式 update を試せる既知状態の `update-attemptable`、自動処理しない `blocked` に分かれます。`install-failed` は今回の試行失敗として配置状態と分けて表示します。新規導入と安全な追加は公式 installer、既知の旧版からの更新は公式 update に委譲し、`ready` は再実行しません。未知の自己整合版や将来版には自動追随せず、term-drift 所有ファイルを独自に修復・上書きしません。0.3.0 では、1語ずつ確認した承認済みの一般語分類を既存の台帳へ保存し、別セッションで復元できます。この承認は語の分類だけに効き、同じ字面の内輪転用や曖昧な文章は引き続き点検します。`ready` になったら、選択中の agent の term-drift 専用 skill から本格的な用語点検を始めます。
...
compass の分割収納への移行は opt-in です。旧単一ファイルの読み取りは恒久的に維持されます。
...
- **`/intent-validate`** — 意図の文書どうしの矛盾・カバレッジ漏れ・境界のずれを、深刻度を付けて読み取り専用で報告します（直すのは提案までで、勝手に書き換えません）。export の前に一度かけておくと安心です。project-local な term-drift 配置物を見つけた場合も、`/intent-validate` 自身は用語や health を独自判定せず、外部コマンドも起動しません。利用者へ通常 installer の `npx intent-planner . --agent <選択中のagent> --dry-run` でhealthを確認するよう案内し、`ready` の確認後だけ term-drift 専用 skill へ進みます。
