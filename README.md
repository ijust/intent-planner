...
特に向いているのは、設計のずれや統合ミスが大きな損失につながり、やり直しにも多くの時間がかかる**高リスクな案件**です。反対に、短い試作や vibe coding で十分な小さな変更には、この工程が**重すぎる**こともあります。その場合は、作業単位（packet）から直接実装する `direct` 経路を選ぶか、intent-planner 自体を省略できます。案件に必要な分だけ使うのが基本です。
...
> 📌 技術的には、intent-planner は **AI コーディングエージェント（Claude Code / Codex / Gemini CLI）向けの「仕様作成前の方針整理レイヤー（Pre-spec Steering Layer）」**です。仕様を書く一段手前で、プロジェクト全体に関わる意図と設計方針を整理し、実装中も参照できる形で残します。エンジニア向けの詳しい説明は[後半](#sec-prespec)にあります。
...
新規導入と安全な追加は公式 installer、対応済みの旧版からの更新は公式 update に任せます。`ready` なら再実行せず、未知の版や将来の版にも自動追随しません。また、intent-planner が term-drift 所有のファイルを独自に修復したり、上書きしたりすることもありません。term-drift 0.3.0 では、1語ずつ確認して承認した一般語の分類を既存の用語台帳に保存し、別のセッションでも再利用できます。ただし、承認されるのは語の分類だけです。同じ表記を内輪の意味で使っている箇所や、曖昧な文章は引き続き点検します。状態が `ready` になったら、選択したエージェントの term-drift 専用スキルから本格的な用語点検を始められます。
...
- **`/intent-validate`** — 意図の文書どうしの矛盾、抜け、境界のずれを、深刻度を付けて読み取り専用で報告します。修正案は示しますが、ファイルを勝手に書き換えません。実装ツールへ渡す前に一度実行しておくと安心です。プロジェクト内に term-drift の配置物がある場合も、`/intent-validate` 自身は用語や導入状態を判定せず、外部コマンドも実行しません。代わりに、通常の installer を `npx intent-planner . --agent <選択中のエージェント> --dry-run` で実行して状態を確認するよう案内します。`ready` が確認できたら、term-drift 専用スキルへ進みます。
