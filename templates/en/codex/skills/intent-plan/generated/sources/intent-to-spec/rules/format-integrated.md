...
### Material for the screens / touchpoints section (projection of the screen-sketch reference; when there is no material, choose per case)

A response to the reader feedback "there are no images, so the UI is unclear." Whether the screens / touchpoints material exists varies by case, and how a case with none is handled **holds no fixed policy — the user chooses per case** (DR190).

- **A case where the screen-sketch record exists as a "path/link"**: place that reference (path/link) in the screens / touchpoints section. Do not place as confirmed a path whose existence cannot be reconciled; add "reference not verified" (the same temperature as the fabrication-guard rule).
- **A case where the screen-sketch record is "out of scope"**: do not raise the screens / touchpoints section (a case with no GUI — and do not add dialogue either).
- **A case where the screen-sketch record is "none" (not out of scope)**: in the pre-generation dialogue (SKILL.md Step 1, the same position as the depth confirmation), the user chooses one of (DR190):
  - **Derive a screen-transition diagram with an inferred mark**: infer the screen transitions from the packet's behavior descriptions (Expected Behavior) and L3's UI/UX constraints, and place it as a Mermaid diagram in the screens / touchpoints section. The diagram **must have an inferred mark** and be listed in the fabrication-guard's completed (inferred) list (do not draw screens absent from the material as confirmed). Reference overview's `mermaid-tree.md` for the drawing conventions (do not duplicate — DR187).
  - **Only state "no screen material"**: show "no screen material" in the screens / touchpoints section, and add one line of a path noting that recording a screen sketch via `/intent-discover` thickens the screen section (remediate on the material side).
- When the handling is made explicit in the arguments, do not ask (the same asymmetry as depth and output form).
...
- Raise the screens / touchpoints section only when the screen-sketch reference exists as a "path/link" and place the reference; do not raise the section when "out of scope." When there is no screen-sketch record (not out of scope), hold no fixed policy: in the pre-generation dialogue, the user chooses per case between "deriving a screen-transition diagram with an inferred mark" and "stating no screen material" (DR190). A derived screen-transition diagram must have an inferred mark and be listed in the fabrication-guard's completed list (do not draw screens absent from the material as confirmed — INV100).
