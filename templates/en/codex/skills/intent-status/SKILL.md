...
- **Success Criteria** (success if achieved; the detailed check procedures are consolidated into Execution Steps, and only the outcomes are declared here):
  ...
  - Unconsumed milestones (recorded but the corresponding Decision revisit unhandled) are included as residuals (Step 3.7; "shared temperature")
...
**What this makes them grasp first**: slim down the default and present (1) the progress rail (🔵 you are here, ⚪ remaining work, 🔴 unreflected visible at a glance) → (2) exactly one next move (one-line summary, never folded, always emphasized) → (3) Candidate Packets count + names → (4) a one-line Ice box notice. The "dangerous notices" (freshness warning, integrity violations, unreflected 🔴) are kept in full in the default, while internal terms and check details are relegated to (5) details.

The output composition, the content of each layer, and the relegation discipline are **governed by Step 5 "Report" (not re-listed here)**. Skeleton only: default ((1) progress rail / (2) next move / (3) Candidate / (4) Ice box notice / ⊕ dangerous notices) → (5) details (the folded position) → (6) Open Questions → (7) Ice box expansion (only on a natural-language trigger).
