...
- **Success Criteria**:
  ...
  - **Text-match** each commit against intent (packet name / parent intent / deltas) and, for matched ones, attach the "why (for which intent it changed)."
  - List unmatched commits as thin changelog lines, **surfacing the gap between intent and reality** (do not silently drop them).
  - Derive output under `.intent/release-note/` following a format (select `rules/format-changelog.md` / `rules/format-github-releases.md` / `rules/format-changelog-customer.md` / `rules/format-pr-description.md` / `rules/format-trajectory.md` by argument) by full replacement.
...
- **When there is no trailer, fall back to text-match (guess)**: commits without a trailer are **text-matched** against intent as before. The priority of matching material is (1) packet name → (2) parent intent → (3) deltas. Match the contents under `.intent/` (`packets/` / `intent-tree.md` / `intent-compass.md` / `deltas.md`) and the commit messages, **within what is mechanically observable from files**. When multiple apply, take the highest-priority material.
...
- Fix the format per `rules/format-select.md`. With argument `changelog` / `github-releases` / `changelog-customer` (customer-facing) / `pr-description` (PR description draft) / `trajectory` (trajectory note, inward-facing), or the default (changelog) if unspecified, and **state in the output which format was generated**.
- Hand the chosen format's output-structure rule (`rules/format-changelog.md` / `rules/format-github-releases.md` / `rules/format-changelog-customer.md` / `rules/format-pr-description.md` / `rules/format-trajectory.md`) the matched material from Step 3 (matched commits with their "why" + unmatched commits as thin lines) and assemble it.
...
- Write the assembled release note to `.intent/release-note/release-note.md` by **full replacement** (derived, regenerable). For the `pr-description` format the output destination is `.intent/release-note/pr-description.md`, and for the `trajectory` format it is `.intent/release-note/trajectory.md` (per the format rule's destination — never overwrite the generated changelog).
...
- The body follows the chosen format's output structure (changelog-style = per-kind categories / github-releases-style = narrative + change list / customer-facing changelog = user-impact first / PR description draft = bottom-line-up-front 4 parts with one modest tool-attribution line at the end / trajectory note = inward-facing, the change history under `.intent/` in chronological order newest first, items whose reason cannot be traced marked "no recorded reason") (the layout of `rules/format-*.md`).
