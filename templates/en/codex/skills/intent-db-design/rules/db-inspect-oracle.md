...
- All inspection is **warn-only**. No inspection result stops the projection or output (it is not a gate). The irreversibility warning and the quality checks assume false-positives; their purpose is to let the user **notice**, not to direct and correct (the same shape as context-cost-cues' "notice, do not direct"). Do not confuse this with an enforcement gate (stopping).
