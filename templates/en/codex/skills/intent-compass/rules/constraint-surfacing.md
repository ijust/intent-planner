...
1. **Read the catalog (pull only the relevant domains from the domain index)**
   ...
   - **When consulting the existing compass, partially load it by domain tag too (INV47/DR71)**: when the existing-derivation step (step 4) reads `.intent/intent-compass.md` to check "is this Anti-direction / Invariant already in compass", do not full-load it — grep only the case's domain tag plus `[領域: always]` (cross-cutting rules) with `grep -nE '\[領域: (<the case's domain>|always)\]' .intent/intent-compass.md` and read only the headers/items that hit (compass-category-tag-grep-filter). Always pull `always` too, so cross-cutting invariants are not dropped by the domain filter (dropping them is drift — Anti-direction 226). Items that carry no tag still fall back to a full read as before (backward compatible). This is realized with grep + inline tags only — no DB or embedding (do not add a helper script). If the split store `.intent/compass/` (INV80) has the symbol, read `index.md` → that file's `## Law` first (otherwise keep this rule's legacy grep; DR133).
