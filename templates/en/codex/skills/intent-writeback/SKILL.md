...
- Read `rules/writeback-protocol.md` and identify exactly one target via the 4-tier priority (1. argument → 2. "## Source Packet" in the cc-sdd drafts (latest export only, single slot) → 3. text matching + user confirmation (ask the user in natural language and wait for the answer) → 4. request a specification).
