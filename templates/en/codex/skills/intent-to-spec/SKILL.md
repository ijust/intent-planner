...
- When the user runs `/intent-to-spec`, first interpret the argument's scope hint and format specification per `rules/source-scope.md`. For any axis the arguments alone do not uniquely determine, ask the user in natural language and wait for the answer before confirming (do not fill in by guessing).
