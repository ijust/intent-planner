...
- **Raise section 0 (intent hierarchy map diagram) at the top, and output it only when the material (intent-tree's L0–L4) has been read.** Place it before purpose (1) to let the reader grasp the whole picture first. When there is no material / intent-tree is absent, omit the diagram and state the reason (do not output an empty diagram). See "Intent hierarchy map diagram" below.
