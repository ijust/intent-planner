...
- Legacy-format detection (packets): when the legacy `.intent/packets.md` remains, announce "it will be migrated automatically into the new structure (under packets/) on the next `/intent-packets` run" (do not treat the remnant as a healthy state).
