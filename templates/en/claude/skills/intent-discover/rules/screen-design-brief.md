...
- **Lightweight loop log**: during the revision loop, carry forward one line per version (version number, what changed, a summary of the requester's reaction) right after the provenance comment at the top of the mock, alongside the critique record (e.g. `v3: shifted the palette warmer (text hard to read → raised contrast)`). Keep only the current version of the file and leave the full-replacement rule unchanged — create no numbered version files and no history directory. For "go back to the previous one", regenerate that state from the log's description (and point to the git history when the file is under git). When the log cannot reproduce it, say so honestly and return to probing (exploration research finds that records of past alternatives prevent repeating the same trials: HistoryPalette, CHI 2026 / DesignTrace, CHI 2026).
