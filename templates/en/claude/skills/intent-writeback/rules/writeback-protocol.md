...
**Directory identification rule (packet name → directory)**: the source of truth for identifying a directory from a packet name is "the `## Source Packet` heading in the selected exit's draft matches the packet name". Slug computation is a fast path for searching; even if the slug matches, do not identify the directory as that packet's when the heading does not match.
...
- Even after writeback completes, the target packet's drafts under the selected exit (`.intent/cc-sdd/<packet-slug>/`, `.intent/openspec/<packet-slug>/`, or `.intent/speckit/<packet-slug>/`) are **never deleted** (they persist per packet). Enumerate missed write-backs by cross-checking all rows of export-log (split-form cross-read) × the surviving packet drafts under the selected exit × deltas (split-form cross-read).
- **Direct-implementation cases (exit `direct`; identified at §1 tier 4) are out of scope for the §8 cross-check**: a direct-implementation case that bypasses cc-sdd / openspec / speckit appears in neither export-log nor export drafts, so the missed-writeback enumeration above (export-log × export drafts × deltas) does not detect it. This is a separate axis (omission enumeration, not target identification); do not bring direct-implementation omission enumeration into §8 (§1's target identification handles direct implementation, while §8's omission cross-check stays scoped to cases that have export drafts — INV34).
...
- Source: latest row of export-log.md | Source Packet in the selected exit's packet draft | specified by the user
