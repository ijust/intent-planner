...
- **Milestone-entry guidance (optional, one trailing line; INV78/DR124)**: at the end of the completion report, you may add exactly one read-only guidance line: "If this implementation marks a milestone event (e.g., finalizing the production configuration, going public), recording one event line in `.intent/milestones.md` makes `/intent-improve`'s Revisit matching effective (the user writes it)." (Optional.) The AI never writes to milestones.md, never makes the entry mandatory, and blocks nothing when it stays unwritten. This is **guidance, not a record** — the existing role boundary that no record-writing (drift etc.) is hooked into the writeback path (the "role boundary" section of improve-axes) is unchanged.
- **Switch-over nudge (optional, one trailing line; INV82-(2)/DR143)**: writeback is a work break, so at the end of the completion report you may add exactly one read-only guidance line to the effect of "You can continue as is, or switch to a new session here; to switch over, you can generate a handoff brief with `/intent-overview`." (Optional; put the conclusion [the choices] first = bluf.) **But add it only when the AI has a qualitative sense that "the context is long"** (a work break AND the sense of length; stay silent in a short session). Do not read the conversation log or token amount, and emit no numbers (INV82-(2); the INV22 constraint). In an environment where the handoff-brief feature is not installed, do not emit the generation-trigger guidance (do not point at a dangling command). This is a read-only guidance line of the same shape as the milestone guidance; it never cuts the session automatically, never writes out a brief on its own, and never corrects "continue" (the role boundary is unchanged).
