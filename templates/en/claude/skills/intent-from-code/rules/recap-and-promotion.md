...
1. **The scope read (what was read)** — presents the target scope (directory · module) read-scope fixed and the areas not read under default exclusion (dependencies · generated artifacts), showing the outline of the extraction. Only when an optional local analysis or index was used, also state that it assisted structural observation. Do not make tool names, queries, raw analysis output, or resource values a mandatory recap schema.
