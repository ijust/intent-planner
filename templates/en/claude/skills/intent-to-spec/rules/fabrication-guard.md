...
1. **Trace each description back to the projection source (Req 3.1)**
   ...
   - **Diagram nodes (the intent hierarchy map diagram, Mermaid, etc.) are also subject to reconciliation**: verify that the nodes and hierarchy relations drawn in a diagram trace back to the projection source (intent-tree's headings and their nesting), just like body descriptions. If a diagram draws a node absent from the material or a hierarchy relation absent from the material, treat it as fabrication (the same as not allowing fabrication in the body — INV100). The drawing conventions specific to diagrams (node IDs, forbidden characters, etc.) are the responsibility of overview's `mermaid-tree.md`; this rule only reconciles whether the diagram's content traces to the material.
...
2. **Mark descriptions with no projection-source basis as inferred (Req 3.2)**
   ...
   - **Treat a derived screen-transition diagram as inferred**: for a case with no screen-sketch record, a screen-transition diagram drawn by inferring from the packet's behavior descriptions and UI/UX constraints (when the user chose derivation in the "Material for the screens / touchpoints section" of `rules/format-integrated.md`) is not a confirmed screen design but an **inference**. Make the inferred mark mandatory, do not mix it with confirmed descriptions, and list it in the completed (inferred) list of Step 3 below. Make it traceable to the material it was derived from (which packet's behavior, which constraint) (a trace is attached, but mark that the concrete screen form is an inference).
