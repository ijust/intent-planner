...
- `## Non-scope` — What is not included. **Non-scope exists to prevent unsanctioned expansion during implementation. When evidence arrives that the problem framing itself is wrong (a negative reaction from the user, for example), do not invoke Non-scope to refuse re-evaluation: stop the packet and go back to re-diagnose the upstream intent (Intent Tree, Compass, priorities).** Keep blocking "more requests that still solve the same problem" as before, and treat "the problem we are solving is the wrong one" as the signal to stop and go back (confusing the two is how a wrong problem framing gets completed to a high standard).
...
- `## Expected size` — **Optional**. A declaration for the implementation-time gap check (the execution contract's "Declaration-implementation gap check"). Write two things: the **nature of deliverables** (what kinds of artifacts this work adds) and the **size band** (small/medium/large or similar, in natural language; legitimately large work states "large expected" explicitly). Packets without the section are never checked (optional, backward compatible). Numbers never become mechanical thresholds (the verdict is a comparison by meaning). Never mandatory; never backfilled into existing packets.
...
- `## How to Proceed` — How to proceed from this packet into specification or direct implementation, including the intent, constraints, and verification methods that must carry forward. Read `## cc-sdd Mapping`, `## Handoff`, and `## Next-stage Mapping` in old packets as legacy names for this section; do not require a bulk rename of existing files. Use `## How to Proceed` for newly created or substantively updated packets.
...
- **Optional (recommended)**: `## Out of scope` / `## Verification protocol` and the downstream trace links (realized-by / verified-by), plus the 5 sections for PdM/PjM/service-design use (`## Value (what happens for whom)` / `## Estimate` / `## Risks` / `## Experience stage` / `## Reason parked and revisit cue`) and `## Expected size` (the gap-check declaration). If unfilled, the section **may be omitted** (maintaining the lightweight philosophy that avoids packet bloat and decision fatigue; never made mandatory).
...
- **Absence of the optional sections (Value / Estimate / Risks / Experience stage / Reason parked and revisit cue / Expected size)** → read as "unfilled" (do not fill in by guessing; every old packet behaves as before).
