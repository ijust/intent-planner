...
This directory holds **handoff briefs for switching sessions**. When you ask `/intent-overview` for a "handoff brief", it derives a short brief here from the current state of `.intent/` (active packets, plan, assignment declarations, discovery directories, recent records) so the next session can pick up the work.
...
- Generate: ask `/intent-overview` for something like "make a handoff brief" (a natural-language trigger).
