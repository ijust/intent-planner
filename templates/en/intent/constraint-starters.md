...
| Layer | code / backend (business logic) | `constraint-starters/code-backend.md` | Idempotency/retry safety, transaction boundaries/atomicity, lost-update prevention/locking, fail-safe, time handling, and state machine design (server-side correctness, write consistency, concurrency, temporal values, or state transitions) |
