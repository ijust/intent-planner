...
Domains are ordered along three classification axes. **Layer** (which part of the architecture: frontend / API & boundary / backend / data & persistence / infra) + **cross-cutting concern** (effective across layers: security, AI/LLM) + **process / artifact** (testing & verifiability, VCS hygiene, non-code document/communication). Design (UI/UX) is not a standalone domain; it appears cross-cuttingly inside the frontend layer.
