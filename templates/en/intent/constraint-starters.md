...
| Layer | code / backend (business logic) | `constraint-starters/code-backend.md` | Idempotency/retry safety, transaction boundaries/atomicity, lost-update prevention/locking, fail-safe (correctness of server-side logic, write consistency, work involving concurrent updates) |
...
| Cross-cutting | code / security (cross-cutting) | `constraint-starters/code-security.md` | SQLi, XSS, CSRF, authorization (least privilege), secrets management, preventing sensitive information in logs, security response headers/transport protection (work touching user input, authentication, secrets, or transport; effective across layers) |
