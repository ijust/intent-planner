...
| Layer | code / backend (business logic) | `constraint-starters/code-backend.md` | Idempotency/retry safety, transaction boundaries/atomicity, lost-update prevention/locking, fail-safe, explicit time zones, injectable clocks, zoned day boundaries, temporal type selection, calendar-date validation (server-side correctness, write consistency, concurrent updates, or obtaining/deriving/validating temporal values) |
