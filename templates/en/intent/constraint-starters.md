...
Domains are ordered along three classification axes. **Layer** (which part of the architecture: frontend / API & boundary / backend / data & persistence / infra) + **cross-cutting concern** (effective across layers: architecture quality, security, AI/LLM) + **process / artifact** (testing & verifiability, VCS hygiene, non-code document/communication). Design (UI/UX) is not a standalone domain; it appears cross-cuttingly inside the frontend layer.
...
| Layer | code / backend (business logic) | `constraint-starters/code-backend.md` | Idempotency, transactions, concurrency, time handling, state machines, hierarchical/parallel states, and durable-workflow-engine comparison boundaries (server logic, state transitions, or long-running process control) |
| Layer | code / data & persistence | `constraint-starters/code-data.md` | Backward-compatible migrations, schema integrity, indexing, N+1, connection pools, temporal data models, and immutable data models (DB schemas, historical facts, append-only models, or query efficiency) |
...
| Cross-cutting | code / distributed asynchronous messaging | `constraint-starters/code-async-messaging.md` | Atomic recording of state changes and publication intent, duplicate-safe side effects, business-scoped ordering, bounded in-flight work and queues (work involving queues, events, streams, or asynchronous jobs) |
| Cross-cutting | code / architecture quality (cross-cutting) | `constraint-starters/code-architecture.md` | Measurable quality-attribute scenarios, information-hiding change boundaries, rationale and tradeoffs for significant structural decisions, dependency-boundary conformance checks (work designing or reviewing cross-module structure and quality tradeoffs) |
...
| Process | code / testing & verifiability | `constraint-starters/code-testing.md` | Specification-derived equivalence classes, boundaries, invalid inputs, and test oracles; conditional property-based testing; selective mutation testing for important scopes (work designing or reviewing test strategy, acceptance criteria, or regression tests) |
| Process | code / release, configuration, and recovery | `constraint-starters/code-release-recovery.md` | Configuration semantic validation, observable progressive delivery, mixed-version upgrade/downgrade tests, and restored-data/function verification (work designing or reviewing code or configuration delivery, rolling updates, backup, and recovery) |
...
| Artifact | non-code / document | `constraint-starters/non-code-document.md` | Decks, requirement clarity, documentation types, RFPs, and effort estimation (building decks, specs, procurement documents, or development plans) |
