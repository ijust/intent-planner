...
Before implementing, read only the relevant **packet** and the **Invariant** / Decision Rule that touch it. Do not constantly load the full Compass or full Tree. Do not transcribe Spec/Invariant bodies here; point to the source instead (`.intent/intent-compass.md`, `.intent/intent-tree.md`, the relevant packet under `.intent/packets/`). When compass has grown so large that even a full-text grep is heavy, use the domain tag `[領域: <name>]` at the end of each header/item: pull the case's domain tag together with the cross-cutting `[領域: always]` (`grep -nE '\[領域: (<the case's domain>|always)\]' .intent/intent-compass.md`) and partially load only the items that hit (always include `always` so cross-cutting invariants are not dropped; untagged items fall back to a full read; grep only, no DB — compass-category-tag-grep-filter / INV47 / DR71). (When the split store `.intent/compass/` exists, pull only `index.md` → the symbol's `## Law`; otherwise as before = INV80/DR133.)
