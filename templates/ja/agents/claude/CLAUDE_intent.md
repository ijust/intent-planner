...
> **出口は案件種別で選ぶ**: 実装する案件は選んだSDDツールに対応する上記exportへ、読める成果物（文書・調査メモ等）が目的でSDDツールを使わない案件は `/intent-to-spec` へ。選び方の詳細は出口判定に委ねる。
...
Intent の知能と planning 成果物は `.intent/` にあり、エージェント非依存です（`intent-tree.md`・`intent-compass.md`・`packets/`・mode・SDDツールへ渡す下書き等）。一覧と詳細は `.intent/README.md` を参照してください。
