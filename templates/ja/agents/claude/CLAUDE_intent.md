...
> **出口は1つではなく案件種別で選ぶ**: 実装する案件は cc-sdd（`/intent-export-cc-sdd`）または OpenSpec（`/intent-export-openspec`）へ、spec ツールを使わず読める成果物（文書・調査メモ等）が目的の案件は `/intent-to-spec` へ。選び方の詳細は出口判定に委ねる。

上の4つは「計画」フェーズです。export 後も intent は使い捨てず、維持・随時の4スキルでサイクルとして育て続けます: `/intent-status`（随時・現在地と「次の一手」を1つ推奨・読み取り専用）・`/intent-validate`（export 前・矛盾や漏れの検証・読み取り専用）・`/intent-writeback`（packet 実装後・学びを delta として記録し承認分だけ canonical へ昇格）・`/intent-improve`（節目・`.intent/` と実装の再整合）。使い分けの詳細は `.intent/README.md` の表を参照。
...
Intent の知能と planning 成果物は `.intent/` にあり、エージェント非依存です（`intent-tree.md`・`intent-compass.md`・`packets/`・mode・下流 spec ツールへ渡す下書き等）。一覧と詳細は `.intent/README.md` を参照してください。

## 下流 spec ツール連携（cc-sdd / OpenSpec）

実装する案件は、export が生成する下書き（`.intent/cc-sdd/<スラッグ>/` は cc-sdd の `/kiro-spec-init` へ・`.intent/openspec/<スラッグ>/` は OpenSpec のフローへ）を渡して spec 駆動フローへ引き継ぎます。intent-planner は下書きまでで、本体は下流 spec ツールが生成し各フェーズで人間がレビューします。spec ツールを使わない案件は `/intent-to-spec` で `.intent/nl-spec/` に出します。
