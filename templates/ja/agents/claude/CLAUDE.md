...
1. `/intent-discover` — Intent Tree（L0–L4）を構築し、Intent の詰め方モードを推奨・確定し、問いの代行（designer-questions）の要否を確認・記録する
...
`/intent-export-cc-sdd` が生成する対象 packet の `.intent/cc-sdd/<スラッグ>/requirements.md` を cc-sdd の `/kiro-spec-init` に渡すことで、Intent Planning の成果を cc-sdd の spec 駆動フローへ引き継げます。intent-planner は下書きまでで、本体は cc-sdd が生成し各フェーズで人間がレビューします。
