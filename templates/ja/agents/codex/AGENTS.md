...
以下の `/intent-*` は skill 名です。Codex ではスラッシュ付きで入力せず、「`intent-discover` を実行して」のように自然文で依頼してください。
...
実装前に読むのは「該当 **packet** ＋ 関係する **Invariant** / Decision Rule だけ」。Compass 全文・Tree 全文を常時ロードしない。（分割収納 `.intent/compass/` が在れば `index.md`→該当記号の `## Law` だけを引く・無ければ従来どおり＝INV80/DR133）Spec/Invariant 本体はここに転記せず、参照先を案内する（`.intent/intent-compass.md`・`.intent/intent-tree.md`・`.intent/packets/` 配下の該当 packet）。compass が肥大化して全文 grep でも重いときは、各見出し・項目末尾の領域タグ `[領域: <name>]` を使い、案件の領域タグと横断規律の `[領域: always]` を一緒に引いて（`grep -nE '\[領域: (<案件の領域>|always)\]' .intent/intent-compass.md`）当たった項目だけを部分ロードする（`always` を必ず含める＝横断 Invariant を落とさない・タグ未付与は全文読みにフォールバック・DB を入れず grep のみ＝compass-category-tag-grep-filter・INV47・DR71）。
