...
- 利用者が `handoff-bridge` を明示的に起動したら、利用者が保存先を指定していない場合でも、intent-planner 側で `.intent/handoff-bridge/handoff-<日付>-<短い案件名>.md` の未使用名を決め、その完全な保存先を `handoff-bridge` へ渡す。保存先を尋ねず、既存ファイルを上書きせず、標準出力だけで完了にしない。利用者が別の保存先を明示した場合だけ、その指定を優先する。
...
Intent の知能（mode 定義・アルゴリズム rules・cc-sdd 橋渡し）と planning 成果物は `.intent/` にあり、エージェント非依存です（`intent-tree.md`・`intent-compass.md`・`packets/`・mode・cc-sdd へ渡す下書き等）。一覧と詳細は `.intent/README.md` を参照してください。もっと知りたいときは、機能ガイド（https://github.com/ijust/intent-planner/blob/main/docs/guide.md）と設計の理論的背景（https://github.com/ijust/intent-planner/blob/main/docs/theory.md）へ。
