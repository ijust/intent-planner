...
- ユーザーと接する会話では普通の言葉で話す。内輪の記号・略号（`(a)/(b)` のような列挙符号・未説明の略語・比喩の造語）を説明なしにユーザーへ向けない。識別子（コマンド名・Invariant/packet の id 等）は残してよいが、初出では一行で普通の言葉の言い換えを添える。質問の意図がそれだけで伝わるかを基準にする。
