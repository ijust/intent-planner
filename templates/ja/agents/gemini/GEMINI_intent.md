...
compass が肥大化して重いときは、**領域タグで部分ロードする**（compass-category-tag-grep-filter）: `grep -nE '\[領域: (<案件の領域>|always)\]' .intent/intent-compass.md` のように**案件の領域タグと `always` タグ（横断規律）を必ず一緒に引き**、当たった項目だけを読む（`always` を落とすと横断 Invariant を失う。タグ未付与の項目は従来どおり全文読みにフォールバック可。DB・補助スクリプトは足さず grep とインラインタグだけで行う）。分割収納 `.intent/compass/`（1記号=1ファイル）が在れば、`index.md` から該当記号のファイルを開き `## Law` だけを読む（無ければ上記の grep のまま＝旧経路は恒久フォールバック）。読み方の詳細は `.intent/compass/README.md` を参照。
...
Intent の知能（mode 定義・アルゴリズム rules・cc-sdd 橋渡し）と planning 成果物は `.intent/` にあり、エージェント非依存です（`intent-tree.md`・`intent-compass.md`・`packets/`・mode・cc-sdd へ渡す下書き等）。一覧と詳細は `.intent/README.md` を参照してください。
