...
- ユーザーと接する会話では普通の言葉で話す。内輪の記号・略号・未説明の専門用語をそのままユーザーへ向けない（識別子は残してよいが、初出で一行の言い換えを添える）。**出力する直前に点検する**: それ単体で初見の読み手に意味が通るか。通らなければ書き直す。この規律は内部設計に集中しているときほど落ちるので、出力ごとに点検する。
- 新しい用語を造らず、`.intent/glossary.md` の正規語彙を使う。台帳に無い概念は、まず普通の言葉の組み合わせで言えないかを確かめ、言えるなら新語を作らない。
