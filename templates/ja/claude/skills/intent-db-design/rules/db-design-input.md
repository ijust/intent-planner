...
- **堰（既存の read-only 姿勢と同型）**: 候補提示まで（canonical・既存スキーマ・DB 設計出力を自動改変しない）・当てはまりが弱ければ黙る（無関係な定石を出さない）・意味照合（機械スコアリングしない・INV2/A1）・repo 内のみ・カタログ/台帳/器の不在は沈黙（停止しない・後方互換）。**これは補助であって DB 設計の射影を置き換えない**（三層の読み取り・射影・捏造抑制は従来どおり）。
