...
| coinage-suspect | 品質 | 母集合＝`.intent/glossary.md`（正規語彙の軽量台帳）に照らし、台帳のどこにも無い語を「造語の疑い」として read-only で名指しする。判定は意味的（固有名詞・既存英語用語・初出一行説明済みの正当な新語を除外）で機械検査に寄せない。台帳の行に語の状態（承認済み/暫定/否認済み）が書かれていれば母集合の解釈に反映する: **承認済み**＝チーム共通語として沈黙／**明示の暫定**＝登録済みでも「チーム合意が未了のまま使われている語」として候補提示してよい／**否認済み**＝沈黙集合に入れず、成果物に現れたら「一度否認された語の再発明の疑い」として台帳の該当行（あれば言い換え例）を添えて名指しする。状態が書かれていない行・状態列の無い旧3列の台帳は「暫定」として読むが、検出の出力は従来どおり（登録済み＝沈黙）に保つ（後方互換・新しい検出軸は立てず本軸の母集合解釈の拡張に留める）。対象リポに term-drift（用語の検出・救済ツール）の目印（`.term-drift/`）があるときは、検出のやり方を term-drift の検出 rules（`.term-drift/rules/detect.md`）を Read で読んで LLM がこの文脈内で実行する形へ委ね（外部プロセスを起動しない＝tool 契約は Read/Glob/Grep のまま）、射程に下流の生成文書（`.kiro/specs/` 等・在れば）を含めてよい。目印が無ければ一切発火せず従来どおり（不在時に挙動が変わる実装は誤り＝後方互換）。候補提示に留め断定せず、造語の疑いが無ければ沈黙する | 常時（`.intent/glossary.md` あり） | info |
...
## 造語の疑い検査と term-drift の接合の注記（検出正本の共有・存在検知・後方互換・read-only）

- **term-drift**（用語の検出・救済ツール・別リポジトリの独立ツール）が対象リポに導入されているとき、`coinage-suspect` は**検出のやり方を term-drift の検出 rules に委ねる**（検出アルゴリズムの正本を二重に持たない）。導入の目印は**対象リポに配置された `.term-drift/`**（ディレクトリの存在）。目印が無ければ以下は一切発火せず、従来どおりの台帳照合で動く（**不在時に挙動が1ビットも変わらない**＝後方互換。不在でも案内・検査を足す実装は誤り）。
- **見るのは対象リポに配置された実体だけ**: 検出 rules の解決は `.term-drift/rules/detect.md`（対象リポに配置されたもの）を見る。**`node_modules` や npx のキャッシュを検知・解決の経路にしない**（そこには過去に publish された古いコピーが落ちてくるため、見に行くと開発中の正本でなく過去版を掴む）。目印はあるが rules が見つからないときは、その旨を1行告げて従来の台帳照合へ縮退する（黙って推測で補わない）。
- **読むだけで実行する（tool 契約を破らない）**: term-drift の検出 rules（自然言語）を **Read で読み、その規定どおりに LLM がこの文脈内で実行する**。外部プロセス・CLI を起動しない（`intent-validate` の allowed-tools は `Read, Glob, Grep` のまま。Bash を足したら違反）。委ねるのは検出の**やり方**（多層検出＝台帳差集合＋普通の言葉の内輪転用＋定義への到達可能性、と3分類の運用的定義）であって報告の温度ではない: 深刻度（情報）・候補提示・沈黙・gate にしない・canonical を自動改変しない、はこちら側の規律のまま保つ（INV49）。
- **射程**: term-drift が在るときは、`.intent/` の成果物に加えて**下流の生成文書**（`.kiro/specs/` 等・在れば）も造語検査の対象に含めてよい（`draft-content-dropped` が下流を読む前例と同型・read-only）。下流が無ければ従来どおり `.intent/` の成果物だけを見る。
- **新しい検出軸を立てない**: これは `coinage-suspect` の検出のやり方と射程の拡張であり、検査 ID・区分・深刻度は従来のまま（所見を他軸と混ぜない）。
