...
7. **制約下の意思決定（④）の posture を確認する**
   ...
   - これは決定そのものを今ここで埋める手順ではなく、**どの能力に決定スロット（④）を播くべきかの posture を捉える**手順である。妥当な既定値は提示しない（anchoring bias の回避）。スロットの一覧・発火条件は `intent-packets/rules/decision-slots.md` が正本。
...
off のとき発火するのは、手順 1〜2 の要否確認と、手順 2.5（目的・成功・想定ユーザーの追認）のみ。手順 3〜7（検証性確認・仮説3質問・L1計測基準・画面ラフ・④ posture）と手順 6.5（tree版 recap）は発火しない。L0 目的・成功・想定ユーザーは PoC 専用情報（手順 3〜6）と性質が異なり、product 開発でも意図の根として必要なため、off の最小構成にも含める。purpose の値が残っていても、designer-questions が on と記録されていない限り参照しない。
