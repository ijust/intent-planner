...
- **この照合は `drift-watch` の値に関わらず常時行う（A40・DR83 宿主④）**。上の drift-patterns 照合・コンテキストコストの気づきが `drift-watch: on` 限定なのと異なり、定石の叩き台の気づきだけは off / 未記載 / 不正値でも走る（案件の最初の工程で定石に気づけるのが最も手戻りが小さいため常時化した・利用者確定 2026-07-04）。`.intent/constraint-starters.md` が不在のときは照合をスキップしてその旨を告知する（停止しない）。薄い照合であり、関係領域だけ pull し当てはまりが弱ければ黙る（後述の堰）。
