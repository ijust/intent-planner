...
- `## 次の進め方` — この packet から仕様作成または直接実装へどう進めるか。引き継ぐ意図・制約・検証方法もここに記す。旧 packet の `## cc-sdd Mapping`・`## Handoff`・`## Next-stage Mapping` は同じ節の旧名として読み、既存ファイルへの一括改名を必須にしない。新規作成・内容更新時は `## 次の進め方` を使う。
