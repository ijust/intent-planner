...
- `## Next-stage Mapping` — この packet の意図・制約・検証方法を、選んだ次の工程へどう引き渡すか。旧 packet の `## cc-sdd Mapping` と `## Handoff` は同じ節の旧名として読み、既存ファイルへの一括改名を必須にしない。新規作成・内容更新時は `## Next-stage Mapping` を使う。
