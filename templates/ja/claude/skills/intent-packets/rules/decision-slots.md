...
- **聞き切りの形**: 当該 packet に播いたスロットのうち、load-bearing で未確定のものを**関係するスロットごとにまとめて提示し**、利用者が「答える／後で／非該当」を選んで閉じる。各スロットを1つずつ尋問しない（歯止め・下記）。回答は該当スロットの4ステータス（回答済み／未定／非該当／ADR候補へ送る）で閉じ、「後で」は `未定`（理由・Revisit when 併記）として閉じる（未確定を器に入れて保持する既存規律のまま）。
- **歯止め（INV58・厳守）**: まとめて少数（1バッチ最大4問・一問一答で連射しない）・各問に理由一行（尋問調にしない）・毎回「後で／不明／該当なし」を選べる・回答を強制しない。
- **anchoring 回避は deep でも不変（3規律の①を継承）**: 聞き切りレーンでも「妥当な既定値／推奨値」を先に提示しない。**問いはするが値は置かない**。deep が広げるのは問いの範囲であって、既定値の提示ではない。
