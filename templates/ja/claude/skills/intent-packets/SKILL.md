...
- 事後起草の判別（実装が先行していた場合）: 起動の文脈や利用者の申告から「**対応する Packet が無いまま実装が進んだ／完了した**」ことが分かる場合は、これを通常の起草と同じ手順で扱う（事後でも Packet を起こす。実装済みであることは起草を省く理由にしない）。このとき:
  ...
  - **まだ固定できない仕様**（起動契機・閾値・判定手段など、実装に踏み切れていない／暫定で置いている決定）は、推測で埋めず、作成するpacketファイルの Open Questions または Deferred に記録する。Deferredには `未定（遅延中・再訪条件付き）` と記し、再確認する条件を必ず併記する。「仕様が固定できないからPacketを作らない」は誤りであり、未確定の事項を未確定のまま記録することもPacketの役割である。
...
- **乗り換えの促し（任意・末尾1行・INV82-(2)/DR143）**: packets は工程の切れ目なので、出力の末尾に「このまま実装（出口）へ続けることも、ここで新しいセッションに乗り換えることもできます」という趣旨の read-only の案内を1行だけ添えてよい（任意・結論〔選択肢〕を先に置く＝bluf）。**ただし添えるのは「文脈が長い」という定性的な自己感覚があるときだけ**（工程の切れ目 × 長さの自覚の AND・短いセッションでは黙る）。さらに、出す前に損得を自問する（DR159）: (1) 残作業の性質＝設計判断が残るか・書いてある通りにやるだけか (2) 引き継ぎで失われるセッション固有の暗黙知の量 (3) この切れ目の自然さ。「引き継がない方が得」と見積もったら推奨せず黙る（または「続ける方が得」と一言添える）。見積もれないときも黙る側に倒す。推奨するときは見積もりの一言（定性・数値なし）を案内に添える。会話ログ・トークン量は読まず数値も出さない（INV82-(2)・INV22 の制約）。現在の agent 面で互換確認済みの `handoff-bridge` skill が利用できる場合だけ、その skill を利用者が明示起動できると案内する。起動されたら、保存先は利用者に確認せず intent-planner 側が自動で導出して明示保存先として渡す（`.intent/handoff-bridge/handoff-<日付>-<短い案件名>.md` の形で未使用の名前を決め、既存ファイルとぶつかるときは別名にして上書きしない。利用者が別の保存先を明示したときだけそれを優先する）。生成した handoff はファイル単体で次セッションが再開できる（冒頭の派生注記と Receiver Checks が受け手を正本と最初の行動へ導く）——利用者に口上の別渡しを求めない。完了報告には保存したファイル名を明記する。未配置・非互換・確認失敗の場合は生成の案内を出さないで静かに縮退し、内部 generator や overview へ fallback しない。自動でセッションを切らず・ブリーフを勝手に書き出さず・「続ける」を矯正しない（次の一手＝出口の案内は変えない）。
