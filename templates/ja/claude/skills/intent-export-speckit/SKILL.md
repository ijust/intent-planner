...
- 出力の主役は自然言語案内: 対象 packet の `.intent/speckit/<スラッグ>/specify-input.md` と `spec-hints.md` のパスを示し、「このまま Spec Kit に渡してよいか」を確認する。
- 利用者が続行を指示したら、対象 packet の `.intent/speckit/<スラッグ>/specify-input.md` から機能記述を読み、それを引数として `/speckit.specify` を起動する（`Skill` を使う。利用者にコピペを強制しない）。
- `/speckit.specify` に渡すのは `specify-input.md` だけとし、内部の `constraint-selection.md` は下流へ渡さない。
