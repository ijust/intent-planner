...
利用者が「成果についての学びを記録する」と明示したときだけ、成果分岐を選ぶ。通常の実装学習を書き戻すのか、リリース後の成果についての学びを記録するのかが曖昧な場合は、その2つを普通の言葉で示して操作種別を確認し、回答を待つ。文脈から推測しない。

「実装の学び」を選んだ場合は、以下の既存5段の対象特定、5観点の抽出、承認、Packet完了処理を従来どおり行う。成果分岐の追加を通常の実装学習の条件や出力に混ぜない。
...
## 1.5 成果分岐の対象と pending 記録

成果分岐でも、対象 packet は§1の5段優先順を変更しない。`active/` で見つからなければ `archive/` も探し、完了済みまたは superseded の対象 packet も受け付ける。

次に Intent Tree から対象L1の本文を逐語引用で特定する。同じ逐語引用が複数ある場合は、周辺見出しまたは位置を候補とともに示し、利用者の選択を待つ。Packet名、表現の似たL1、成果の内容から自動で対応づけない。一意に特定できるまで記録を止める。

対象L1の `成果の物さし:` が未記入でも成果記録を受け付け、物さしの起案が必要と知らせる。誰が・いつ・どこで計測したかという出所が不足していても記録を受け付け、欠けた項目を知らせる。既存観測との重複の疑いがあっても、自動で削除や統合をしない。利用者が別観測として残せるよう、新しい `pending` の観測ブロックをPacket単位deltaへ追記する。

成果分岐の pending 記録の前後で、対象Packetの `state` は変更しない。`closed_at` は変更しない。`spec_refs` は変更しない。Packetの配置場所は変更しない。`index` は変更しない。この段階で追加するのは、新しいpendingの観測だけとする。

外部サービスやファイルから成果データを取得することは行わない。数値の自動スコア化や達成判定は行わない。成果記録を不具合のバグ分類と自動で統合しない。売上記録、利用者ログ、発言録などの生データはdeltaに保存せず、判断に必要な要約だけを記録するよう案内する。
...
| `[bug-intent-wrong]` | 意図の誤り（書いてあった意図自体が間違っていた） | compass の該当 Invariant / Decision Rule への改訂**候補**として提示する（人間承認が歯止め＝§4 の ADR 昇格規約を通す・AI が勝手に compass を書き換えない・Anti-direction 303） |
...
実装結果と突き合わせる再確認候補は、**対象 packet** の `## Decisions` 内の Agent-discretion で、未定の理由と同一項目の `Revisit when` を持つものだけとする。Compass 全体や他 packet から候補を収集しない。候補が実装で現実化しなければ delta の学びへ加えず、現実化しても境界内なら確認を増やさず既存の5観点で扱う。境界を越える差異にだけ、次の共通契約を適用する。

`.intent/execution-contract.md` があれば JIT で読み、実装と合意済み意図の差異へ同契約の境界内／境界越えの判断と A/B/C をそのまま適用する（判断材料・拘束力・選択肢をここへ再定義しない）。選択前は delta を pending のまま保ち、canonical へ昇格しない。Aなら意図側を変えず実装を合意済み設計へ戻す対象として明示し、Bなら根拠付き Decision と supersede の正規経路を完了してから昇格し、Cなら後続 packet 候補として明示して現 packet の canonical へ昇格しない。

契約不在の旧環境ではその事実を明示し、従来の §3 承認フローへ fail-open する。契約不在だけで writeback を停止しない。
...
- **自動で台帳に追記しない。** 追記は利用者が採否を承認してから行う（自動的な蓄積は行わない＝read-only の歯止め＝§3 冒頭「承認なしの自動書き換え禁止」を蓄積側へ延長）。**昇格候補タグの学びが無ければ・候補が全て既載なら何も問わない。**
...
### 成果についての学び（任意・観測ごとに追記）

リリース後の成果を記録するときだけ、次の観測ブロックを使います。ユーザビリティ検証や単一の利用者の声も、他の成果と同じ観測ブロックで記録します。同じ意図に別の結果が得られたらブロックを追記し、過去の観測を上書きせずに残します。対象 L1 に `成果の物さし:` が未記入でも記録を受け付け、物さしの起案が必要と示します。売上、利用者ログ、被験者の発言などの生データは貼らず、結果を要約します。

#### 観測: <ISO 8601 日時または人が区別できる名前>
