...
- **下流タスク分割への引き渡し**: 複数タスクへ分ける場合、packet の各要件・受入項目を少なくとも1つのタスクへ割り当てるよう、tasks ヒントで確認を求める。共同担当は許す。単一タスクなら非該当と明記できる。
...
対象 packet の本文が Graphiti 検索（`graphiti-search-boundary.md` または工程別検索の契約）を明示参照する場合だけ、下書きへ次を追加する。参照が無ければ何も足さない（既存の下書き構成のまま）。

- `requirements.md` の末尾に独立節「## Graphiti Search Conditions」を置き、冒頭に「これは候補・条件であって要件ではない。採否と実行は下流の判断に委ねる」旨を1文で明記する（「関係定石」節と同じ境界）。
- 節の中身は対象 packet と検索契約から写す次の条件だけとし、検索結果・資料本文・秘密を下書きへ埋め込まない:
  - requirements: 適用すべき規則と例外を検索する（対象 group・knowledge kind を明示する）。
  - design: 制約とその理由を検索する。
  - tasks・実装: その作業に必要な規則・例外・根拠だけを検索する（task の範囲を暗黙に広げない）。
  - 共通: 結果は元パスまたはURL・版または内容識別・観測時刻・Episode・有効時期へ遡れる未検証の候補であり、確定は現在の正本を開いて人が行う。Graphiti 不在・時間切れ・古い・出典なし・時期不明は確定根拠にせず、正本と元資料へ戻る。Graphiti と承認済み requirements・design が矛盾したら、影響する task だけを止めて上流確認へ戻す（自動反映しない）。検索は読取専用（状態確認と検索だけ）とする。
- `design.md` ヒントへ「制約と理由の検索」の1項目、`tasks.md` ヒントへ「作業に必要な根拠の検索（invariant 参照つき）」の1項目を足す。
- cc-sdd 本体・`.kiro/` の生成・承認へは介入しない（既存の責任分界のまま）。
