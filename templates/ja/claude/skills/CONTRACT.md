...
- **引き出し・起案の姿勢（`proposals:` 行）の読み取り契約（C82・DR196）**: 発行ディレクトリの `mode.md` には、AI からの提案供給（呼び水・複数案の起案）の姿勢を記録する `proposals:` 行が置かれうる。値域は `on | off` の2トークン。**書き手は intent-discover のみ**（format/question-depth と同じ一元化・DR26 同型）。読み手（discover の問いレーン。将来 compass / packets の起案レーンが同じ行を読む）は read-only で読み、**`off` のときだけ**呼び水・複数案の起案・観点の名乗りと常時の気づきの各レーンを止めて従来の問い中心の体験へ縮退する。**未記載・行なし・未知値・旧 scaffold は on 扱い**（既定 on・DR196。question-depth の「未記載=standard」と向きが逆——この行は既定そのものが on——である点に注意）。
