...
### Step 1: source 解釈（範囲・詳細度を確定する。曖昧/不在なら生成しない）
- 利用者が `/intent-to-spec` を実行したとき、まず引数の範囲ヒント・format 指定・詳細度指定を `rules/source-scope.md` に従って解釈する。範囲が引数だけで一意に定まらない軸は、`AskUserQuestion` で利用者に問い、回答を待ってから確定する（推測で埋めない）。
- **詳細度（output depth）の確認（`rules/source-scope.md` の「詳細度の軸」に従う）**: どこまで詳しく書くか（`概要 / 標準 / 詳細`）を、範囲軸と同じ対話補完に相乗りして確定する。**引数で詳細度が明示されているとき（例:「詳細」「概要で」等）は問わない**（不要な問いを足さない）。**明示されていないときは、生成前に1問** `AskUserQuestion` で確認する（3段を対等に提示し「妥当な既定値」を先に置かない＝anchoring 回避）。これは体裁（format）無指定時に黙って既定を使う扱いとは**意図的に非対称**である（厚みの不足は読み手から見えないため生成前に確認する・詳細は source-scope の当該節）。
- **出力形（output form）の確認（DR189・複数選択可）**: どの形で出すか（`Markdown` / `自己完結 HTML` / `Marp スライド用 Markdown`）を、詳細度確認と同じ位置に相乗りして確定する。**引数で形が明示されているとき（例:「HTML で」「Marp も」等）は問わない**（不要な問いを足さない）。**明示されていないときは、生成前に1問** `AskUserQuestion` で確認する（複数選択可・3形を対等に提示）。無指定・無回答・対話不能のときは既定の `Markdown` のみ（従来互換・既定形は暫定）。選ばれた形だけを Step 5 で書く。
- **画面素材の扱いの確認（DR190・画面ラフ記録が無い案件のみ発火）**: 生成体裁が画面・接点を扱い（統合設計書等）、かつ intent-tree の `## 画面ラフ参照` が**「無い」（対象外でない・パス/リンクも無い）**案件のときだけ、生成前に1問 `AskUserQuestion` で「**推測（inferred）標識付きの画面遷移図を導出する** / **『画面素材なし』と明示するだけ**」を利用者に選ばせる。画面ラフが「パス/リンク」で在る案件・「対象外」の案件では発火しない（在れば射影・対象外なら画面節を立てない）。引数で扱いが明示されていれば問わない。規約の本体は `rules/format-integrated.md` の「画面・接点節の素材」に従う（推測図は inferred 標識必須・捏造しない）。 source scope が曖昧（引数でも対話でも軸が一意に定まらない）、または該当成果物が不在（指定された intent-tree / compass / packet / steering が存在しない・未記入）のときは、自然言語 Spec を**生成せず**、`.intent/nl-spec/` 配下に一切書き込まない。何が曖昧か（利用可能な範囲: 実在するサブツリー・packet 一覧・steering の有無）、または不足している成果物（用意する該当スキル: discover / compass / packets 等）を名指しして提示し、止まる。
