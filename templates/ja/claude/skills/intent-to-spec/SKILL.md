...
- **重要判断の開始時確認**: 自然言語 Spec の生成開始時に確認する。対象範囲の Intent Tree、Intent Compass、packet とそこから参照される Open Questions を読み、`CONTRACT.md` の共通契約に従って人の決定が必要な重要判断かを分類する。重要判断には、少なくとも1つの暫定回答案、理由、推奨を変える条件を示す。利用者から決定、今回の範囲外、範囲限定の明示続行のいずれかを得るまで、影響範囲だけは自然言語 Spec の生成を開始しない。停止範囲に含まれない作業は継続でき、重要判断ではない通常の Open Question は従来どおり継続できる。許された結果を得た後は、影響する成果物を更新または再確認してから、その範囲だけを再開する。
...
- 本 Spec は内部管理上、派生（derived）・再生成可能・正本ではない・Git 非追跡として扱う。ただし、**内部運用の注記は既定では出力しない**。「これは正本ではありません」、正本の所在、`.intent/` の内部パス、Git 管理、派生物であることを説明する定型文は、冒頭・末尾を含めどの出力形にも置かない。利用者が内部運用情報を明示的に求めた場合だけ、顧客向けではない内部版へ含めてよい。実際に inferred の記述がある場合は、内部注記の有無と切り離し、Step 4 に従ってその記述の個別標識と確認用一覧を残す（R4.3）。
