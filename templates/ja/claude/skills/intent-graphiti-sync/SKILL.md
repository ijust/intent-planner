...
6. 承認済みまたは同範囲差分の対象ごとに、同期契約のlocator検査を通過した対象だけを読み取り・取得する。形式（Markdown・テキスト・JSON・PDF・`.docx`・`.pptx`・`.xlsx`・許可Webページ）ごとに、現在の実行環境で使える読解手段で本文と出典位置を抽出する。抽出器・外部製品をインストールしない。元のファイル・ページを変更しない。
7. 旧Office形式・OCRが必要なPDF等、現在の環境で抽出できない対象は、対象と理由を示して`skipped`にし、他の対象の処理を続ける。locator検査・秘密検出で拒否した対象も`skipped`とし、理由の種類だけを示して値・本文を表示しない。
8. 通過した本文を、`project`・`group`・`source`・`contentId`の識別を持つEpisodeとして`upsert`する。上限時間（`upsert` 30,000 ms・`web-fetch` 20,000 ms・retry 0）を保証できない対象、送信が失敗・時間切れの対象は`failed`にする。
9. 結果を`success`・`skipped`・`failed`の3分類で対象ごとに表示し、1件でも`failed`があれば全体を成功と表示しない。
10. `success`の内容識別と確認済み範囲を状態記録へ記録する（形式と置き場は同期契約が定める。次回の差分同期と失敗分だけの再実行に使う）。
