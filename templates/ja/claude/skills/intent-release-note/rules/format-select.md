...
| `changelog-customer`（顧客向け・利用者向け等の同義指定） | `rules/format-changelog-customer.md` | 顧客向け changelog（利用者影響先行・内部語を開く） |
| `pr-description`（PR 説明・PR 下書き等の同義指定） | `rules/format-pr-description.md` | PR 説明の下書き（結論先行の4部構成・出力は `.intent/release-note/pr-description.md`・貼り付けは人） |
| `trajectory`（変遷ノート等の同義指定） | `rules/format-trajectory.md` | 変遷ノート（内向き・読み手=将来の自分・`.intent/` の変更履歴を時系列新しい順・出力は `.intent/release-note/trajectory.md`） |
...
- 委譲先（format-* ルール）: 受け取った素材を、その format の出力構造（セクション・カテゴリ・並び）に配置する。git 読み・照合は行わない（SKILL.md の責務）。
