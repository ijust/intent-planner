...
- **この照合は `drift-watch` の値に関わらず常時行う（A40・DR83 宿主④）**。上の drift-patterns 照合が `drift-watch: on` 限定なのと異なり、定石の叩き台の気づきだけは off / 未記載 / 不正値でも走る（案件の最初の工程で定石に気づけるのが最も手戻りが小さいため常時化した・利用者確定 2026-07-04）。`.intent/constraint-starters.md` が不在のときは照合を省略して、その旨を伝える（処理は停止しない）。関係する領域だけを読み、当てはまりが弱い定石は提示しない（後述の歯止め）。
- **これは提示であって自動転記ではない**。候補に気づかせるだけで、Anti-direction / Invariants へ自動で書き込まない。採否・記述は compass で人が行う（主接点）。
- **既に決まった採否を確認し、同じ定石を再提示しない（INV57・DR84）**: 引き継がれた発行ディレクトリに `constraint-ledger.md`（`.intent/discovery/<スラッグ>-<rand>/constraint-ledger.md`）があれば読み、同じ発行において採用済み・否認済みの定石は再提示しない。ファイルが無ければ、過去の採否確認だけを省略して候補の選定を続ける。目的・文脈が否認時から変わったと判断できる場合は、否認済みの定石も候補へ戻してよい（機械条件なし・INV2）。discover で採否が決まったら、`constraint-ledger.md` に1行追記する（`| 定石id | 局面=discover | 採否 | 文脈一行 | 日付 |`）。発行ディレクトリまたはファイルが無ければ、記録だけを省略する。詳細は `.intent/discovery/README.md` の「定石の採否記録」を正とする。この採否記録以外は**どのログにも追記しない**（drift-log にも context-cost 系のログにも書かない）。
...
3. **該当する定石があるとき（気づきの提示・採否記録のみ）**
   ...
   - 採否が決まったら、上記の規則に従って `constraint-ledger.md` に記録する。**このファイル以外のログには追記しない**。

4. **該当する定石がないとき**
   - 該当する定石がなければ何も提示せず、`constraint-ledger.md` にも他のログにも記録しない。
