...
`generated/CONTRACT.md` の同名契約を工程横断で引き継ぐ。各工程の完了を固定した質問数や往復回数から推測せず、共通の確認対象へ現在の工程固有の観点だけを追加する。

- 各質問束の前に、確定した内容と残る内容を示す。回答済み事項を次工程で聞き直さず、前工程から残る未確認事項と現在工程で増えた事項を同じ4状態で扱う。
- 一度だけ行う recap、質問パック照合、観点レビュー、設計フレーム照合を工程全体の終了条件にしない。そこから見つかった重要事項は共通の未確認事項へ加える。
- 工程承認や短い「OK」で重要な未確認事項を解除しない。利用者が質問を終了した場合は新しい質問を止め、その事項が影響する範囲だけを停止し、影響しないと根拠を示せる範囲は進める。
- 独自の質問台帳や質問アルゴリズムは作らず、共通契約と各工程の生成済み指示をそのまま適用する。

### 内部にない専門的な観点

`generated/CONTRACT.md` の「内部にない専門的な観点の補強」を工程横断で引き継ぐ。各工程の内部照合で重要な専門的根拠が足りないと分かったときだけ、調査前の明示確認へ進む。工程が変わっても、同じ判断・文面の調査を重複実行せず、未確認の結果を確定事項へ格上げしない。
