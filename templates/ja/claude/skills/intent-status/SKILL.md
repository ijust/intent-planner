...
- **Success Criteria**（達成されていれば成功。検査手順の詳細は Execution Steps に一本化し、ここでは成果のみ宣言する）:
  ...
  - 「危険な知らせ」（鮮度警告・packets 整合違反・反映漏れ `🔴`）は実害がある場面では絵文字を通常の文字として直接表示し（折りたたまない）、詳細側にも「`⚠` N 件あり（詳細参照）」のサマリ1行を残す。説明の場面（凡例・0件サマリ等）では絵文字を直接表示せず、インラインコードまたは語句でトーンダウンする（INV32）
...
> 凡例の状態表示は見た目をトーンダウンしてインラインコードで示す（説明の場面で裸の警告色を並べない＝INV32）。実際に該当 packet がある実害の場面では INV31 どおり絵文字を通常の文字として直接表示する。
