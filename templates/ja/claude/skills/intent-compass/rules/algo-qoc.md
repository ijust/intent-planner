...
## Compass 固有を共通契約の確認範囲で閉じる

`CONTRACT.md` の共通契約「質問の確認範囲と終了条件」を使い、今回触れる判断について **North Star、Invariant、Decision Rule、Anti-direction** を確認対象にする。選択肢と選択基準、守る制約、避ける失敗方向、決定の帰結と見直し条件を Compass 固有の観点として加える。固定カテゴリ、プレモータム、omission recap は確認事項の発見元であり、その一巡を全体の終了条件にしない。深さ、状態、質問束、追加、終了、停止範囲は共通契約から再定義しない。
...
6. **omission recap で抜け・過剰を確認する**
   ...
   - recap 自体は一度だけ行う。ここで見つかった論点は共通の確認範囲に加え、recap 一度の完了を Compass 全体の終了条件にはしない。
