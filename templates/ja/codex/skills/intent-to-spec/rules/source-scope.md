...
- **「次の進め方」は外向き本文へ写さない**: packet の `## 次の進め方` は、仕様作成または直接実装へ進むための内部情報であり、生成文書の素材に含めない。旧 packet の `## cc-sdd Mapping`・`## Handoff`・`## Next-stage Mapping` も同じ内部情報として扱い、見出し・本文とも生成文書へ写さない。必要な要求・制約・検証内容は、それぞれ `## Expected Behavior` / `## Safety / Invariants` / `## Validation` から読む。
