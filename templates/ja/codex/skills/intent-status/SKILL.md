...
- 工程レールの Packet 名は index の `name` を一次情報とし、index 不在時は active Packet frontmatter の `name` を使う。どちらからも読めない場合は ID だけで継続し、名前を推測・要約しない。
...
- ① **工程一覧**: 全 packet を縦に並べ、各 packet を ✅ 反映済 / 🔵 今ここ / ⚪ 未着手 / 🔴 反映漏れ / ◻ 統合済 のいずれかで示し、続けて `[現在の工程 → 次に通る工程]` を併記する。状態の判定と工程の併記は、overview の `progress-readout.md` にある同じ規律に従う。例: `P2  🔵 今ここ [implementing → 次: verify→writeback]` / `P3  ⚪ 未着手 [ready → 次: export→実装]`。これにより、現在地・残る工程・反映漏れを一覧できる。各状態には用語一覧に従って意味を併記する。この一覧は既存成果物を読み取って表示するだけで、status は何も変更しない。
  - 各行は `<短縮ID> <Packet名> <状態信号> [工程]` の順にする。正本の `name` が32文字以内なら全文、超える場合は先頭32文字と `…` を表示する（`…` は32文字に含めない）。ID は必ず残す。名前を取得できない場合だけ Packet 名を省き、AI が要約・言い換えを作らない。詳細には短縮 ID と `name` 全文の対応を示す。例: `3q0c チーム opt-in の確立と着手宣言の共有… ⚪ 未着手 [ready → 次: export→実装]`。状態判定、decision-table の first-match、次の一手は変えない。
