# コードから意図を逆引きできない領域一覧（coverage map・対象範囲の指定があるときだけ生成する面）

指定されたコード領域を対応する意図を確認できるか、read-only で突合し、`.intent/overview/coverage-map.md` へ派生出力する手順。`intent-overview` の Step 2 で、**利用者が対象範囲を指定したときだけ**適用する（無指定の既定実行では本面を生成せず、Output の誘導1行だけを出す）。逆読み（コード→intent の内向き射影）をどこへ向けるかの優先判断の地図であり、判定・スコア化はしない（C38 の面(5)・A49・INV63）。

## 適用条件（既定では走らない）

- 利用者が `/intent-overview` に**対象範囲（ディレクトリ/モジュール）を指定した**ときだけ生成する。全リポ走査を既定にしない（pull 規律・コンテキストコスト）。
- 無指定の既定実行では、本面を生成せず overview.md の誘導1行（「対象範囲を指定すると、各コード領域から対応する意図を確認できるか調べ、その結果を一覧にできる」）に留める。既定実行の出力・所要は従来と変わらない（behavior-preserving）。
...
3. **意図との対応を確認できない領域を列挙する**: (a)(b)(c) の**いずれにも当たらない**領域を、意図との対応を確認できない領域として、根拠（どの面と突合して空だったか・観測できなかった面はどれか）付きで列挙する。各行が出所（読んだファイル）へ辿れる形式にする。
...
- 意図との対応を確認できない領域の節: **3種類の記録を照合しても意図との対応を確認できなかった領域だけを、根拠付きで再掲する。そのような領域がゼロなら、その旨を明示する**（無理に警告を作らない）。
