...
- **Success Criteria**:
  ...
  - 生成物が派生・再生成可能であり正本ではないこと（および Git 非追跡であること）をビュー冒頭に明示している（R1.5）
  - 関心ごとの派生ビュー（意図ビュー / 依存・ブロックビュー / 進捗ビュー）として整理し、進捗を単一％でなく性質の異なる軸で映している
  - 進捗ビューで全 packet を1本の工程レールとして並べ、残工程（⚪ 未着手）と書き戻し漏れ（🔴 反映漏れ）を一望できるようにしている（5信号を read-only で映すのみで、状態を算出・推論しない）
...
- ビュー冒頭に、本ビューが派生（derived）であり・再生成可能であり・正本ではなく・Git 非追跡であることを明示する（R1.2 / R1.3 / R1.5）。
- 関心別の各派生ビュー内にも、それが derived・再生成可能であること（正本ではないこと）を明示する（R9.5）。

## Output Description
- `.intent/overview/overview.md`（派生・再生成可能・Git 非追跡。正本ではない旨をビュー冒頭に明示）。内容は関心別の派生ビューとして構成する:
  ...
  - **進捗ビュー**: 全 packet を5信号（✅ 反映済 / 🔵 今ここ / ⚪ 未着手 / 🔴 反映漏れ / ◻ 統合済）で並べた工程レール（残工程と書き戻し漏れを一望）に続けて、3 軸（意図の安定度 / 実現の完了度 / 証拠の確定度）と各軸の出所、軸間のズレ、設計意図 vs 実装実態のギャップ集約。
- 各ビューに「派生・再生成可能・正本ではない」注記を含む。素材が無いビュー・軸は省略し理由（未観測／未生成）を明示する。
