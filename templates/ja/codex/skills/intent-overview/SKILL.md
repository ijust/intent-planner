...
- **Success Criteria**:
  ...
  - 生成物を派生（derived）として扱い、canonical な `.intent/*.md` 成果物を一切作成・変更・削除していない。書込み先は `.intent/overview/` 配下限定である（R1.2。引き継ぎブリーフのみ `.intent/handoff/` へ書き出す明示の例外・DR142）
  ...
  - 利用者が自然言語で「引き継ぎブリーフ」「handoff」「乗り換え」などを求めたとき、`.intent/` の直列化済み状態から6要素のブリーフを `.intent/handoff/`（git 非追跡）へ派生生成し、会話ログ・トークン量を読まず、冒頭に生成時点と「派生であって正本ではない」を明記し、canonical な `.intent/*.md` には書き込んでいない（C-hb2/INV82・詳細は `rules/handoff-brief.md`）
...
- `rules/handoff-brief.md` — **引き継ぎブリーフ（自然言語トリガ時だけ生成・C-hb2/INV82/DR142/DR144）**: 利用者が「引き継ぎブリーフ」「handoff」「乗り換え」等を求めたときだけ、`.intent/` の直列化済み状態から6要素（現在地・残タスク・正本への pull ポインタ・罠・repo 状態・実走オラクル）のブリーフを `.intent/handoff/`（git 非追跡）へ派生する。**会話ログを読まず・トークン量を測らない**（ハーネスの領分・INV82/INV22）。冒頭に生成時点と「派生であって正本ではない・正本が常に勝つ」を明記し、機微は正本への pull ポインタで示す（全文転記しない）。書き出し先は本ビューに限り `.intent/handoff/`（`.intent/overview/` 配下の境界に対するブリーフ限定の明示例外・DR142）。**無指定の既定実行では生成せず**、Output の誘導1行に留める（behavior-preserving）。
...
- 引き継ぎブリーフのトリガが要求された場合のみ、**書き出し先はブリーフに限り `.intent/handoff/`**（`.intent/overview/` 配下の書込み境界に対するブリーフ限定の明示例外・DR142・git 非追跡）へ全置換する。他の派生ビューの書き出し先（`.intent/overview/` 配下）は不変。canonical な `.intent/*.md` には書き込まない。
...
## Safety & Fallback
- **書込み境界**: 書込み先は `.intent/overview/` 配下限定である。唯一の例外は引き継ぎブリーフで、ブリーフに限り `.intent/handoff/`（git 非追跡）へ書き出す（DR142 の明示例外・Step 2/3 参照）。canonical な `.intent/*.md` は read-only であり、そこへは作成・変更・削除を一切行わない（frontmatter の `Write` は `.intent/overview/` 配下および `.intent/handoff/` への書き込みのためにのみ許可される）。
