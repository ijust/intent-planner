...
5. **採用した制約を個人台帳へ蓄積するか確実に問う（叩き台採用の直後・read-only 提示・人が採否）**
   ...
   - **自動で台帳に追記しない。** 追記は利用者が手動で行うか、利用者の明示承認のもとでのみ行う（自動的な蓄積は行わない＝read-only の堰）。**採用しなければ何も問わず何も追記しない。** ③までの提示の挙動は変えない（本手順は採用後の蓄積の問いを確実にするだけ）。
...
6. **手段向きの制約は実装フェーズでも発火させる（個人台帳のみ・read-only 候補提示）**
   ...
   - 発火点を実装フェーズへ広げても本手順の堰をすべて継承する: 候補提示まで（compass も packet も library も自動で書き込まない）・当てはまりが弱ければ黙る（無関係な制約を出さない・台帳全件を読まない）・意味照合（機械スコアリングしない）・repo 内の `.intent/constraint-library.md` のみ読む（外部証拠源を読まない）・どのログにも記録しない。
