...
- `## 価値（誰に何が起きるか）` — **任意**。この packet が支える利用者・事業の価値と「やらないと何が起きるか」を自然文で書く（点数・スコアを付けない）。export 時に下書きの冒頭文脈へ引き継がれる（結線は export 側の規律）。
...
- **任意（推奨）**: `## Out of scope` / `## Verification protocol` と下流トレースリンク（realized-by / verified-by）、および PdM/PjM/サービスデザイン向けの5節（`## 価値（誰に何が起きるか）` / `## 見積もり` / `## リスク` / `## 体験段階` / `## 保留の理由と再検討の目安`）。未記入なら**節を省略してよい**（packet の肥大化と decision fatigue を避ける軽量思想を維持する。必須化しない）。
...
- **任意5節（価値/見積もり/リスク/体験段階/保留の理由と再検討の目安）の不在** → 「未記入」として読む（推測で埋めない・旧 packet はすべて従来どおり）。
