...
## frontmatter スキーマ（11キー固定）

各 packet ファイルは先頭に YAML frontmatter（`---` 区切り）を持つ。キーは次の **11キー固定**: `packet_id` / `name` / `state` / `created_at` / `updated_at` / `closed_at` / `parent_intents` / `spec_refs` / `superseded_by` / `summary` / `depends_on`。
...
- frontmatter は **11キー固定**のまま変更しない。本節群の追加は**本文節のみ**であり、frontmatter を増やさない（トレースリンクも本文に持ち、frontmatter キーを足さない）。
