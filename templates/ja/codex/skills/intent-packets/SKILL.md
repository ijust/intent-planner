...
- **乗り換えの促し（任意・末尾1行・INV82-(2)/DR143）**: packets は工程の切れ目なので、出力の末尾に「このまま実装（出口）へ続けることも、ここで新しいセッションに乗り換えることもできます。乗り換えるなら `/intent-overview` で引き継ぎブリーフを生成できます」という趣旨の read-only の案内を1行だけ添えてよい（任意・結論〔選択肢〕を先に置く＝bluf）。**ただし添えるのは「文脈が長い」という定性的な自己感覚があるときだけ**（工程の切れ目 × 長さの自覚の AND・短いセッションでは黙る）。会話ログ・トークン量は読まず数値も出さない（INV82-(2)・INV22 の制約）。引き継ぎブリーフ機能が未配置の環境では生成トリガの案内を出さない。自動でセッションを切らず・ブリーフを勝手に書き出さず・「続ける」を矯正しない（次の一手＝出口の案内は変えない）。
