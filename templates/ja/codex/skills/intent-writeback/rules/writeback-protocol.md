...
- **タグだけで問わず「定石性」でふるう（false positive より silence へ倒す）。** タグが `[decision]`／`[invariant-violation]` でも、それだけで自動的に問わない。`[decision]` は実装すればほぼ毎回出る（packet に書いていない判断は普通に発生する）ため、タグ一致を昇格条件にすると毎回発火して推薦が雑になる。個人台帳は**定石（他案件で再利用できる制約の定番）の台帳**なので、**この packet 固有でしか効かない一回性の判断は問わず**、横展開できる定石性（他の案件・他の packet でも同じ制約として効く見込み）が読み取れるものだけに絞る。定石性が弱い／読み取れないときは問わない（沈黙に倒す＝`constraint-surfacing.md` 手順5「starter 機能への信頼を保つため false positive より silence に倒す」と同じ規律を writeback 側にも持ち込む）。この判定は機械的スコアでなく学びの内容から意味で読む（新フィールドは足さない）。
