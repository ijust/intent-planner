...
成果分岐では、観測が `pending` の間は Intent Tree をバイト単位で変更しない。観測内容と投影予定の1行を利用者へ示し、人が明示的に承認した場合だけ対象L1へ投影する。

承認時は現在の Intent Tree を再読し、観測に記録した対象L1の逐語引用と、必要なら対象L1の位置を照合する。逐語引用が一意に解決できた場合だけ更新する。同じ逐語引用が複数あり位置を含めても一意にならない場合は投影しない。候補を示して利用者の選択を待つ。

承認した観測の `Status` を `promoted (<昇格日>)` へ更新する。結果の「価値が出た」「価値が出なかった」「まだ分からない」は、いずれも読み替えず `成果についての学び:` へ反映する。現在結果行は `成果についての学び: <結果> — <要約>（記録: <delta参照>）` とし、観測の結果、要約、delta参照をそのまま辿れるようにする。

`成果についての学び:` はL1ごとに最大1行とする。対象L1にまだ無ければ追加し、すでに `成果についての学び:` がある場合は、承認した最新の観測でその1行だけを置き換える。他のL1行や `成果の物さし:`、`計測基準:` は変更しない。

見送り時は観測の `Status` を `closed (<クローズ日>)` へ更新する。見送った観測は削除しない。Intent Tree はバイト単位で変更しない。見送り理由を記録する場合も既存の観測ブロックへ追記し、観測自体を置き換えない。

反復観測の履歴はdeltaが正本である。過去の `promoted`、`closed`、`pending` の観測は、承認や見送りのたびに削除も上書きもしない。新しい観測は別ブロックへ追記し、反復承認ではIntent Treeの現在結果行だけを最新の承認済み観測へ置き換える。

承認、見送り、反復承認のいずれでも、対象Packetの `state`、`closed_at`、`spec_refs`、配置場所、`index` は変更しない。Packetの完了処理を再実行しない。この成果分岐の承認規約を、通常の実装学習の昇格粒度、5観点、Packet完了処理へ混ぜない。
