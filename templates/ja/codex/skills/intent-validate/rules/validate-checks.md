...
| decision-slot-empty | 最低限の確認 | packet の `## Decisions` 節に播かれた意思決定スロット（④）のうち、値が空（未記入）のもの。理由付きの `未定` は降格規則により情報へ降格 | `## Decisions` 節にスロットが播かれた packet | 推奨 |
| decision-slot-unsown | 最低限の確認 | `## Decisions` 節は存在するが、共通コアスロット（`decision-slots.md` の8 ID）が1つも播かれていない | `## Decisions` 節を持つ packet（節自体が無い旧 packet は未検証対象としてスキップ） | 推奨 |
...
| cross-packet-contract-coverage | カバレッジ | Impact Analysis で「共有契約」と明示された新規・更新対象を同一の出典参照で辿り、少なくとも1つの active packet の `## Safety / Invariants` に保護担当と検証方法が届くか、複数担当の保護内容が両立するか、統合後の保存特性を落とせる統合時オラクルがあるかを意味照合する。未担当・矛盾・統合時オラクル欠落を根拠付きで名指しするが、推奨（warn-only）に留め停止しない。共有契約がない案件は軸ごと沈黙し、archive は対象から除外して遡及変更しない。共有契約の出典参照・統合時オラクルの読み先には、plan.md の共有契約表に加えて在ればジャーニー正本（`.intent/packets/journeys/*.md` の本文）も含める（journeys/ が無ければ従来どおり＝読み先の追加であって置き換えではない・INV103） | 新規・更新対象の Impact Analysis に共有契約の明示がある（無ければスキップ） | 推奨 |
...
| coinage-suspect | 品質 | 最初に project-local な term-drift 配置物（対象リポ直下の `.term-drift/` または対象リポ内の選択中 agent の設定ディレクトリにある `skills/term-drift/SKILL.md`）の有無だけを見る。配置物が一つでも在れば本軸の用語判定を止め、`detect.md` / term-drift rules を読まず、version・hash・必要ファイル集合・agent skill の互換性を再判定せず、既存 installer の `--with-term-drift --dry-run` health 確認と、`ready` 確認後の専用 skill だけを案内する。配置物が一つも無い場合だけ、母集合＝`.intent/glossary.md`（正規語彙の軽量台帳）に照らし、台帳のどこにも無い語を「造語の疑い」として read-only で名指しする。判定は意味的（固有名詞・既存英語用語・初出一行説明済みの正当な新語を除外）で機械検査に寄せない。台帳の行に語の状態（承認済み/暫定/否認済み）が書かれていれば母集合の解釈に反映する: **承認済み**＝チーム共通語として沈黙／**明示の暫定**＝登録済みでも「チーム合意が未了のまま使われている語」として候補提示してよい／**否認済み**＝沈黙集合に入れず、成果物に現れたら「一度否認された語の再発明の疑い」として台帳の該当行（あれば言い換え例）を添えて名指しする。状態が書かれていない行・状態列の無い旧3列の台帳は「暫定」として読むが、検出の出力は従来どおり（登録済み＝沈黙）に保つ（後方互換・**新しい検出軸は立てず**本軸の母集合解釈の拡張に留める）。候補提示に留め断定せず、造語の疑いが無ければ沈黙する | term-drift 配置物なし かつ `.intent/glossary.md` あり（配置物ありでは用語判定をスキップし既存 health 確認へ案内） | info |
...
| unverified-hypothesis | 品質 | 成果物（packet の確信・Decisions の Human-fixed 値・compass の Decision/Invariant 等）に残る「証拠の裏が無いまま確定された仮説（暫定の確信）」を read-only で名指しする。仮説を `.intent/`（Invariant/Decision Rule/過去 delta＝証拠 pool）と照合し、対応する証拠が辿れないものを反証/未検証の観点を添えて挙げる。判定は意味的（証拠が `.intent/` に実在し裏付けられた確信は除外）で機械検査・必須フィールドの有無に寄せない。`groundless-conclusion`（結論に根拠が辿れない）とは検出軸を分け所見を混ぜない（こちらは仮説に証拠の裏が無い・検証の軸）。**証拠として特に次の2種を読む**: **(1) 利用者確認の範囲**＝Human-fixed（人が確定した判断）として記録された原因・解決策に、対応する利用者の確認が辿れるか。利用者が確認したのは症状（「使いにくい」等）までなのに、AI が立てた原因の見立て・解決案まで確定として記録されていれば、証拠の裏が無い確定の疑いとして挙げる（INV97）。**(2) 少数例の代表性**＝少数の参考例に由来して canonical 化されたスキーマ・分類に、代表性の確認（全対象に共通する項目と種別固有の項目の区別・例数の十分性）または「暫定」の標識が辿れるか。どちらも辿れなければ、代表性未確認のまま正規モデルへ昇格した疑いとして挙げる（INV98）。確認範囲が明記された記録・「暫定」標識付き・代表性確認済みのものは沈黙する。候補提示に留め断定せず、疑いが無ければ沈黙する。射程は当該実行の新規/差分に揃え遡及全体スキャンを既定にしない。証拠 pool が空のときは即警告にせず「判定不能」を明示する。canonical を自動改変せず証拠の補完は更新案提示まで | 常時 | info |
...
| draft-content-dropped | 整合 | export 下書き（`.intent/cc-sdd/<スラッグ>/*.md`）と、それを渡して下流の spec ツールが生成した成果物（`.kiro/specs/<feature>/*.md`）を突き合わせ、下書きに載せた中身が下流で落ちていないかを read-only で報告する。見るもの＝**(1) 意図の参照の落ち**＝下書きが引いた Invariant / Decision Rule / 親の意図の参照が、下流の requirements / design / tasks のどこにも残っていない／**(2) 受入基準の材料の落ち**＝下書きの `## Acceptance Material`（受入基準の材料）由来の判定条件が下流の requirements の受入基準に現れていない／**(3) 言葉の規律の落ち**＝下書きが `## Acceptance Material` の末尾に同梱した言葉の規律（平易さの固定文）の趣旨が、下流の requirements / design / tasks のどこにも残っていない（同梱の無い旧下書きでは本観点を黙ってスキップする＝後方互換）。突合の相手の同定は、export 記録の分割ファイル（`.intent/export-log/<packet-slug>.md`）の `- feature:` 追記行を**一次の手がかり**とし、無ければ下書きの `## Source Packet` の packet 名が下流の成果物に含まれるかで補助照合する。どちらでも相手を同定できないときは「突合不能」を1行で告げ、推測で対応づけない（誤検知を出さない）。判定は LLM が両者を読む意味判断で、生存率のスコア化・閾値判定・`scripts/intent-check.mjs`・正規表現の機械的一致には寄せない（INV2/A1）。`export-draft-mismatch`（下書きと packet 定義の整合）/ `requirement-oracle-check`（下書きの受入基準が誤実装を弾けるか）とは検出軸を分け所見を混ぜない（こちらは下書き→下流生成物の落ち）。候補提示に留め断定せず、落ちが無ければ沈黙する。下流の成果物も canonical も自動で書き換えず、gate にせず export・実装を止めない（INV49 の warn-only） | 下流の spec 生成物がある（`.kiro/specs/` に成果物があり、対応する export 下書きもある。どちらか無ければ軸をスキップ） | 推奨 |
| capability-starvation | カバレッジ | intent-tree の North Star / Current Drift に直結する canonical 能力の packet が未 export・未着手のまま、補助的 packet の完了だけが先行している疑いを read-only で報告する。packet frontmatter の state、`packets/plan.md` の工程計画 / Walking Skeleton（在ればジャーニー正本 `.intent/packets/journeys/*.md` の工程計画も同格の読み先に加える・無ければ従来どおり・INV103）、export-log、tree の North Star / Current Drift だけを突き合わせ、LLM の意味判断で候補提示する。日数・完了件数の固定閾値や score を持たず、packet の存在や親参照だけを価値到達の証拠にしない。価値の中心の packet が export 済み・進行中、または Walking Skeleton / 最優先として進んでいる健全な計画では沈黙する。材料が読めない場合は落差なしとせず未検証としてスキップする。canonical や packet の優先順位・state を自動変更せず、gate にしない | tree に North Star / Current Drift 相当の記述があり active packet が複数ある（無ければ未検証として軸をスキップ） | 情報 |
| outcome-provenance-missing | 品質 | Packet単位の成果記録に「成果についての学び」の観測がある場合だけ、出所3項目 `誰が計測したか`・`いつ計測したか`・`どこで計測したか` を LLM が意味で確認する。1項目でも複数項目でも不足していれば、欠けた項目をこの順でまとめて列挙する。出所3項目がすべて記述された観測と「成果についての学び」が存在しない通常案件では沈黙する。分割deltaを優先し、そこに無い旧形式の記録は単一 `.intent/deltas.md` から読む。同じ観測を二重報告せず、同じ入力には同じ欠落項目の所見を返す。他の検査軸と所見を混ぜず、read-only の候補提示に留めて canonical を変更しない。記録・検証・export・実装を止めない warn-only の独立検査とする | 分割または旧形式のdeltaに「成果についての学び」の観測がある（無ければ軸ごと沈黙） | 推奨 |
...
## 最低限の確認に関する注記（推測せず、書かれた内容だけを確認する）

- `decision-slot-empty` / `decision-slot-unsown` は、実装前に最低限決めるべき事項の空欄を検出する。整合性・冪等性・エラー処理・認可などの判断が未記入のまま、export や実装へ進もうとしていることを知らせる。確認項目の定義は `intent-packets/rules/decision-slots.md` に置く。
...
- 理由付きの `未定` は `decision-slot-empty` の所見を「情報」として扱う。意図的に判断を後へ延ばすことは認める。この検査が問題にするのは理由のない空欄であり、すべての項目を直ちに確定することではない。
...
## 判断ライフサイクルと relevance 検査の注記（独立軸・根拠付き・read-only / warn-only）

- `decision-lifecycle-relevance` は、強度昇格、関係する active 判断の欠落、無関係・superseded 判断の現行関門への混入、不明判断の黙殺だけを独立に見る。各所見には検査 ID `decision-lifecycle-relevance`、ファイル名を伴う逐語的な根拠、修正の候補または `unconfirmed` の温度を付ける。疑いが無ければ軸ごと沈黙する。
- `compass-rule-decay`、`decision-rule-mismatch`、`draft-content-dropped` とは突合面を分け、所見を混ぜない。前者は canonical 規律そのものの生死、次は packet と Decision Rule の整合、最後は export 下書きから下流成果物への落ちを扱い、本軸は判断の強度と lifecycle / relevance の選別だけを扱う。
- 検出は read-only / warn-only の一方向報告で、export・実装を止めない。旧4欄 Decision、旧 Compass fallback、canonical data、status を自動変更しない。旧4欄 Decision は `Revisit when` 欄の不足だけを理由に警告しない。
- `compass-rule-decay` が不要な可能性のある Invariant を名指しするときは、Decision 化・packet 固有化・supersede の三整理候補を根拠付きで提示するだけで、原本を変更しない。どれかへ自動確定せず、反映は人の承認後の既存経路へ委ねる。
...
## 下書きの中身の落ち検査の注記（下書き→下流生成物・read-only・LLM 文脈・軸を分ける・INV75/DR122/A60）

- **背景**: 下書きは「渡した」だけでは下流の生成過程で静かに落ちる。本リポジトリの実測（36 export × 下流 spec の突合・2026-07）では、下書きが引いた意図の参照のうち下流の requirements に残るのは約3分の2、生成された各タスクへ不変則参照が転記される率は1割強だった。写像（下書きが受入基準の材料を運ぶ）と案内（フェーズごとにヒントを手渡す）を直しても、下流の生成の癖として再発しうるため、減衰を見張る目を1本置く。
- **突合の相手の同定は記録された対応を優先する**: export 記録の分割ファイル（`.intent/export-log/<packet-slug>.md`）の `- feature:` 追記行に、packet と生成 spec の対応が記録されている。無い場合だけ `## Source Packet` の packet 名の包含で補助照合する。どちらでも同定できないときは「突合不能」を1行で告げ、**推測で対応づけない**（誤検知を出すくらいなら黙る側に倒す）。
- **既存軸と軸を分ける**: `export-draft-mismatch`（下書きと packet 定義の**整合**）/ `requirement-oracle-check`（下書きの受入基準が**誤実装を弁別できるか**）と本検出（**下書きの中身が下流の生成物で落ちていないか**）を別検査として報告し、所見を混ぜない（突合面が違う）。
- **JIT 二層入力の生存と強度を突合する（INV95・DR175）**: 確定材料が下流で消えた **確定材料の欠落**、対象 packet の Agent-discretion と同一項目の `Revisit when` から運んだ非拘束候補が MUST / SHALL / Invariant / 受入条件へ昇格した **再確認候補の拘束力昇格**、無関係な **Tree / Compass / archive の全量注入**を、本軸の下書き→下流突合として推奨で名指しする。`export-draft-mismatch` の packet→下書き整合責務とは分離する。全量注入は、見出し構造・連続引用・出典範囲など**全文と分かる構造的・意味的な証拠**がある場合だけ検出し、単一の必要参照を誤検出しない。候補が無い場合は候補の生存・昇格検査を黙ってスキップする。
- **言葉の規律の突合（DR151 の対の検査側）**: export 下書きが `## Acceptance Material` 末尾に同梱する言葉の規律（平易さの固定文）も突合対象に含める。趣旨が下流のどこか（requirements / design / tasks のいずれか）に残っていれば落ちとしない（逐語一致を要求しない＝意味の読み）。同梱の無い旧下書きでは本観点を黙ってスキップし、「落ちなし」と誤標識しない（後方互換）。
- **スコア化しない**: 「生存率 何％」のような数値化・閾値判定を持たない（INV2）。**どの参照が・どのファイルで落ちたか**を名指しするところまでで、良し悪しの点数を付けない。
- **沈黙・後方互換**: `.kiro/specs/` が無い（cc-sdd を使わない・直接実装する案件）ときは本検出をスキップし、他検査を続行する（「落ちなし」と誤標識しない）。落ちが1件も無いときは本検出に関する出力を一切発火しない。
- **自動改変しない・gate にしない**: 名指しと補い直しの候補は read-only の報告に留め、下流の成果物も canonical も自動で書き換えない。深刻度「推奨」の一方向報告で、export・実装を止めない（誤検知前提・INV49 の warn-only）。

## 造語の疑い検査と term-drift の責務境界（配置検知・health 正本の単一性・後方互換・read-only）

- **最初に配置の有無だけを見る**: 対象リポ直下の `.term-drift/`、または対象リポ内の選択中 agent の設定ディレクトリにある `skills/term-drift/SKILL.md` のどちらかが在れば「term-drift 配置物あり」とみなす。これは専用点検を始める条件ではなく、`coinage-suspect` を止めて案内へ切り替えるための保守的な存在検知にすぎない。
- **配置物ありでは用語判定も health 判定もしない**: `detect.md` を読まず、term-drift rules を実行せず、用語の意味・version・hash・必要ファイル集合・agent skill の互換性を判定しない。配置だけで `ready` と断定しない。intent-validate は他の構造検査を続ける。
- **既存 installer の dry-run へ案内する**: 利用者へ `npx intent-planner . --agent <選択中のagent> --with-term-drift --dry-run` を実行して既存 installer の health を確認するよう短く案内し、そこで `ready` と表示された場合だけ選択中 agent の term-drift 専用 skill から本格点検へ進むよう示す。intent-validate 自身はコマンドや外部プロセスを起動しない（allowed-tools は `Read, Glob, Grep` のまま）。health の正本は installer inspector 一つであり、validate-checks に互換版・hash・必要ファイル集合・agent 対応を複製しない。
- **配置物なしだけ従来検出**: project-local な term-drift 配置物が一つも無いときだけ、従来の台帳照合による `coinage-suspect` を同じ射程・温度で続ける。この分岐では term-drift 案内を出さず、不在環境の挙動を変えない。
- **新しい検出軸を立てない**: 配置検知は `coinage-suspect` の入口分岐であり、新しい検査 ID・health 軸・用語レビュー軸を追加しない。

## 造語の疑い検査の台帳状態の注記（語のライフサイクル・後方互換・read-only）

- `coinage-suspect` の母集合 `.intent/glossary.md` は、行ごとに任意の**状態**（承認済み/暫定/否認済み）と**言い換え例**を持てる（スキーマの正本は台帳側）。状態の解釈: **承認済み**＝チームで合意済みの共通語＝沈黙。**明示の暫定**＝登録した人の暫定の語（チームの合意はまだ）＝台帳に載っていても「チーム合意が未了のまま使われている語」として候補提示してよい（未登録の造語の疑いとは言い回しを分け、承認済みへの昇格または言い換えの検討を添える）。**否認済み**＝チームが「採用しない」と決めた語＝沈黙集合に入れず、成果物に現れたら「一度否認された語の再発明の疑い」として、否認の台帳行（あれば言い換え例）を添えて個別に名指しする（否認済みを台帳に削除せず残す目的＝再発明の防止がここで効く）。
- **後方互換（旧3列は挙動不変・必須）**: 状態が書かれていない行・状態列の無い旧3列の台帳は「暫定」として読む（＝チーム承認済みとは主張できず、承認済みへの昇格は人の1語ずつの個別承認のみ）。ただし**検出の出力は従来どおり**（登録済み＝沈黙・新たな通知も出さない）。明示の暫定だけを候補提示するのは、人が「まだ合意していない」と宣言した語には知らせる価値がある一方、状態を書いていない既存の台帳（配布済みプロジェクト全部）へ通知を浴びせると、黙らせるための一括昇格（承認洗浄）を誘発するため。旧3列の台帳で挙動が変わる実装は誤りとして扱う。
- **新しい検出軸を立てない**: 状態の読みは本軸（`coinage-suspect`）の母集合解釈の拡張であり、検査 ID・区分・深刻度（情報）・温度（意味的判定・候補提示・沈黙・gate にしない・自動改変しない）は従来のまま。登録・状態の昇格・言い換えの適用は人が1語ずつ個別承認する別アクション（一括登録・確認を省いた承認・承認なしの昇格は登録として成立しない）で、検査層は台帳を書き換えない。
