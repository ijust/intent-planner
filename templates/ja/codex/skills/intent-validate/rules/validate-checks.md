...
- **突合の相手の同定は記録された対応を優先する**: export 記録の分割ファイル（`.intent/export-log/<packet-slug>.md`）の `- feature:` 追記行が実線の対応（packet → 生成 spec）を持つ。無い場合だけ `## Source Packet` の packet 名の包含で補助照合する。どちらでも同定できないときは「突合不能」を1行で告げ、**推測で対応づけない**（誤検知を出すくらいなら黙る側に倒す）。
