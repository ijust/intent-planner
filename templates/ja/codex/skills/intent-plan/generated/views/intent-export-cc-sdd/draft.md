...
- **parked 除外**: 候補を提示するとき、`state: parked`（保留＝今はやらない）の packet は**候補列挙から外す**（parked は export の対象にしない）。引数で明示的に parked packet を指定されたときだけ、「これは保留中の packet です。export を続けますか」を自然言語で問い、回答を待つ（確認なしに parked を export しない）。
...
- feature 名による対応記録は fail-open: 取得できない・書き込めないときは何も書かず警告も出さず、export の成否に影響させない（DR121）。
