...
## packets 固有を共通契約の確認範囲で閉じる

`CONTRACT.md` の共通契約「質問の確認範囲と終了条件」を使う。今回触れる L2/L3 の枝と各 packet を確認対象にし、**decision slot、Expected Behavior、Validation**、Non-scope、依存、rollback を packets 固有の観点として加える。スロット表や Example Mapping は確認事項の発見元であり、固定質問表や全体の終了条件ではない。

- 深さ、質問束、追加、終了、停止範囲は共通契約から再定義しない。packet の ready 判定には、スロット固有の状態名ではなく、次の対応後の共通状態を使う。
- `回答済み` → 共通の `回答済み`。`非該当` → 共通の `今回の範囲外`。
- `未定` → 理由、再確認条件、確定まで影響しない根拠がすべてある場合だけ共通の `理由付きで後回し`。一つでも欠ければ共通の `未確認`。
- `ADR候補へ送る` → ADR の判断が解決され packet へ反映されるまでは共通の `未確認`。候補へ送ったこと自体では確認を閉じない。

## 深掘りの聞き切りレーン（question-depth=deep のときのみ・A46・DR86・INV58）

既定（question-depth=standard）でも、共通の確認範囲に入る重要なスロットは推論した案の確認または質問で閉じる。利用者が明示的に **deep（深掘り）** を選んだときは、追加範囲まで聞き切るレーンを重ねる。
...
- **聞き切りの形**: 当該 packet に播いたスロットのうち、load-bearing で未確定のものを**関係するスロットごとにまとめて提示し**、利用者が「答える／後で／非該当」を選べるようにする。各スロットを1つずつ尋問しない（歯止め・下記）。回答は既存のスロット状態へ記録し、上の対応で共通状態を判定する。「後で」は、理由と再確認条件だけでなく影響しない根拠も揃わない限り、ready を許す状態にしない。
- **歯止め（INV58・厳守）**: 共通契約の表示上限でまとめて少数にし、一問一答で連射しない。各問に理由を一行添え（尋問調にしない）、毎回「後で／不明／該当なし」を選べるようにして回答を強制しない。
