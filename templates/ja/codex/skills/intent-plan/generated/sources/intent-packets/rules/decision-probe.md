...
Decision Probe 自体は従来どおり `.intent/` の証拠だけを read-only で読み、外部情報を勝手に取りに行かない。作業単位や後戻りしにくい判断に必要な専門的観点が内部と利用者資料にないときは、probe の証拠に直接混ぜず、`CONTRACT.md` の「内部にない専門的な観点の補強」へ渡す。結果は判断候補とし、利用者の採用前に packet へ転記しない。
## 問いの平易さ点検（出力直前・共通）
