...
- **次工程への反映情報は外向き本文へ写さない**: packet の `## Next-stage Mapping` は、選んだ次工程へ意図・制約・検証方法を渡すための内部情報であり、生成文書の素材に含めない。旧 packet の `## cc-sdd Mapping` と `## Handoff` も同じ内部情報として扱い、見出し・本文とも生成文書へ写さない。必要な要求・制約・検証内容は、それぞれ `## Expected Behavior` / `## Safety / Invariants` / `## Validation` から読む。
...
- **画面素材（画面ラフ参照）**: `intent-tree.md` の `## 画面ラフ参照` セクション（`/intent-discover` が designer-questions=on のとき案件ごとに記録・値は「パス/リンク」または「対象外」または理由付き「無し」）を素材として読む。これは体裁が定める「画面・接点」節（format 系ルール）の素材になる。読み取りの段階では値をそのまま保持する（パス/リンクの実在照合・図化の要否判断は本ルールでなく format 系ルールと fabrication-guard の責務）。「対象外」と記録された案件では画面素材を持たないものとして扱う（画面節を立てない材料）。
