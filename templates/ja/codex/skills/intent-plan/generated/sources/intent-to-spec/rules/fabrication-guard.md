...
2. **射影元に根拠のない記述を inferred として標識する（Req 3.2）**
   ...
   - **推測導出した画面遷移図は inferred として扱う**: 画面ラフ記録が無い案件で、packet の振る舞い記述・UI/UX 制約から推測して描いた画面遷移図（`rules/format-integrated.md` の「画面・接点節の素材」で利用者が導出を選んだとき）は、確定の画面設計ではなく**推測**である。inferred 標識を必須にし、確定の記述と混ぜず、下記手順3の補完（inferred）一覧に載せる。導出の元にした素材（どの packet の振る舞い・どの制約）へは辿れる形にする（トレースは付くが、画面の具体形は推測であることを標識する）。
