...
ユーザー・事業・運用・開発体験にどんな状態変化を起こしたいか。purpose が poc のときは、各 L1 項目に `計測基準:` 行（達成をどう観測・判定するか）を併記する。
...
ユーザー向け画面を含む場合のみ。ラフ（ワイヤーフレーム・スケッチ等）のパスまたはリンク。UI 非該当なら「対象外」を記録する。無しと判断した場合はその理由を書く。
