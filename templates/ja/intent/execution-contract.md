...
- **記録**: 警告・空振り・利用者判定を drift-log の既存9キーで記録します（`pattern: uncatalogued:declaration-gap`（作りすぎ側）または `uncatalogued:declaration-gap-thin`（薄すぎ側）・`stage: export`（既存3値を増やさず、実装段の照合である旨を note に明記する既存の扱い）・`mechanism: declaration-gap`・`outcome: caught` は下書きで確定は利用者判定）。
