...
- 境界内の実装手段は AI が選び、そのまま進めます。
- より良い案が合意済み境界を越える場合、設計と違うことだけを理由に黙って捨てません。一方、良い案であっても無断で実装しません。
- 次の判断材料を提示し、人の回答を待ちます。
  - 新事実
  - 越える境界
  - 利益
  - リスク
  - 理由付きの推奨
  - A. 合意済み設計を維持する
  - B. 設計変更として承認する（該当する canonical の Decision / Invariant / packet を正規経路で更新し、export 済みなら再exportしてから実装を再開）
  - C. 次の packet へ送る
