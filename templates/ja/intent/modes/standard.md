...
意図の言語化を汎用に行う標準（既定）モード。グリーンフィールド専用ではない: 新規プロジェクトに加え、状況特化モード（refactor / behavior-unknown）が当てはまらない既存プロジェクトの意図言語化・機能追加にも使う。組み合わせるアルゴリズム（GORE-lite + QOC + Example Mapping）は全モードの基幹となる汎用の詰め方である。
...
- 既存プロジェクトでの新機能追加や汎用的な意図整理で、refactor / behavior-unknown が当てはまらないとき。**この場合は既存のアーキテクチャ境界・依存方向を Compass の Invariants に明示し、新しい作業が既存境界を壊さないよう拘束する**
