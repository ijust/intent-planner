...
| 次の進め方 | 読める仕様書または直接作業 | 選んだ packet から、非プログラム向けの読める仕様書を作るか、packet に沿って直接作業する |

各アルゴリズムの詳細は、対応する skill の `rules/algo-*.md` にあります。次の進め方は `intent-packets` の `rules/export-route.md` に従います。このモード定義はそれらを「どのフェーズで使うか」の組み合わせ表です。新規 algo は導入せず、既存 algo を非プログラム成果物へ流用します。
...
### 次の進め方
- packet 1つから `/intent-to-spec` で読める仕様書を作るか、packet に沿って直接作業する。
