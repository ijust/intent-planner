...
この契約が確定するのは、同期と明示的完全削除のためにこの契約の範囲で定める規則だけです。`search`の上限値・検査手順は、この契約で確定しません（工程別検索specが確定します）。
...
- 本節が確定するのは手順と上限だけです。共有契約の検証済みprofileでは`purge`系toolは`not-enabled-in-this-spec`の`unavailable`のままであり、実行時に完全削除へ到達するには、共有契約の「profile変更の条件」（一次資料・完全一致tool名・schema・effect・到達最大状態・正負fixtureを同じ変更で）を満たすprofileの有効化が別途必要です。
