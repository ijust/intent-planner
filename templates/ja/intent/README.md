...
UI（利用者向け画面）を含む案件で deep（深掘り）または「画面を設計する観点」を選んだときだけ、`/intent-discover` が画面の目的・情報の優先順位・主要状態・見た目の方向を対話で詰め、推測標識付きの派生下書きを `.intent/nl-spec/screen-design-brief.md` に生成します。下書きへの参照は intent-tree の「画面ラフ参照」に記録され、仕様書生成と export が既存の経路で運びます。画像・完成モック・デザインシステム・ブランドガイドの自動生成は対象外です。
...
実装時の単一の正本は `.intent/execution-contract.md`。対象 packet と関係する Invariant / Decision Rule と一緒に JIT で読み、境界内は実装者の裁量、境界越えだけ人の判断待ちとして扱う。契約が無い旧環境では従来の packet + Compass で続行する。
