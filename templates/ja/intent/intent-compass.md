...
現役の INV / DR / Anti / C / A は `.intent/compass/` の分割収納を正本として参照します。ここは旧形式の読み取りフォールバックとして骨格を残します（分割収納が無い既存プロジェクトも恒久サポート）。
...
- **packet 固有 invariant**（特定の作業単位でのみ守る制約）の正本は各 packet ファイル（`.intent/packets/active/<packet_id>.md`）の Safety / Invariants 節。compass には書かない。仕様作成へ進む場合は、選んだ出口の下書きへ packet ファイルから転記する。直接実装では packet ファイルをそのまま参照する。packet が archive へ移動すると packet 固有 invariant も packet ファイルとともに退場する（compass 側に残骸を残さない）。
