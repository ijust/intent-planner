...
後続の同期機能がlocator検査を通過したローカルファイルを読んだ後、または許可済みURLから内容を取得した後に使う契約です。取得した内容は出所にかかわらず`trusted: false`の`RetrievedUntrustedContent`として扱い、Graphitiを呼ぶ前にguard自身が秘密を検査します。

呼出し側は、同じ呼出し内でlocator guardが発行したlocatorと、そこから実際に読み出した未検証のbodyだけを渡せます。安全性、検査結果、発行者を自己申告できません。
...
guardは次の順序で判定し、locator、body、検査結果を同じ呼出しに結び付けます。判定後に一つでも差し替えた場合は、再利用せず最初から検査します。
...
textとして検査できる内容では、少なくとも次の形式をローカルで検出します。環境変数の内容は、`.env`というファイル名だけでなく、抽出済み本文に秘密用途の変数代入が含まれる場合も拒否します。秘密の値は検査中だけ保持し、判定結果へ写しません。textとして完全に検査できない形式は安全と推測しません。
...
`ApprovedOutboundPayload`は、秘密が検出されず、同じ呼出し内でguardが発行したlocatorとbodyの同一性が保たれた場合だけguard自身が作ります。Graphitiへ渡せるのは、その呼出し内でguardが返した`ApprovedOutboundPayload`だけです。呼出し側が同じ形を組み立てる、`verifiedBy`だけを付ける、保存済みの判定を再利用する、判定後にlocatorまたはbodyを差し替えることを許可しません。
...
拒否報告には安全に示せる対象、理由、検出した秘密の種類だけを含め、秘密値、本文、credentialを含めません。秘密を検出しなかったときも、検査結果を本文と切り離して保存せず、後の呼出しへ持ち越しません。
...
本specのpreflightはcontentを入力に取らず、このpayload検査を実行しません。したがってpreflight中の文書read、秘密検出、`ApprovedOutboundPayload`発行、Graphitiへの文書送信はすべて0件です。
