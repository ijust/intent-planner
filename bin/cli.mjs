...
オプション:
  ...
  --with-term-drift term-drift 0.3.6 は標準導入される。この旧flagは互換性のため受理する
...
Options:
  ...
  --with-term-drift term-drift 0.3.6 is installed by default; this legacy flag remains accepted
...
const MSG_JA = {
  ...
  gitignoreCreate: (dry) =>
    dry
      ? `\n.gitignore を作成予定です (export 下書きを Git 非追跡化)\n`
  ...
  trackedCcSddHeader: (n) => `\n注意: Git 追跡中の export 下書きがあります (${n} 件):\n`,
  // 既定はこの1行の結論のみ（存在・件数は見出し・結論はここ）。全一覧と対処手順の全文は --verbose。
  trackedCcSddBrief:
    `  意図して Git で共有しているなら対応は不要です (戻し方と全一覧は --verbose で表示されます)。\n`,
  trackedCcSddNote:
    `  export 下書き (cc-sdd / OpenSpec / Spec Kit) は、既定ではローカル専用 (Git 非追跡)\n` +
  ...
  agentHeader: (agent, skillDest) => `\n配置エージェント: ${agent} (skill: ${skillDest}/intent-*/)\n`,
  ...
  // 冒頭の結論行（結論先行）: 何が変わったか・対応が要るかを最初の1行で伝える。
  resultLine: ({ placed, updated, skipped, attention }) => {
    const body =
      placed + updated > 0
        ? `新規 ${placed} 件・更新 ${updated} 件を配置しました (スキップ ${skipped} 件)`
        : `変更はありません (${skipped} ファイルは配置済み・最新)`;
    const tail =
      attention > 0 ? `／要対応の注意 ${attention} 件 (下記)` : `／対応が要る項目はありません`;
    return `結果: ${body}${tail}\n`;
  },
  // 既定サマリのスキップ要約1行（内訳の見出し・但し書きの全文は --verbose 側で従来どおり出す）。
  skipSummaryLine: ({ userData, shared, upToDate }) => {
    const parts = [];
    if (userData > 0) parts.push(`あなたのデータを保護 ${userData}`);
    if (shared > 0) parts.push(`共有ファイルを尊重 ${shared}`);
    if (upToDate > 0) parts.push(`既に最新 ${upToDate}`);
    return `スキップ: ${parts.join("・")} (内訳と各ファイルの一覧は --verbose)\n`;
  },
...
const MSG_EN = {
  ...
  gitignoreCreate: (dry) =>
    dry
      ? `\nWould create .gitignore (keeps export drafts out of Git)\n`
  ...
  gitignoreNone: `\n(the export-draft ignore entries are already present in .gitignore)\n`,
  gitignoreSkippedNotGit: `\n(not a git repository, so .gitignore setup was skipped)\n`,
  trackedCcSddHeader: (n) => `\nNote: some export drafts are tracked by Git (${n}):\n`,
  // Default is this one-line conclusion (existence/count live in the header). Full list and
  // untracking instructions are shown with --verbose.
  trackedCcSddBrief:
    `  If you track them in Git on purpose, no action is needed (add --verbose for the full list and how to untrack).\n`,
  trackedCcSddNote:
    `  Export drafts for cc-sdd / OpenSpec / Spec Kit are local-only (untracked)\n` +
  ...
  agentHeader: (agent, skillDest) => `\nAgent: ${agent} (skills: ${skillDest}/intent-*/)\n`,
  ...
  // Leading conclusion line (bottom line up front): what changed and whether anything needs attention.
  resultLine: ({ placed, updated, skipped, attention }) => {
    const body =
      placed + updated > 0
        ? `placed ${placed} new, updated ${updated} (skipped ${skipped})`
        : `no changes (${skipped} files already placed and up to date)`;
    const tail =
      attention > 0
        ? ` / ${attention} notice(s) below need your attention`
        : ` / nothing needs your attention`;
    return `Result: ${body}${tail}\n`;
  },
  // One-line skip summary for the default view (per-category headers and the full notes
  // still appear with --verbose, unchanged).
  skipSummaryLine: ({ userData, shared, upToDate }) => {
    const parts = [];
    if (userData > 0) parts.push(`protecting your data ${userData}`);
    if (shared > 0) parts.push(`respecting shared files ${shared}`);
    if (upToDate > 0) parts.push(`already up to date ${upToDate}`);
    return `Skipped: ${parts.join(", ")} (add --verbose for the breakdown and per-file list)\n`;
  },
  nextAction: (agent) => {
    const toolName = toolNameOf(agent);
    const invoke =
...
export function main() {
  ...
  // 毎回同文の但し書き・対処手順の全文と、ファイル1件ずつの列挙は --verbose 側で従来どおり出す。
  // --dry-run（確認用途）は従来どおり全列挙のまま（結論行も足さない＝プレビュー契約を変えない）。
  // 警告・エラーは既定でも存在・件数・結論を必ず残す（畳むのは同文説明の全文だけ・安全側）。
  ...
  // 冒頭の結果1行（結論先行）。「要対応」は後段で注意ブロックとして出る信号の数
  // （追跡中 export 下書き / 追跡中 mode.local / term-drift 不整合・失敗 / handoff-bridge 失敗 /
  //  --enforce の .git 不在）。新しい検出は持たず、既に表示に使う結果だけを数える。
  if (!opts.dryRun) {
    const attention =
      (trackedExportDrafts.length > 0 ? 1 : 0) +
      (trackedModeLocal ? 1 : 0) +
      (termDriftCode !== 0 ? 1 : 0) +
      (handoffBridgeCode !== 0 ? 1 : 0) +
      (opts.enforce && enforceHookSkippedNoGit ? 1 : 0);
  ...
  if (skipped.length > 0) {
    if (update) {
      // update モードの skip は3つの理由に分かれる。理由を混同させない仕分けはそのまま:
      //   - user-data : ユーザー成果物・ログ・状態（保護）
      //   - shared    : ユーザー領域共有ファイル（既存を尊重・AGENTS.md / pre-push）
      //   - code      : 既にソースと byte 一致＝最新なので更新不要
      // 既定は仕分け済みの件数を1行に畳む（毎回同文の但し書きは出さない）。カテゴリ別の
      // 見出し・ファイル列挙・但し書き全文（データ保護の --force 警告 / --update-shared 案内）は
      // --verbose / --dry-run で従来どおり出す（install-output-brevity・INV56 は危険側操作に
      // 言及する場面の規律のため、既定サマリが --force に言及しない構成はこれに抵触しない）。
  ...
  if (gitignore === "create") {
    ...
    // 整備済み (変更なし) は毎回同文の no-op 報告のため、既定サマリでは畳み --verbose /
    // --dry-run でのみ告知する（install-output-brevity。create / append / skipped-not-git の
    // 3 アクションは変化・警告なので従来どおり既定でも出す）。
    if (showList) process.stdout.write(T.gitignoreNone);
  } else if (gitignore === "skipped-not-git") {
    process.stdout.write(T.gitignoreSkippedNotGit);
  }

  // Git 追跡済みの export 下書きの案内 (4.4)。案内のみで、追跡解除は決して自動実行しない。
  // 既定は「存在・件数（見出し）+ 結論1行（対応の要否）」に畳み、ファイル一覧と対処手順の
  // 全文は --verbose / --dry-run で出す（install-output-brevity・INV117: 警告は存在・件数・
  // 結論を必ず残し、畳むのは毎回同文の説明の全文だけ）。
  if (trackedExportDrafts.length > 0) {
    process.stdout.write(T.trackedCcSddHeader(trackedExportDrafts.length));
    if (showList) {
      for (const f of trackedExportDrafts) process.stdout.write(`  - ${f}\n`);
      process.stdout.write(T.trackedCcSddNote);
    } else {
      process.stdout.write(T.trackedCcSddBrief);
    }
  }
  ...
  if (!opts.dryRun && termDriftCode === 0 && handoffBridgeCode === 0) {
    ...
    process.stdout.write(
      `\n${yellow}🌟 If this tool saved your time, please star us on GitHub → https://github.com/ijust/intent-planner${reset}\n`,
    );
  }
